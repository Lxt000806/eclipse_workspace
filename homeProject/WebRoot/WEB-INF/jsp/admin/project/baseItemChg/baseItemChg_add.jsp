...
<head>
	...
	var iChangeAmount = 0;
	//已有项干系人不可修改标识
	var transActionStakeholderDisabled = false;
	...
	function refreshDataTable(){
		var mydata = JSON.stringify($("#dataTable").jqGrid("getRowData"));
      	$("#dataTable").jqGrid("setGridParam", {
        	...
        	postData: {"params": mydata, "orderBy": "fixareaseq"},
        	page: 1
      	}).trigger("reloadGrid");
	}
	
	function doSave(){
		if("${projectMan}" == "${baseItemChg.lastUpdatedBy}"){
			$("#prjStatus").val("2");
		}
		var datas = $("#dataForm").jsonForm();
		...
		$.ajax({
			url:"${ctx}/admin/baseItemChg/doSave",
			type: "post",
			data:datas,
			dataType: "json",
			cache: false,
	...
	function goTransAction(){
		...
		Global.Dialog.showDialog("baseItemChg_transAction",{
			title:"基装增减明细--已有项变动",
			url:"${ctx}/admin/baseItemChg/goTransAction?custCode=${baseItemChg.custCode}&custTypeType=${baseItemChg.custTypeType}",
	...
	*/
	/*function judgeStakeholderIsDisabled() {
		...
		//当主项目无明细时，重置常规变更和干系人
		if (0 == reqPKDataTable.datas.length) {
			$("#isAddAllInfo").val("1");
	...
	function calcDiscPer(){
		...
		var befAmount = $.trim($("#befAmount").val()); //优惠前金额
		var zhFee = $.trim($("#zhFee").val()); //综合费用
		var baseFeeDirct = $.trim("${baseItemChg.baseFeeDirct }"); //客户信息表的直接费
		...
		datas.detailJson=JSON.stringify(rowData);
	   	art.dialog({
			...
			content:'<input type="checkbox" id="checkCanreplace" />可替换项重新生成：勾选可替换项将被覆盖<br/> '
					...
					+' 此操作将生成新加基础增减明细,原有明细重新计算数量，<br/>是否确定要重新生成 ？',
			ok: function () {
				...
				$.ajax({
			      ...
			      success: function(obj){
					    if(obj.rs){
					  		...
					  		$("#dataTable").jqGrid("setGridParam", {
					        	url: "${ctx}/admin/itemChgDetail/goTmpJqGrid",
					        	postData: {"params": params, "orderBy": "fixareaseq"},
					        	page: 1
					      	}).trigger("reloadGrid");
					  		changeAmount();
					  		art.dialog({
...
<div class="body-box-form">
	...
	<div class="panel panel-info">  
    	<div>
            <form role="form" class="form-search" id="dataForm" action="" method="post" target="targetFrame">
				...
				<input type="hidden" name="prjStatus" id="prjStatus" value="0"/>
				...
				<ul class="ul-form">
					<li>
						<label>批次号</label>
						<input type="text" disabled="disabled" id="no" name="no" value="保存时自动生成" />
					</li>
					<li>
						<label>楼盘</label>
					...
					<li id="discAmountLi">	
						...
						<input type="text" id="discAmount" name="discAmount" onblur="changeAmount()" value="0" />
					</li>
					<li>	
						<label>实际总价</label>
					...
					<li>	
						<label>基础管理费</label>
						<input type="text" id="manageFee" name="manageFee"  value="${baseItemChg.manageFee }" onchange="calculateTax()"/>
					</li>
					</c:if>
						<li>
							<label>税金</label>
					...
					<ul>
						...
						<li>	
							<label>基装增减状态</label>
							<house:xtdm id="status" dictCode="ITEMCHGSTATUS" value="1" disabled="true"></house:xtdm>
						</li>
						<li>	
							<label>变更日期</label>
	...
	<div class="btn-panel">
      	<div class="btn-group-sm">
	    	...
	    	<c:if test="${baseItemChg.signQuoteType =='2' && existsTemp=='false'}">
...
$(function() {
	...
	var jqGridOption = {
		...
		colModel : [
			...
			{name: "fixareaseq", index: "fixareaseq", width: 30, label: "fixareaseq", sortable: true, align: "left",hidden:true },
			...
			{name: "isoutsetdescr", index: "isoutsetdescr", width: 55, label: "套外", sortable: true, align: "left",},
        ...
        afterSaveCell:function(id,name,val,iRow,iCol){
            ...
            switch (name){
            	case "qty":
                	...
                	break;
            	case "unitprice":
            		$("#dataTable").setCell(id, "lineamount", myRound(rowData.qty * parseFloat(val) + rowData.qty * parseFloat(rowData.material), 0));
            		changeAmount();
            		break;
            	case "material":
            		$("#dataTable").setCell(id, "lineamount", myRound(rowData.qty * parseFloat(rowData.unitprice) +rowData.qty * parseFloat(val), 0));
            		changeAmount();
            		break;
            	default :
                	break;
