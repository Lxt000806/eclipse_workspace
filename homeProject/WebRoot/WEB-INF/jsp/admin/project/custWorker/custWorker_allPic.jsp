		...
		<div class="panel panel-system" >
			<div class="panel-body" >
				<div class="btn-group-xs" >
					...
					&nbsp;&nbsp;&nbsp;该明细：<span id="btn_span" style="font-weight:bold;"></span>
				</div>
			</div>
		</div>
...
$(function(){
	 Global.JqGrid.initJqGrid("dataTable",{
		url:"${ctx}/admin/custWorker/goWorkSignPic",
		postData:{workSignNo:"${no}"},
		colModel : [
		...
		gridComplete:function(){
			...
			for(var i=0;i<photoName.length;i++){
				...
				console.log($.trim("${photoPath }")+photoName[i]);
				var li="<li id='li_"+i+"' style='float: left;margin-right:20px;margin-top:5px'>"
						+"<div>"
							+"<div style='width:210px;height:210px'>"					//http://localhost:8080/homeProject/homePhoto/prjProg/
								+"<img style='float:center' id='conPicture"+i+"' src='"+$.trim("${photoPath }")+photoName[i]+"' width='210' height='210'>"
							+"</div>"
	...
	function checkPic(pk,i){
		...
		$.ajax({
			url:"${ctx}/admin/custWorker/updateIsPushCust",
			type:'post',
	...
	function updateView(val){
		...
		$.ajax({
			url:"${ctx}/admin/custWorker/updateIsPushCustAll",
			type:'post',
			data:{workSignNo:"${no}",isPushCust:isPushCust},
			dataType:'json',
			cache:false,
			error:function(obj){
				showAjaxHtml({"hidden": false, "msg": '获取数据出错~'});
