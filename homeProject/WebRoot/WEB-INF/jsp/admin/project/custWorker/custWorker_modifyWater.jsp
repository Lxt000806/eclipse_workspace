...
<head>
	...
	$(function() {
		...
		// 水电工资发放后，不允许更改水电材料发包工人标记。 modify by zb on 20200327
		if ("1" == "${isWaterCostPay}") {
			$("#isWaterItemCtrl").prop("disabled","true");
		}
	
		$("#workerCode").openComponent_newWorker({callBack:function(){validateRefresh();},showValue:"${custWorker.workerCode}",showLabel:"${worker.nameChi}",condition:{workType12:$.trim($("#workType12").val()),isSign:"1",custCode:$.trim($("#custCode").val()),normDay:$("#constructDay").val()},readonly:true});
					...
					<ul class="ul-form">
						...
						<div class="validate-group row">
							...
							<li class="form-validate">
								...
								value="<fmt:formatDate value='${custWorker.comeDate}' pattern='yyyy-MM-dd hh:mm:ss'/>" disabled="true"/>
							</li>
						</div>
						<div class="validate-group row">
							<li class="form-validate">
						...
						<div class="validate-group row">
							<li class="form-validate">
								...
								<input type="text" id="endDate" name="endDate" onchange="validateRefresh()" class="i-date" style="width:160px;"  
										...
										value="<fmt:formatDate value='${custWorker.endDate}' pattern='yyyy-MM-dd'/>" disabled="true"/>
							</li>
