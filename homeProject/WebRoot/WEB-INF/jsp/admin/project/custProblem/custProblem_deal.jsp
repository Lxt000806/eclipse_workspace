...
<script type="text/javascript"> 
	...
	$("#deal").on("click", function() {
			...
			$.ajax({
				url : "${ctx}/admin/custProblem/doDeal",
				type : "post",
				data : {no:$('#no').val(),pk:$('#pk').val(),dealDate:$('#dealDate').val(),dealRemarks:$('#dealRemarks').val()},
				dataType : "json",
				cache : false,
				error : function(obj) {
					showAjaxHtml({
					...
					<ul class="ul-form">
						...
						<div class="validate-group row">
							...
							<li class="form-validate"><label><span
									class="required">*</span>实际处理时间</label> <input type="text"
								id="dealDate" name="dealDate" class="i-date"
								style="width:160px;" onchange="validateRefresh()" required
								data-bv-notempty-message="实际处理时间不能为空"
								onFocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd'})"
						...
						<div class="validate-group row">
							...
							<li class="form-validate"><label class="control-textarea"><span
									class="required">*</span>处理结果</label>
								<textarea id="dealRemarks" name="dealRemarks" 
								style="overflow-y:scroll; overflow-x:hidden; height:75px;"/>${custProblem.dealRemarks}
								</textarea></li>
						</div>
						<div class="validate-group row">
						...
						<div class="validate-group row">
							<li class="form-validate"><label class="control-textarea">投诉内容</label>
								<textarea id="remarks" name="remarks"
									...
									disabled="true" />${custProblem.remarks }</textarea>
							</li>
						</div>
					</ul>
				</form>
