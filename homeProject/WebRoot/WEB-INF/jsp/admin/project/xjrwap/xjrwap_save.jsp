			  ...
			  <form role="form" class="form-search" id="page_form"  action="" method="post" target="targetFrame" >
					...
					<input type="hidden" name="jsonString" value="" />
						<ul class="ul-form">
							...
							<li>
								<label>编号</label>
								<input type="text" id="No" name="No" style="width:160px;" placeholder="保存自动生成" value="${progCheckPlan.no }" readonly="readonly"/>                                             
							</li>
							<li>
								<label><span class="required">*</span>巡检类型</label>
								<house:xtdm id="type" dictCode="CHECKPLANTYPE"  value="${progCheckPlan.type }" ></house:xtdm>
							</li>	
							<li>
								<label><span class="required">*</span>巡检人</label>
							...
							<li class="form-validate">
								...
								<input type="text" id="crtDate" name="crtDate" class="i-date" style="width:160px;" onFocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd'})" 
									onchange="validateRefresh()" value="<fmt:formatDate value='${progCheckPlan.crtDate}' pattern='yyyy-MM-dd'/>"  />
							</li>	
							<li>
...
function save(){
	...
	var param= Global.JqGrid.allToJson("dataTable");
		...
		Global.Form.submit("page_form","${ctx}/admin/xjrwap/doSave",param,function(ret){
			if(ret.rs==true){
				art.dialog({
					content:'保存成功',
					time:1000,
...
$(function(){
	...
	var gridOption = {
		...
		colModel : [
			...
			{name: "pk", index: "pk", width: 50, label: "编号", sortable: true, align: "left",hidden:true ,count:true},
			...
			{name: "appdate", index: "appdate", width: 129, label: "巡检日期", sortable: true, align: "left", formatter: formatTime},
	...
	$("#froAdd").on("click",function(){
		...
		var apppk = Global.JqGrid.allToJson("dataTable","apppk");
		var arr = new Array();
			arr = apppk.fieldJson.split(",");
	...
	$("#delDetail").on("click",function(){
		...
		var froNum = Global.JqGrid.allToJson("dataTable","froNum");
           ...
           $("#dataTable").jqGrid('setSelection',id+1, true);
		var Ids =$("#dataTable").getDataIDs();
		if(Ids.length>x){
