...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#add").on("click",function(){
				Global.Dialog.showDialog("goSave",{
					title:"基础算量模板管理--增加",
					url:"${ctx}/admin/basePlanTemp/goSave",
				    height:700,
				    width:1350,
		...
		$("#update").on("click",function(){
				...
				Global.Dialog.showDialog("goUpdate",{
					title:"基础算量模板管理--编辑",
					url:"${ctx}/admin/basePlanTemp/goUpdate?id="+ret.no,
				    height:700,
				    width:1350,
					returnFun:goto_query
				});
		...
		$("#view").on("click",function(){
				...
				Global.Dialog.showDialog("goDetail",{
					title:"基础算量模板管理--查看",
					url:"${ctx}/admin/basePlanTemp/goDetail?id="+ret.no,
				    height:700,
				    width:1350,
					returnFun:goto_query
				});
	...
	function doExcel(){
		var url = "${ctx}/admin/basePlanTemp/doExcel";
		doExcelAll(url);
	}
		function clearForm(){
		$("#page_form input[type='text']").val("");
...
<body>
	...
	<div class="btn-panel">
		<div class="btn-group-sm">
			<house:authorize authCode="BASEPLANTEMP_ADD">
				<button type="button" class="btn btn-system" id="add">
					<span>新增</span>
				</button>
			</house:authorize>
			<house:authorize authCode="BASEPLANTEMP_UPDATE">
				<button type="button" class="btn btn-system" id="update">
					<span>编辑</span>
				</button>
			</house:authorize>
			<house:authorize authCode="BASEPLANTEMP_VIEW">
				<button type="button" class="btn btn-system" id="view">
					<span>查看</span>
				</button>
			</house:authorize>
			<house:authorize authCode="BASEPLANTEMP_EXCEL">
