...
function save(){
	...
	var datas = $("#dataForm").jsonForm();//form表单
	var custTypeJson=allToJson('dataTable_custType','custTypeJson');//适用产品线表格数据
	var itemJson=allToJson('dataTable_item','itemJson');//对应材料表格数据
	$.extend(datas,custTypeJson);//合并json
	$.extend(datas,itemJson);
	$.ajax({
		url:"${ctx}/admin/gift/doSave",
		type: "post",
		data: datas,
		dataType: "json",
		cache: false,
...
function changeType(){
	 ...
	 // 现金
	 if($("#type").val()=="1"){
		 $("#isProvideDiscToken").removeAttr("disabled");
	 }else{
		 $("#isProvideDiscToken").val("0");
		 $("#isProvideDiscToken").attr("disabled",true); 
	 }
}
</script>
</head>
				...
				<div class="body-box-form">
					<form role="form" class="form-search" id="dataForm" action=""
						...
						<div class="validate-group"> 
							<li class="form-validate"><label><span class="required">*</span>优惠标题</label> <input
								type="text" id="descr" name="descr" value="${gift.descr}"/>
							</li>
							...
							<li class="form-validate"><label><span
									class="required">*</span>优惠金额分类</label> <select id="discAmtType"
							...
							<div class="validate-group">
							<li class="form-validate"><label><span class="required">*</span>活动名称</label> <input
								type="text" id="actName" name="actName" value="${gift.actName}"/>
							</li>
							<li class="form-validate">							
								<label><span class="required">*</span>优惠比例</label>
								<input type="text" id="discPer" name="discPer" style="width:160px;" value="${gift.discPer}"/>
							</li>
							<li class="form-validate"><label style="width:120px;">报价模块</label> <select id="quoteModule"
								name="quoteModule"></select>
							</li>
							<li class="form-validate">
							...
							<div class="validate-group">
							<li class="form-validate"><label class="control-textarea"><span class="required">*</span>优惠项目说明</label>
								<textarea id="remarks" name="remarks">${gift.remarks }</textarea>
							</li>
							<li class="form-validate"><label class="control-textarea" style="width:120px;">最高优惠金额公式</label>
							...
							<li class="form-validate">
								...
								<house:xtdm id="isSoftToken" dictCode="YESNO" value="${gift.m_umState == 'A' ? '0' : gift.isSoftToken}"></house:xtdm>													
							</li>
							<li class="form-validate">
							...
							<div class="validate-group">
								...
								<li class="form-validate">
									<label>是否发放优惠券</label>
									<house:xtdm id="isProvideDiscToken" dictCode="YESNO" value="${gift.isProvideDiscToken}" style="width:160px;"></house:xtdm>													
								</li>
								<li class="form-validate">							
									<label style="width:120px;">审批级别</label>
								...
								<c:if test="${gift.m_umState=='M'}">
									<li><label>过期</label> <input type="checkbox" id="expired_show"
										name="expired_show" value="${gift.expired }"
										${gift.expired!='F' ?'checked':'' } onclick="checkExpired(this)">
									</li>
								</c:if>
							</div>
						</ul>
