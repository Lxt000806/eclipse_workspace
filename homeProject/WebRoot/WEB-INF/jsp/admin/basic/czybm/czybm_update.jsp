...
$(document).ready(function(){
	...
	$.fn.zTree.init($("#treeDept"), setting2, zNodesDept);
	$.fn.zTree.init($("#treeCkAll"), setting, zNodesCkAll);
	$.fn.zTree.init($("#treeCkSelect"), setting2, zNodesCkSelect);
	...
	//部门树先展开后收起,未展开获取不到节点的class 
	zTree.expandAll(true);
	zTree.expandAll(false); 
	if("${operatorType}"!="ADMIN"){
    	$("#jslx").attr("disabled", true);
    }
...
function save(){
	...
	//alert("add="+addIdsCkAll.join(","));return;
	//验证
	var datas = $("#dataForm").jsonForm();
	$.ajax({
		url:'${ctx}/admin/czybm/doUpdate?addIds='+addIds.join(',')+'&addIdsCk='+addIdsCkAll.join(','),
		type: 'post',
		data: datas,
		dataType: 'json',
		cache: false,
...
$(function() {
	if ('${czybm.czylb}'=='2'){
		...
		$("#emnum").setComponent_supplier({showLabel:'${czybm.emnumDescr}',showValue:'${czybm.emnum}'});
		...
		$("#emnum").setComponent_employee({showLabel:'${czybm.emnumDescr}',showValue:'${czybm.emnum}'});
	...
	$("#dataForm").bootstrapValidator({
		...
		$("#openComponent_employee_emnum").attr("readonly", true);
		$("#openComponent_employee_emnum").next().attr("data-disabled", true);
		$("#openComponent_supplier_emnum").attr("readonly", true);
		$("#openComponent_supplier_emnum").next().attr("data-disabled", true);
});
