...
<script type="text/javascript">
	function save() {
	    ...
	    //第一次尝试保存
	    $.ajax({
                url : "${ctx}/admin/applicant/doSave",
                type : "post",
                data : datas1,
                dataType : "json",
                cache : false,
	    ...
	    xhr1.open("post", "${ctx}/admin/applicant/doSave", true);
        xhr1.onload = function(){
            ...
            if(resp1.success === false && resp1.errorcode === "idNumExist"){
                art.dialog({
                    ...
                    //第二次保存
                    ok:function(){
                        formData.set("checkIdNumExist", false);
                        var xhr2 = new XMLHttpRequest();
                        xhr2.open("post", "${ctx}/admin/applicant/doSave", true);
         ...
         <ul class="ul-form">
             <div class="validate-group row">
                 <li class="form-validate"><label>序号</label> <input type="text" id="pk" name="pk"
                     value="${applicant.pk == null ? '保存时自动生成' : applicant.pk}" disabled="disabled" />
                </li>
