...
function validateCom(itemType2,itemType3,itemDesc){
	...
	if("${itemAppConfRule.itemType2}"==$("#itemType2").val() && "${itemAppConfRule.itemType3}"==$("#itemType3").val() && "${itemAppConfRule.itemDesc}"==$("#itemDesc").val()){
		isPass=0;
	}
	return isPass;
} 
$(function(){
	...
	Global.LinkSelect.setSelect({
			...
			secondValue:$.trim("${itemAppConfRule.itemType2}"),
			thirdSelect:"itemType3",
			thirdValue:$.trim("${itemAppConfRule.itemType3}"),
	});
	
});
$(function(){
	$("#itemDesc").val("${itemAppConfRule.itemDesc}");		
});	
function saveBtn(){
	var itemType2=$.trim($("#itemType2").val());	
	var itemType3=$.trim($("#itemType3").val());
	var itemDesc=$.trim($("#itemDesc").val());
	if($.trim($("#itemType2").val())=="" && $.trim($("#itemType3").val())=="" && $.trim($("#itemDesc").val())==""){
		art.dialog({
			content:"保存时数据不能全部为空"
		});
		return;
					...
					<form action="" method="post" id="dataForm" class="form-search">
						...
						<ul class="ul-form">
							...
							<li class="form-validate">
								<label>材料名称</label>
								<input type="text" id="itemDesc" name="itemDesc" value="${itemAppConfRule.itemDesc}"/>
							</li>
						</ul>
					</form>
				</div>
