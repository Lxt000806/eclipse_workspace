  ...
  <head>
	...
	<script src="${resourceRoot}/pub/component_cmpactivity.js?v=${v}" type="text/javascript"></script>
  	<script type="text/javascript">
		...
		function clearForm(){
			$("#dataForm input[type='text'][id!='custCode'][id!='no'][id!='openComponent_cmpactivity_no'][id!='openComponent_customer_custCode']").val('');
			$("#dataForm select").val("");
			$("#status_disabled").val("1");
			$("#status").val("1");/* 
			$("#custCode").isValid();
			$("#no").isValid();
			$("#openComponent_cmpactivity_no").isValid();
			$("#openComponent_cmpactivity_no").isValid(); */
		}
  		function save(){
			...
			$.ajax({
			    ...
			    success: function(obj){
			    	if(obj.rs){
			    		var data = listStrToJson(obj.msg);
			    		art.dialog({
							content: data[0].info,
							time: 1000,
							beforeunload: function () {
								if(data[0].info != "该券号已经登记过"){
			    					clearForm();
			    					$("#dataForm").bootstrapValidator("resetForm");
			    					$("#dataForm").data("bootstrapValidator").validateField("openComponent_cmpactivity_no");
			    					$("#dataForm").data("bootstrapValidator").validateField("openComponent_customer_custCode"); 
						   			$("#itemCode").openComponent_item({
						   				showValue:"${giftToken.itemCode}",
						   				showLabel:"${giftToken.itemDescr}",
						   				condition:{
