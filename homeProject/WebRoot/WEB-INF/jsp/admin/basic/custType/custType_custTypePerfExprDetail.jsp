...
$(function(){
	/**初始化表格*/
	var gridOption ={
			url:"${ctx}/admin/custTypePerfExpr/goJqGrid",
			...
			colModel : [
				...
				{name: "perfexpr", index: "perfexpr", width: 110, label: "业绩公式", sortable: true, align: "left"},
				{name: "perfexprremarks", index: "perfexprremarks", width: 170, label: "业绩公式说明", sortable: true, align: "left"},
				{name: "chgperfexpr", index: "chgperfexpr", width: 110, label: "增减业绩公式", sortable: true, align: "left"},
				{name: "chgperfexprremarks", index: "chgperfexprremarks", width: 170, label: "增减业绩公式说明", sortable: true, align: "left"},
	...
	function d_add() {
		Global.Dialog.showDialog("goSave",{
					title:"分段业绩公式明细--增加",
					url:"${ctx}/admin/custTypePerfExpr/goSave?custType=${custType.code}",
				    height:350,
				    width:700,
					returnFun:function(){
						queryTable();
	...
	function d_update() {
		...
		Global.Dialog.showDialog("goUpdate",{
			title:"分段业绩公式明细--编辑",
			url:"${ctx}/admin/custTypePerfExpr/goUpdate?id="+ret.pk+"&custType=${custType.code}",
		    height:350,
		    width:700,
			returnFun:function(){
				queryTable();
	...
	function d_delete(){
		...
		art.dialog({
			...
			ok: function () {
				$.ajax({
				url : "${ctx}/admin/custTypePerfExpr/doDelete?id="+ret.pk+"&custType=${custType.code}",
				type : "post",
				dataType : "json",
				cache : false,
				error : function(obj) {
	...
	function d_detail() {
		...
		Global.Dialog.showDialog("goDetail",{
			title:"分段业绩公式明细--查看",
			url:"${ctx}/admin/custTypePerfExpr/goDetail?id="+ret.pk+"&custType=${custType.code}",
		    height:350,
		    width:700
		});
	}
	...
	function doExcel(){
		var url = "${ctx}/admin/custTypePerfExpr/doExcel?custType=${custType.code}";
		doExcelAll(url);
	}
</script>
	...
	<div class="panel panel-system">
		<div class="panel-body">
			...
			<div class="btn-group-xs">
				...
				<button id="deleteBut" type="button" class="btn btn-system"
					onclick="d_delete()">删除</button>
