...
function update(){
    ...
    var chgRemarks='';
	if($("#department1").val()!="${employee.department1}"){
	    chgRemarks ="部门 ： [${employee.department1} ${employee.department1Descr}]-> ["+$("#department1").find("option:selected").text()+"]\n";
   		$("#chgRemarks").val(chgRemarks);
	}
	if($("#department2").val()!="${employee.department2}"&&$("#department2").val()!=0){
		var chgRemarks ="部门 ： [${employee.department1} ${employee.department1Descr}][${employee.department2} ${employee.department2Descr}]-> ["
		+$("#department1").find("option:selected").text()+"]"+"["+$("#department2").find("option:selected").text()+"]\n";                      
		$("#chgRemarks").val(chgRemarks);
	};
    if($("#personChgDate").val()!="<fmt:formatDate value='${employee.personChgDate}' pattern='yyyy-MM-dd'/>"){
    	if(chgRemarks!=null){
    		chgRemarks=chgRemarks+"人事调岗时间 ：  [<fmt:formatDate value='${employee.personChgDate}' pattern='yyyy-MM-dd'/>]-> ["+$("#personChgDate").val()+"]\n";
        }else{
        	chgRemarks="人事调岗时间 ：  [<fmt:formatDate value='${employee.personChgDate}' pattern='yyyy-MM-dd'/>]-> ["+$("#personChgDate").val()+"]\n";
        }
        $("#chgRemarks").val(chgRemarks);
    }
    if($.trim($("#idnum").val())==""){
	...
	formData.append("oldDepartment",$("#oldDepartment").val());
    ...
    formData.append("isLead",$("#isLead").val());
    formData.append("leadLevel",$("#leadLevel").val());
    ...
    formData.append("chgRemarks",chgRemarks);
    ...
    formData.append("imgFlag",$("#imgFlag").val());
    if($("#nameChi").val()==$("#nameChiS").val()){
			...
			if (obj.rs) {
				art.dialog({
					...
					beforeunload : function() {
						if(expired=="F" && status=="SUS"){
							var depts=getDeptByLeader();
							if(depts!=""){
								art.dialog({
										content : "该员工为以下部门领导："+depts+"。请联系IT部修改部门领导。",
										okValue: '确定',
									    ok: function () {
									       closeWin();
									    },
...
$(function(){
    var chgRemarks =null;
	if($("#department1").val()!="${employee.department1}"){
	    chgRemarks ="部门 ：&nbsp[employee.department1 &nbsp employee.department1descr ]->["+$("#department1").find("option:selected").text()+"]";
   		$("#chgRemarks").val(chgRemarks);
	}
	
	if($("#department2").attr("value")!="${employee.department2}"){
		var chgRemarks =chgRemarks+"${employee.department2}"; 
		$("#chgRemarks").val(chgRemarks);
	};
...
//查询领导的部门
function getDeptByLeader(){
	var dept="";
	$.ajax({
		url:"${ctx}/admin/employee/getDeptByLeader",
		type:'post',
	...
	<div class="panel panel-system">
	    <div class="panel-body">
	    	<div class="btn-group-xs">
				<button type="button" class="btn btn-system" onclick="updateNew()">保存</button>
				...
				<ul class="ul-form">
					<div class="validate-group">
						<li>
							...
							<input type="text" id="number" name="number" value="${employee.number}" readonly="readonly"/>
						</li>
						<li>
						    ...
						    <div style=" width: 300px; float: left">
								<ul class="ul-form">
									<div class="validate-group row">
									    ...
									    <input type="text" name="photoName" id="photoName" value="${employee.photoName}" hidden="true"/>	
									    <input type="hidden" id="positionType" name="positionType" value="${employee.positionType}"/>
										<input type="hidden" id="secondPositionType" name="secondPositionType" value="${employee.secondPositionType}"/>
										<input type="hidden" id="oldDepartment" name="oldDepartment" value="${employee.department}"/>     
										<li class="form-validate">
											<label>应聘序号</label>
											<input type="text" id="applicantPK" name="applicantPK" value="${employee.applicantPK}"/>
										</li>
										<li class="form-validate">	
											<label>IC卡号</label>
							...
							<div style="width: 750px; float: left;">
								<ul class="ul-form">
									...
									<div class="validate-group row">
										...
										<li>
											<label>状态</label>	
											<house:xtdm id="status" dictCode="EMPSTS" value="${employee.status}" onchange="changeStatus()"></house:xtdm>
										</li>
										<li>
						 	...
						 	<div style="width: 750px;  float: left; position: relative;">
						 		...
						 		<div style="width:620px;float:right;">
						 			<textarea id="remarks" name="remarks" style="width:450px;height:50px;">${employee.remarks}</textarea>
						 		</div>		   								
						 	</div>
						 </div>	
					 </form>
							...
							<form action="" method="post" id="page_form_2" class="form-search">
								<ul class="ul-form" style="height: 450px;">
									...
									<li>
										...
										<textarea id="chgRemarks" name="chgRemarks" rows="16" style="width: 470px">${employee.chgRemarks}</textarea>
									</li>
								</ul>
							</form>
...
function doUpdateNew(data) {
    $.ajax({
        url: "${ctx}/admin/employee/doUpdateNew",
        type: "POST",
        data: data,
        dataType: "json",
        error: function(obj) {
