...
$(function() {
	...
	var gridOption = {
		...
		colModel : [
			...
			{name:'delivcode', index:'delivcode', width:80, label:'批次', sortable:true ,align:"left"},
	...
	window.goto_query = function(){
		$("#dataTable").jqGrid("setGridParam",{datatype:'json',postData:{spcBuilder:'***'},page:1,url:"${ctx}/admin/builder/goSpcBuilderJqGrid",}).trigger("reloadGrid");
	}
	...
	$("#saveBtn").on("click",function(){
		...
		if(!$("#dataForm").data('bootstrapValidator').isValid()) return;
			...
			Global.Form.submit("dataForm","${ctx}/admin/spcBuilder/doSave",param,function(ret){
				if(ret.rs==true){
					art.dialog({
						content:ret.msg,
						time:1000,
		...
		<div class="panel panel-info" >  
				...
				<form action="" method="post" id="dataForm" class="form-search">
					<input type="hidden" name="jsonString" value="" />
						<ul class="ul-form">
							<li >
								<label>专盘编号</label>
								<input type="text" id="code" name="code" style="width:160px;" value="${spcBuilder.code }" placeholder="保存自动生成" readonly="readonly"/>
							</li>
							...
							<li class="form-validate">
								<label>楼盘类型</label>
								<house:xtdm id="type" dictCode="SPCBUILDERTYPE" ></house:xtdm>                     
							</li>
							<li >
								<label>项目类型</label>
								<house:xtdm id="builderType" dictCode="BUILDERTYPE" ></house:xtdm>                     
							</li>
							<li class="form-validate">
								<label>交房时间</label>
								<input type="text" id="delivDate" name="delivDate" class="i-date" style="width:160px;" onFocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd'})" value=""/>
							</li>
							<li >
								<label>总户数</label>
			...
			<div class="btn-panel" >
    			<div class="btn-group-sm"  >
					...
					<button type="button" class="btn btn-system " id="deliv" >
						<span>新增交房批次</span>
					</button>
