...
$(function() {
	...
	Global.LinkSelect.setSelect({firstSelect:'itemType1',
								...
								secondValue:'${itemTypeConfirm.itemType2}',
								thirdSelect:'itemType3',
								thirdValue:'${itemTypeConfirm.itemType3}'
								});	
	...
	$("#saveBtn").on("click",function(){
		...
		var arrString=$.trim($("#arr").val());
	 	...
	 	var itemarr2=$("#arr").val().split(',');
	 	var itemarr3=$("#arry").val().split(',');
	 	for(var i=0;i<itemarr2.length;i++ ){
	 		if(arrString!=''&&$.trim(itemarr2[i])==itemtype2&&$.trim(itemarr3[i])==itemtype3){
	 			art.dialog({
	 			content:'明细表存在相同数据'
	 		});
	 		return false;
		...
		datas.item2Descr=item2Descr;
		datas.item3Descr=item3Descr;
