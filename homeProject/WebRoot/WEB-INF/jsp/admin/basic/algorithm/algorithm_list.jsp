...
<body>
		...
		<div class="query-form">
			<form role="form" class="form-search" id="page_form" action="" method="post" target="targetFrame">
				...
				<ul class="ul-form">
					<li>
						<label>编码</label> 
						<input type="text" id="code"  name="code" style="width:160px;" value="${algorithm.no}" />
					</li>
					<li>
						<label>名称</label> 
						<input type="text" id="descr" name="descr" style="width:160px;" value="${algorithm.descr}"/>
					</li>
					<li>
						<input type="checkbox" id="expired" name="expired" value="${algorithm.expired}" 
							onclick="hideExpired(this)" ${algorithm.expired!='T' ?'checked':'' }/>
	...
	<div class="btn-panel">
		<div class="btn-group-sm">
			...
			<house:authorize authCode="ALGORITHM_VIEW">
...
$(function(){
	Global.JqGrid.initJqGrid("dataTable",{
		url:"${ctx}/admin/algorithm/goJqGrid",
		...
		colModel : [
			...
			{name: "iscalcutfee", index: "iscalcutfee", width: 160, label: "是否计算切割费", sortable: true, align: "left",hidden:true},
			{name: "iscalcutfeedescr", index: "iscalcutfeedescr", width: 105, label: "是否计算切割费", sortable: true, align: "left",hidden:true},
	...
	$("#add").on("click",function(){
		Global.Dialog.showDialog("goSave",{
			title:"快速预报价算法管理--增加",
			url:"${ctx}/admin/algorithm/goSave",
		    height:611,
		    width:950,
	...
	$("#update").on("click",function(){
		...
		Global.Dialog.showDialog("goUpdate",{
			title:"快速预报价算法管理--编辑",
			url:"${ctx}/admin/algorithm/goUpdate",
...
function view(){
	...
	Global.Dialog.showDialog("goDetail",{
		title:"快速预报价算法管理--查看",
		url:"${ctx}/admin/algorithm/goUpdate",
		postData:{id:ret.code,m_umState:"V"},
	    height:611,
	    width:950,
		returnFun:goto_query
	});
...
function doExcel(){
	var url = "${ctx}/admin/algorithm/doExcel";
	doExcelAll(url);
}
</script>
</html>
