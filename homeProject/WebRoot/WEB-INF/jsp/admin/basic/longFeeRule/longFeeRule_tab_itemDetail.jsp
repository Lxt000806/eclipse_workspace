...
<script type="text/javascript">
    function goItem(m_umState, name) {
        var ret = selectDataTableRow("itemDataTable");
        var no = $("#no").val();
        var itemType1 = $("#itemType1").val();
        var itemType2 = $("#itemType2").val();
        var itemType3s = $("#itemDataTable").getCol("itemtype3", false).join(",");
        if (!ret && m_umState !== "A") {
        ...
        Global.Dialog.showDialog("goItem", {
            title: name + "材料页面",
            url: "${ctx}/admin/longFeeRule/goAddItemDetail",
            postData: {
            ...
            returnFun: function(data) {
                var id = $("#itemDataTable").jqGrid("getGridParam", "selrow");
                if (data) {
                    $.each(data, function(k, v) {
                        ...
                        if (m_umState === "M") {
                            Global.JqGrid.updRowData("itemDataTable", id, json);
                        } else if (m_umState === "A") {
                            Global.JqGrid.addRowData("itemDataTable", json);
                        }
    ...
    function delItem() {
        ...
        art.dialog({
            ...
            lock: true,
            ok: function() {
                Global.JqGrid.delRowData("itemDataTable", id);
                var records = $("#itemDataTable").jqGrid('getGridParam', 'records');
    ...
    $(function() {
        ...
        Global.JqGrid.initJqGrid("itemDataTable", {
            url: "${ctx}/admin/longFeeRule/goItemDetailJqGrid?no=" + "${longFeeRule.no}",
            ...
            gridComplete: function() {
                var records = $("#itemDataTable").jqGrid('getGridParam', 'records'); //获取数据总条数
...
<div class="body-box-form" style="margin-top: 0;">
