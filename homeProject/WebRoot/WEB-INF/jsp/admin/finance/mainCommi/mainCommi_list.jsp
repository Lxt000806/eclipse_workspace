...
<head>
	...
	function updatePeriod(){
		...
		Global.Dialog.showDialog("update",{
			title:"编辑周期统计",
			postData:{no:ret.no},
			url:"${ctx}/admin/mainCommi/goUpdate",
			height: 300,
		 	width:650,
			returnFun: goto_query 
		});
	...
	function count(){
		...
		$.ajax({
			url:"${ctx}/admin/mainCommi/isExistsPeriod",
			type: "post",
		    ...
		    success: function(obj){
				if(obj!=null && obj!=""){
					...
					Global.Dialog.showDialog("count",{
						...
						url:"${ctx}/admin/mainCommi/goCount",
						height: 750,
		 				width:1350,
						returnFun: goto_query 
					});
	...
	function view(){
		...
		Global.Dialog.showDialog("view",{
			title:"提成计算——查看",
			postData:{no:ret.no},
			url:"${ctx}/admin/mainCommi/goView",
			height: 750,
		 	width:1350,
			returnFun: goto_query 
		});
	...
	function countComplete(){
		...
		$.ajax({
			url:"${ctx}/admin/mainCommi/checkStatus",
			type:"post",
			data:{no:ret.no},
			dataType:"json",
			cache:false,
				...
				if(ret.status!=obj){
					...
					return;
					 ...
					 art.dialog({
						...
						ok: function () {
							$.ajax({
								url:"${ctx}/admin/mainCommi/doCountComplete",
								type:"post",
								data:{no:ret.no},
								dataType:"json",
								cache:false,
			...
			success:function(obj){
				if(ret.status!=obj){
					...
					art.dialog({
						...
						ok: function () {
							$.ajax({
								url:"${ctx}/admin/mainCommi/doCountBack",
								type:"post",
								data:{no:ret.no},
								dataType:"json",
								cache:false,
	...
	$(function(){
		...
		Global.JqGrid.initJqGrid("dataTable",{
			url:"${ctx}/admin/mainCommi/goJqGrid",
			...
			colModel : [
				...
				{name: "prjperfno", index: "prjperfno", width: 130, label: "结算业绩周期编号", sortable: true, align: "left"},
	...
	function doExcel(){
		var url = "${ctx}/admin/mainCommi/doExcel";
		doExcelAll(url);
	}
	function report(){
		Global.Dialog.showDialog("goReport",{			
			  title:"提成计算——报表",
			  url:"${ctx}/admin/mainCommi/goReport",
			  height: 600,
			  width:800
		});
	}
	function commiPerc(){
       Global.Dialog.showDialog("CommiPerc",{
		  title:"提成计算——设置提成比例",
		  url:"${ctx}/admin/mainCommi/goCommiPerc",
		  height:720,
		  width:870,
		});
	}
		...
		<div class="pageContent">
			<div class="btn-panel">
				<div class="btn-group-sm">
					<house:authorize authCode="MAINCOMMI_COUNT">
						<button type="button" class="btn btn-system" onclick="count()">
							<span>提成计算</span>
						</button>
					</house:authorize>
					<house:authorize authCode="MAINCOMMI_COUNTCOMPLETE">
						<button type="button" class="btn btn-system"
							onclick="countComplete()">
							<span>计算完成</span>
						</button>
					</house:authorize>
					<house:authorize authCode="MAINCOMMI_RETURN">
						<button type="button" class="btn btn-system" onclick="back()">
							<span>退回</span>
						</button>
					</house:authorize>
					<house:authorize authCode="MAINCOMMI_COMMIPERC">
						<button type="button" class="btn btn-system" onclick="commiPerc()">
							<span>设置提成比例</span>
						</button>
					</house:authorize>
					<house:authorize authCode="MAINCOMMI_VIEW">
						<button type="button" class="btn btn-system" onclick="view()">
							<span>查看</span>
						</button>
					</house:authorize>
					<house:authorize authCode="MAINCOMMI_REPORT">
						<button type="button" class="btn btn-system" onclick="report()">
							<span>报表</span>
						</button>
					</house:authorize>
					<house:authorize authCode="MAINCOMMI_ADD">
						<button type="button" class="btn btn-system" onclick="add()">
							<span>新增统计周期</span>
						</button>
					</house:authorize>
					<house:authorize authCode="MAINCOMMI_UPDATE">
						<button type="button" class="btn btn-system"
							onclick="updatePeriod()">
							<span>编辑</span>
						</button>
					</house:authorize>
					<house:authorize authCode="MAINCOMMI_EXCEL">
