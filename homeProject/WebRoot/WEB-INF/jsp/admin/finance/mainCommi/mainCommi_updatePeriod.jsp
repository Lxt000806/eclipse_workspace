...
<head>
	...
	$(function(){
		...
		$("#saveBtn").on("click",function(){
			...
			$.ajax({
				url:"${ctx}/admin/mainCommi/doUpdatePeriod",
				type: "post",
				data: datas,
				dataType: "json",
				cache: false,
					...
					<ul class="ul-form">
						<div class="validate-group row">
							<li class="form-validate"><label>编号</label> <input
								...
								style="width:160px;" value="${mainCommi.no }" />
							</li>
							<li><label>状态</label> <house:xtdm id="status"
									dictCode="SOFTPERFSTATUS" value="${mainCommi.status }"
						...
						<div class="validate-group row">
							<li class="form-validate"><label>开始时间</label> <input
								...
								value="<fmt:formatDate value='${mainCommi.beginDate}' pattern='yyyy-MM-dd'/>" />
							</li>
							<li class="form-validate"><label>结束时间</label> <input
								...
								style="width:160px;"
								onFocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd'})"
								value="<fmt:formatDate value='${mainCommi.endDate}' pattern='yyyy-MM-dd'/>" />
							</li>
						</div>
						<div class="validate-group row">
							<li><label>结算业绩周期编号</label> <input type="text" id="prjPerfNo" name="prjPerfNo"
								style="width:160px;" value="${mainCommi.prjPerfNo}"/>
							</li>
						</div>
						<div class="validate-group row">
							<li><label class="control-textarea">说明</label> <textarea
									id="remarks" name="remarks" rows="2">${mainCommi.remarks }</textarea>
							</li>
						</div>
					</ul>
				</form>
