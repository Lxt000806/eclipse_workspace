...
$("#checkPassBtn").on("click",function(){
	...
	if(documentNo==''){
		art.dialog({content: "填写凭证号",width: 200});
		return false;
	}
	...
	art.dialog({
		 content:'是否确定审核通过['+rows+']条领料发货单,['+rows2+']条销售单？',
		 lock: true,
		 width: 200,
		 height: 100,
		 ok: function () {
			...
			Global.Form.submit("dataForm","${ctx}/admin/whCheckOut/doCheckPass",param,function(ret){
				if(ret.rs==true){
					art.dialog({
						content: ret.msg,
						time: 1000,
...
$("#checkCancelBtn").on("click",function(){	
	...
	art.dialog({
		 content:'是否确定审核取消['+rows+']条领料发货单,['+rows2+']条销售单？',
		 lock: true,
		 width: 200,
		 height: 100,
		 ok: function () {
		    ...
		    var salesInvoiceDetail = $('#dataTable_salesInvoice').jqGrid("getRowData");
			var param = {"itemAppsendDetailJson": JSON.stringify(itemAppsendDetail),"salesInvoiceDetailJson":JSON.stringify(salesInvoiceDetail)};
			Global.Form.submit("dataForm","${ctx}/admin/whCheckOut/doCheckCancel",param,function(ret){
				if(ret.rs==true){
					art.dialog({
						content: ret.msg,
						time: 1000,
