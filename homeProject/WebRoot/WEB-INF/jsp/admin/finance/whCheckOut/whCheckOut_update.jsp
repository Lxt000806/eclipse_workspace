...
<script type="text/javascript">
	...
	$(function() {
		...
		if("<%=czylb %>"=="1"){
       	 	$("#whCode").openComponent_wareHouse({condition:{ctrlItemType1:"1"},showValue:"${whCheckOut.whCode}",showLabel:"${whCheckOut.whDescr}",readonly:true,callBack:fillData});
      	}else{
            $("#whCode").openComponent_wareHouse({showValue:"${whCheckOut.whCode}",showLabel:"${whCheckOut.whDescr}",condition:{czybh:"<%=czybh %>"},readonly:true,callBack:fillData});
            $("#documentNo").hide();
...
/* 		var itemAppsendDetailJson = Global.JqGrid.allToJson("dataTable_itemAppSend");
		...
		Global.Form.submit("dataForm","${ctx}/admin/whCheckOut/doUpdate",param,function(ret){
			if(ret.rs==true){
				art.dialog({
					content: ret.msg,
					/* time: 1000, */
			  ...
			  returnFun:function(data){
				  if(data){
					  ...
					  $.each(data,function(k,v){
					  	  ...
					  	  console.log(v.projectothercost);
					  	  if (v.whfeecosttype=='2'){//公司和项目经理成本
	...
	$("#delItemAppSend").on("click",function(){
		...
		art.dialog({
			 ...
			 height: 100,
			 ok: function () {
			 		Global.JqGrid.delRowData("dataTable_itemAppSend",rowId);
				    //仓库可以修改
