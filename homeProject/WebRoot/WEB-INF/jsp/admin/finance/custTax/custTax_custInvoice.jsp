			...
			<div class="pageContent">
				...
				<!-- <div class="btn-panel">
					<div class="btn-group-sm" style="padding-top: 5px;">
						...
						<button type="button" class="btn btn-system" id="deleteRow">
							<span>删除选中</span>
						</button>
						<button type="button" class="btn btn-system" id="outModelData" 
							onclick="doExcelNow('开票明细导入模板','modelDataTable')">
...
$(function(){
	...
	Global.JqGrid.initJqGrid("dataTable",{
		...
		colModel : [
			...
			{name: "date", index: "date", width: 80, label: "开票日期", align: "left", formatter: formatDate},
			{name: "invoiceno", index: "invoiceno", width: 80, label: "发票号", align: "left"},
			{name: "invoicecode", index: "invoicecode", width: 80, label: "发票代码", align: "left"},
			{name: "taxservice", index: "taxservice", width: 100, label: "应税服务名称", align: "left"},
			{name: "buyer", index: "buyer", width: 60, label: "购买方", align: "left"},
			{name: "amount", index: "amount", width: 80, label: "开票金额", align: "right"},
			{name: "taxper", index: "taxper", width: 60, label: "税率", align: "right"},
			{name: "notaxamount", index: "notaxamount", width: 80, label: "不含税金额", align: "right"},
			{name: "taxamount", index: "taxamount", width: 60, label: "税额", align: "right"},
			{name: "remarks", index: "remarks", width: 266, label: "备注", align: "left"},
        ]
	});
	//初始化excel模板表格
	Global.JqGrid.initJqGrid("modelDataTable",{
		colModel : [
			...
			{name: "invoiceno", index: "invoiceno", width: 80, label: "发票号", align: "left"},
			{name: "invoicecode", index: "invoicecode", width: 80, label: "发票代码", align: "left"},
			{name: "taxservice", index: "taxservice", width: 80, label: "应税服务名称", align: "left"},
			{name: "buyer", index: "buyer", width: 80, label: "购买方", align: "left"},
			{name: "amount", index: "amount", width: 80, label: "开票金额", align: "left"},
			{name: "taxper", index: "taxper", width: 80, label: "税率", align: "left"},
			{name: "notaxamount", index: "notaxamount", width: 80, label: "不含税金额", align: "left"},
			{name: "taxamount", index: "taxamount", width: 80, label: "税额", align: "left"},
			{name: "remarks", index: "remarks", width: 266, label: "备注", align: "left"},
        ]
	});
...
function loadData(){
	if(check()){
		...
		$.ajax({
			url: "${ctx}/admin/custTax/loadExcel",
			type: "POST",
			data: formData,
			contentType: false,
			processData: false,
...
function importData(){
	...
	Global.Form.submit("page_form", "${ctx}/admin/custTax/doCustInvoice", param, function(ret){
		if(ret.rs==true){
			art.dialog({
				content: ret.msg,
				time: 700,
