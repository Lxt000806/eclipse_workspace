...
$(function(){
	...
	var gridOption = {
		...
		{name: "provideremark", index: "provideremark", width: 120, label: "发放备注", sortable: true, align: "left",editable:true},
...
$(function(){
	...
	$("#checkPassBtn").on("click",function(){
		...
		art.dialog({
			...
			ok:function(){
				...
				Global.Form.submit("dataForm","${ctx}/admin/prjProvide/doCheckPass",param,function(ret){
					if(ret.rs==true){
						art.dialog({
							content: ret.msg,
							/* time: 1000, */
	...
	/* 审核取消 */
	$("#checkCancelBtn").on("click",function(){
		var row = $("#dataTable").getGridParam("records");
		if((row==0)){
			art.dialog({content: "明细表中无数据！",width: 200});
			return false;
		...
		art.dialog({
			...
			ok:function(){
				var prjproCheckDetail=$('#dataTable').jqGrid("getRowData");
				var param = {"prjproCheckDetailJson": JSON.stringify(prjproCheckDetail)};
				Global.Form.submit("dataForm","${ctx}/admin/prjProvide/doCheckCancel",param,function(ret){
					if(ret.rs==true){
						art.dialog({
							content: ret.msg,
							/* time: 1000, */
	...
	<div class="panel panel-system" >
	    <div class="panel-body" >
	    	<div class="btn-group-xs" >
				<button type="button" class="btn btn-system "  id="checkPassBtn">审核通过</button>
				<button type="button" class="btn btn-system "  id="checkCancelBtn">审核取消</button>
			...
			<form action="" method="post" id="dataForm" class="form-search">
				...
				<ul class="ul-form">	
					<div class="validate-group">		  
						...
						<li>	
							<label>凭证号</label>
							<input type="text" id="documentNo" name="documentNo" value="${prjProvide.documentNo}"/>
						</li>
						<li>
