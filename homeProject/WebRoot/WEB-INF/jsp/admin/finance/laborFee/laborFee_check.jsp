...
<title>人工费用管理-审核</title>
	...
	$(function(){
		...
		$("#confirmCZY").openComponent_employee({showValue:"${laborFee.confirmCZY}",showLabel:"${confirmDescr}",readonly:true });
	...
	$(function(){
		...
		var gridOption = {
			...
			colModel : [
				...
				{name: "amount2", index: "amount2", width:85, label: "配送费(自动生成)", sortable: true, align: "right",},
	...
	function check(){
		var url="${ctx}/admin/laborFee/doCheck";
		save(url);
		
	}
	function checkCancel(){
		var url="${ctx}/admin/laborFee/doCheckCancel";
		save(url);
	}
	function checkBack(){
		var url="${ctx}/admin/laborFee/doCheckBack";
		save(url);
	}
	function save(url){
		...
		var detailParam= Global.JqGrid.allToJson("dataTable_account",null,null,true);
        param["accountDetailJson"] = detailParam["detailJson"];
		Global.Form.submit("dataForm",url,param,function(ret){
			if(ret.rs==true){
				art.dialog({
					content:ret.msg,
					time:1000,
		...
		<div class="content-form">
   				...
   				<div class="panel-body">
      				<div class="btn-group-xs">
						<button type="button" class="btn btn-system" style="color:#777777; cursor:default">
 						...
 						<button type="button" class="btn btn-system" id="check" onclick="check()">
 							<span>审核通过</span>
 						</button>
 						<button type="button" class="btn btn-system" id="checkCancel" onclick="checkCancel()">
 							<span>审核取消</span>
 						</button>
 						<button type="button" class="btn btn-system" id="checkBack" onclick="checkBack()">
 							<span>审核退回</span>
 						</button>
	         		...
	         		<div class="panel-body">
	         			<form role="form" class="form-search" id="dataForm" action="" method="post" target="targetFrame">
							...
							<ul class="ul-form">
									...
									<li class="form-validate">
										<label>卡号</label>
										<input type="text" id="cardID" name="cardID" style="width:160px;" value="${laborFee.cardID }" readonly="true"/>
									</li>
									<li>
										<label>开户行</label>
									...
									<li>
										<label>审核人</label>
										<input type="text" id="confirmCZY" name="confirmCZY" style="width:160px;" value="${laborFee.confirmCZY}" />
									</li>
									<li>
										<label><span class="required">*</span>凭证号</label>
										<input type="text" id="documentNo" name="documentNo" style="width:160px;" value="${laborFee.documentNo}" />
									</li>
									<li>
			...
			<div class="btn-panel">
	    		<div class="btn-group-sm">
					<button type="button" class="btn btn-system" id="add" disabled>
						<span>新增</span>
					</button>
					<button type="button" class="btn btn-system" id="update" onclick="updateAccount()" disabled>
						<span>编辑</span>
					</button>
					<button type="button" class="btn btn-system" id="delete" disabled>
						<span>删除</span>
					</button>
