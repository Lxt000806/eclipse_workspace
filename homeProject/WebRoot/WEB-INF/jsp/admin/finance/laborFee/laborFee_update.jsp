...
<title>人工费用管理-编辑</title>
	...
	function save(){
		...
		// param 会多传一倍的数据量
		var map = {};
		map["detailJson"] = param["detailJson"];
		map["accountDetailJson"] = detailParam["detailJson"];
		
		Global.Form.submit("dataForm","${ctx}/admin/laborFee/doUpdate",map,function(ret){
			if(ret.rs==true){
				art.dialog({
					content:ret.msg,
					time:1000,
	...
	function sendFeeImport(){
		...
		if(itemType1!="ZC" && itemType1!="JC"){
			art.dialog({
				content:"只有主材、集成才能进行配送费导入操作！"
			});		
			return;
		}
	...
	function installFeeImport(){
		...
		var custCodes="";
		for(var i=0;i<jq.length;i++){
			if((jq[i].feetype=="15" || jq[i].feetype=="29")){
		...
		<div class="content-form">
			...
			<div class="btn-panel">
	    		<div class="btn-group-sm" id="mainBtn">
					...
					<button type="button" class="funBtn funBtn-system" onclick="excelImport()">
						<span>从Excel导入</span>
					</button>
					<button type="button" class="funBtn funBtn-system" onclick="doExcel()" title="导出检索条件数据">
						<span>输出到excel</span>
					</button>
					...
					<button type="button" class="funBtn funBtn-system" id="sendFeeImport" onclick="sendFeeImport()">
						<span>配送费导入</span>
					</button>
					<button type="button" class="funBtn funBtn-system" id="tileCutFeeImport" onclick="tileCutFeeImport()">
						<span>导入瓷砖加工费</span>
					</button>
					<button type="button" class="funBtn funBtn-system" id="bathFeeImport" onclick="bathFeeImport()">
						<span>导入浴室柜安装费</span>
					</button>
					<button type="button" class="funBtn funBtn-system" id="whInstallFeeImport" onclick="whInstallFeeImport()">
						<span>导入仓库装卸费</span>
					</button>
					<button type="button" class="funBtn funBtn-system" id="transFeeImport" onclick="transFeeImport()">
						<span>导入窗帘灯具搬运费</span>
					</button>
					<button type="button" class="funBtn funBtn-system" id="installFeeImport" onclick="installFeeImport()">
						<span>导入窗帘灯具安装费</span>
					</button>
					<button type="button" class="funBtn funBtn-system" id="intFeeImport" onclick="intFeeImport()">
						<span>导入集成安装费</span>
					</button>
				</div>
