...
$(function(){
	if($.trim("${supplierPrepay.wfProcInstNo}") == ""){
		$("#purchaseAdvance").hide();
	}
}); 
function doPrint(){
	var reportName = $('input:radio:checked').val();
	var no = "${supplierPrepay.no}";

	if(reportName == "purchaseAdvance.jasper"){
		no = "${supplierPrepay.wfProcInstNo}";
		...
		function(){
	   	...
	   	if ("${supplierPrepay.type}"=="2"){
			var reportName = "yfjgl_dj.jasper";
		}else if ("${supplierPrepay.type}" =="1"){
			var reportName = "yfjgl_yf.jasper";
		} 
	   	Global.Print.showPrint(reportName, {
			No: no,
			LogoFile : "<%=basePath%>jasperlogo/logo.jpg",
	         		...
	         		<div class="panel-body">
	         			<form role="form" class="form-search" id="dataForm" action="" method="post" target="targetFrame">
							...
							<ul class="ul-form">
								...
								<label style="padding-left:20px" id="purchaseAdvance"><input type="radio" name="rowNums" style="" value="purchaseAdvance.jasper"/>预付单</label>
							</ul>	
						</form>
					</div>
				</div>
