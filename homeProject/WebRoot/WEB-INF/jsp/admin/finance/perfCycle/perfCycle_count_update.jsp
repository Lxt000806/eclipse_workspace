...
<html>
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			}else if("${perfCycle.m_umState}"=="R"){
				//查看原业绩时窗口右移,与原页面区分开
				/* var openCount="${perfCycle.openCount}"==""?0:parseFloat("${perfCycle.openCount}")-1;//打开次数,如果没有初值，则设置为0
				var dialogId="ViewOldPerf"+openCount;//动态dialogId，可无限次打开
				parent.$("div[aria-describedby=dialog_"+dialogId+"]").css("left","100px");
				parent.$("div[aria-describedby=dialog_"+dialogId+"]").css("top","25px"); */
			...
			$("#isModified,#dataType,#type,#isAddAllInfo").attr("disabled",true);
			...
			getInfoByCustCode("${perfCycle.custCode}");
  			//实时计算费用
		...
		function changeIsCheck(){
			...
			$.ajax({
				url:"${ctx}/admin/perfCycle/changeIsCheck",
				type:'post',
				data:{isChecked:isChecked,pk:"${perfCycle.pk}"},
				dataType:'json',
				cache:false,
				async:false
			});
		...
		function importOldPerf(){
			...
			Global.Dialog.showDialog("goRegPerformance",{
				...
				returnFun:function(){
					$.ajax({
						url:"${ctx}/admin/perfCycle/getRegImport?custCode="+custCode,
						type:'post',
						data:{},
						dataType:'json',
						cache:false,
		...
		function calcBaseItemChg(){
	 		...
	 		$("#setAdd").val(basePlan);  // 增减套外基础增项同基础预算 by zjf20201221
		}
		//计算材料增减金额
		...
		function viewOldPerf(){
			...
			var openCount="${perfCycle.openCount}"==""?0:"${perfCycle.openCount}";//打开次数,如果没有初值，则设置为0
			var dialogId="ViewOldPerf"+openCount;//动态dialogId，可无限次打开
			var newOpenCount=parseFloat(openCount)+1;
			if(regPerfPk==""){
				art.dialog({
					content: "没有原业绩！"
				});
			...
			Global.Dialog.showDialog(dialogId,{
				title:"业绩明细--查看原业绩",
				url:"${ctx}/admin/perfCycle/goViewOldPerf?regPerfPk="+$("#regPerfPk").val()+"&openCount="+newOpenCount+"&m_umState=R",
				height:750,
				width:1450
			});
		}
		...
		function accAdd(arg1,arg2){  
			var r1,r2,m;  
			try{r1=arg1.toString().split(".")[1].length;}catch(e){r1=0;}//第一个加数的小数的位数
			try{r2=arg2.toString().split(".")[1].length;}catch(e){r2=0;}//第二个加数的小数的位数
			m=Math.pow(10,Math.max(r1,r2)); //n=取大的那个位数，m取10的n次方 
			return (arg1*m+arg2*m)/m; //两个加数分别乘以m,再除以m，相当于转化成整数相加，加完再转化为小数
		}
		//查看赠送项目明细
		function viewZsxm(){
			/*Global.Dialog.showDialog("goViewGift",{
				title:"业绩明细--查看赠送项目明细",
				url:"${ctx}/admin/perfCycle/goViewGift",
				postData:{custCode:$("#custCode").val()},
				height:650,
				width:1200
			});*/
			Global.Dialog.showDialog("countDisc",{
				title:"业绩明细--查看赠送项目明细",
				url:"${ctx}/admin/customerSghtxx/goCountDisc",
				postData:{id:$("#custCode").val(),m_umState:"V"},
				height:720,
				width:1170,
		...
		*/
		...
		function getBasePersonalPlan(){
			...
			$.ajax({
				...
				success: function(obj){
					if(obj && obj.length>0){
						$("#basePersonalPlan").val(obj[0].basePersonalPlan).trigger("changeFee");
...
<body>
		...
		<div class="panel panel-system">
			<div class="panel-body">
				<div class="btn-group-xs" style="float:left">
					<button type="button" class="btn btn-system viewFlag" id="saveBtn" onclick="save()">保存</button>
					<button type="button" class="btn btn-system" id="calcType">人工修改</button>
					<button type="button" class="btn btn-system viewFlag" id="importOldPerf"
						onclick="importOldPerf()" disabled="true">导入原业绩</button>
					...
					id="isChecked" name="isChecked" onclick="changeIsCheck()" ${perfCycle.isChecked!='0' ?'checked':''}
					 />
					<p style=" position:absolute;left:515px;top:13px;">复核</p>
				</div>
			</div>
		</div>
		<div class="panel panel-info"
			...
			<form action="" method="post" id="page_form" class="form-search">
				<input style="width:150px" type="hidden" name="jsonString" value="" /><input style="width:150px"
					type="hidden" name="isExitTip" id="isExitTip"  /> <input style="width:150px"
					...
					<input style="width:150px" type="hidden" name="regPerfPk" id="regPerfPk" value="${perfCycle.regPerfPk}"/>
					<input style="width:150px" type="hidden" name="pk" id="pk" value="${perfCycle.pk}"/>
					<input style="width:150px" type="hidden" name="payType" id="payType" value="${perfCycle.payType}"/>
					<input style="width:150px" type="hidden" name="regAchieveDate" id="regAchieveDate" value="${perfCycle.regAchieveDate}"/>
					<input style="width:150px" type="hidden" name="maxMaterPerfPer" id="maxMaterPerfPer" value="${perfCycle.maxMaterPerfPer}"/>
					<input style="width:150px" type="hidden" name="basePerfPer" id="basePerfPer" value="${perfCycle.basePerfPer}"/>
					<input style="width:150px" type="hidden" name="isCalcBaseDisc" id="isCalcBaseDisc" value="${perfCycle.isCalcBaseDisc}"/>
					<input style="width:150px" type="hidden" name="perfExpr" id="perfExpr" value="${perfCycle.perfExpr}"/>
					<input style="width:150px" type="hidden" name="regRealMaterPerf" id="regRealMaterPerf" value="${perfCycle.regRealMaterPerf}"/>
					<input style="width:150px" type="hidden" name="sumChgRealMaterPerf" id="sumChgRealMaterPerf" value="${perfCycle.sumChgRealMaterPerf}"/>
					<input style="width:150px" type="hidden" name="perfAmount" id="perfAmount"  value="${perfCycle.perfAmount}"/>
					<input style="width:150px" type="hidden" name="tileDeduction" id="tileDeduction"  value="${perfCycle.tileDeduction}"/>
					<input style="width:150px" type="hidden" name="bathDeduction" id="bathDeduction"  value="${perfCycle.bathDeduction}"/>
					<input style="width:150px" type="hidden" name="mainDeduction" id="mainDeduction"  value="${perfCycle.mainDeduction}"/>
					<input style="width:150px" type="hidden" name="realPerfAmount" id="realPerfAmount" value="${perfCycle.realPerfAmount}"/>
					<input style="width:150px" type="hidden" name="tileStatus" id="tileStatus" value="1" value="${perfCycle.tileStatus}"/>
					<input style="width:150px" type="hidden" name="bathStatus" id="bathStatus" value="1" value="${perfCycle.bathStatus}"/>
					<input style="width:150px" type="hidden" name="marketFund" id="marketFund" value="${perfCycle.marketFund}"/>
					<input style="width:150px" type="hidden" name="basePerfRadix" id="basePerfRadix" value="${perfCycle.basePerfRadix}"/>
					<input style="width:150px" type="hidden" name="baseFee_Dirct" id="baseFee_Dirct" value="${perfCycle.baseFee_Dirct}"/>
					<input style="width:150px" type="hidden" name="baseFee_Comp" id="baseFee_Comp" value="${perfCycle.baseFee_Comp}"/>
					...
					<div id="mainInfo" class="tab-pane fade in active"
						...
						<div class="row">
							<li><label>业绩类型</label> <house:xtdm id="type"
									dictCode="PERFTYPE"  onchange="changePerfType()" value="${perfCycle.type}"></house:xtdm>
							</li>
							<li><label>常规变更</label> <house:xtdm id="isAddAllInfo"
									dictCode="YESNO" value="${perfCycle.isAddAllInfo}"></house:xtdm>
							</li>
							<li><label>客户编号</label> <input style="width:150px" type="text" id="custCode"
								name="custCode" value="${perfCycle.custCode}" />
							</li>
							<li><label>档案号</label> <input style="width:150px" type="text" id="documentNo"
								name="documentNo" value="${perfCycle.documentNo}" readonly/>
							</li>
							<li><label>楼盘</label> <input type="text" id="address"
								name="address"  style="width:160px;" value="${perfCycle.address}" readonly/>
							</li>
						</div>
						<span
						...
						<div class="panel panel-info"
							...
							<div class="row" style="padding:2px">
								<li style="padding:0px"><label>基础预算</label> <input style="width:150px" style="width:150px"
									type="number" id="basePlan" name="basePlan" class="init calcfee" value="${perfCycle.basePlan}" />
								</li>
								<li><label>远程费</label> <input style="width:150px" style="width:150px" type="number" id="longFee"
									name="longFee" class="init calcfee" value="${perfCycle.longFee}" >
								</li>
							</div>
							<div class="row" style="padding:2px">
								<li style="padding:0px"><label>主材预算</label> <input style="width:150px" style="width:150px"
									type="number" id="mainPlan" name="mainPlan" class="init calcfee" value="${perfCycle.mainPlan}" />
								</li>
								<li><label>服务性预算</label> <input style="width:150px" style="width:150px" type="number"
									id="mainServPlan" name="mainServPlan" class="init calcfee" value="${perfCycle.mainServPlan}" />
								</li>
							</div>
							<div class="row" style="padding:2px">
								<li style="padding:0px"><label>软装预算</label> <input style="width:150px" style="width:150px"
									type="number" id="softPlan" name="softPlan" class="init calcfee" value="${perfCycle.softPlan}" />
								</li>
								<li><label>软装家具费</label> <input style="width:150px" type="number"
									id="softFee_Furniture" name="softFee_Furniture" class="init calcfee" value="${perfCycle.softFee_Furniture}" />
								</li>
							</div>
							<div class="row" style="padding:2px">
								<li style="padding:0px"><label>集成预算</label> <input style="width:150px"
									type="number" id="integratePlan" name="integratePlan" class="init calcfee" value="${perfCycle.integratePlan}" /></li>
								<li><label>橱柜预算</label> <input style="width:150px" type="number"
									id="cupboardPlan" name="cupboardPlan" class="init calcfee" value="${perfCycle.cupboardPlan}" />
								</li>
							</div>
							<div class="row" style="padding:2px">
								<li style="padding:0px"><label>套餐内减项</label> <input style="width:150px"
									type="number" id="setMinus" name="setMinus" class="init" value="${perfCycle.setMinus}" />
								</li>
								<li><label>协议优惠额</label> <input style="width:150px" type="number" id="baseDisc"
									name="baseDisc" class="init calcfee" value="${perfCycle.baseDisc}" />
								</li>
							</div>
							<div class="row" style="padding:2px">
								<li style="padding:0px"><label>套外基础增项</label> <input style="width:150px"
									type="number" id="setAdd" name="setAdd" class="init" value="${perfCycle.setAdd}"/>
								</li>
						...
						<div class="panel panel-info"
							...
							<div class="row" style="padding:2px">
								<li style="padding:0px"><label>管理费合计 </label> <input style="width:150px"
									type="number" id="manageFee_Sum" name="manageFee_Sum"  class="calcfee" value="${perfCycle.manageFee_Sum}" readonly/></li>
								<li><label>基础管理费</label> <input style="width:150px" type="number"
									id="manageFee_Base" name="manageFee_Base" class="init calcfee" value="${perfCycle.manageFee_Base}" />
								</li>
							</div>
							<div class="row" style="padding:2px">
								<li style="padding:0px"><label>主材管理费</label> <input style="width:150px"
									type="number" id="manageFee_Main" name="manageFee_Main" class="init calcfee" value="${perfCycle.manageFee_Main}" /></li>
								<li><label>服务性管理费</label> <input style="width:150px" type="number"
									id="manageFee_Serv" name="manageFee_Serv" class="init calcfee" value="${perfCycle.manageFee_Serv}" />
								</li>
							</div>
							<div class="row" style="padding:2px">
								<li style="padding:0px"><label>集成管理费</label> <input style="width:150px"
									type="number" id="manageFee_Int" name="manageFee_Int" class="init calcfee" value="${perfCycle.manageFee_Int}" /></li>
								<li><label>橱柜管理费</label> <input style="width:150px" type="number"
									id="manageFee_Cup" name="manageFee_Cup" class="init calcfee" value="${perfCycle.manageFee_Cup}" />
								</li>
							</div>
							<div class="row" style="padding:2px">
								<li style="padding:0px"><label>软装管理费</label> <input style="width:150px"
									type="number" id="manageFee_Soft" name="manageFee_Soft" class="init calcfee" value="${perfCycle.manageFee_Soft}" /></li>
								<li><label>税金</label> <input style="width:150px"
									type="number" id="tax" name="tax" value="${perfCycle.tax}" class="init calcfee"/></li>
							</div>
							<div class="row" style="padding:2px">
								<li style="padding:0px"><label>套餐内管理费</label> <input style="width:150px"
									type="number" id="manageFee_InSet" name="manageFee_InSet" class="init calcfee" value="${perfCycle.manageFee_InSet}" /></li>
								<li><label>套餐外管理费</label> <input style="width:150px" type="number"
									id="manageFee_OutSet" name="manageFee_OutSet" class="calcfee" value="${perfCycle.manageFee_OutSet}" readonly/>
								</li>
							</div>
							<div class="row" style="padding:2px">
						...
						<div class="panel panel-info"
							...
							<div class="row" style="padding:12px 2px 2px 2px">
								<li style="padding:0px"><label>实际材料业绩</label> <input style="width:150px"
									type="number" id="realMaterPerf" name="realMaterPerf" value="${perfCycle.realMaterPerf}" readonly /></li>
							</div>
							<div class="row" style="padding:2px">
								<li style="padding:0px"><label>已算材料业绩</label> <input style="width:150px"
									type="number" id="alreadyMaterPerf" name="alreadyMaterPerf" value="${perfCycle.alreadyMaterPerf}" readonly /></li>
							</div>
							<div class="row" style="padding:2px">
								<li style="padding:0px"><label>封顶材料业绩</label> <input style="width:150px"
									type="number" id="maxMaterPerf" name="maxMaterPerf" value="${perfCycle.maxMaterPerf}" readonly /></li>
							</div>
							<div class="row" style="padding:2px">
								<li style="padding:0px"><label>材料业绩</label> <input style="width:150px"
									type="number" id="materPerf" name="materPerf" value="${perfCycle.materPerf}" readonly />
								</li>
							</div>
						</div>
						<span
							style="position:absolute;left:20px;top:300px;z-index:1;background-color:white"><b>业绩增减项目</b></span>
						<div class="panel panel-info"
							style="width:555px;height:95px;position:absolute;left:1px;top:305px">
							<div class="row" style="padding:2px">
								<li style="padding:0px"><label>实物赠送</label> <input style="width:150px"
									type="number" id="gift" name="gift" class="init calcfee" value="${perfCycle.gift}" />
								</li>
								<li><label>软装券金额</label> <input style="width:150px" type="number"
									id="softTokenAmount" name="softTokenAmount" class="init calcfee" value="${perfCycle.softTokenAmount}" />
								</li>
							</div>
							<div class="row" style="padding:2px">
								<li style="padding:0px"><label>基础单项扣减</label> <input style="width:150px"
									type="number" id="baseDeduction" name="baseDeduction" class="init calcfee" value="${perfCycle.baseDeduction}" /></li>
								<li><label>材料单品扣减</label> <input style="width:150px" type="number"
									id="itemDeduction" name="itemDeduction" class="init calcfee" value="${perfCycle.itemDeduction}" />
								</li>
							</div>
						</div>
						<span
						...
						<div class="panel panel-info"
							...
							<div class="row" style="padding:0px">
								<li style="padding:0px"><label>合同总价</label> <input style="width:150px"
									type="number" id="contractFee" name="contractFee"  class="calcfee" value="${perfCycle.contractFee}" readonly/>
								</li>
								<li><label>实收设计费</label> <input style="width:150px" type="number" id="designFee"
									name="designFee" class="init calcfee" value="${perfCycle.designFee}"  />
								</li>
							</div>
							<div class="row" style="padding:0px">
								...
								<li><label>总价+设计费</label>
									...
									name="contractAndDesignFee" value="${perfCycle.contractAndDesignFee}"  readonly/>
								</li>
							</div>
							<div class="row" style="padding:0px">
								<li style="padding:0px"><label>实际业绩</label> <input style="width:150px"
									type="number" id="recalPerf" name="recalPerf" value="${perfCycle.recalPerf}" class="init" />
								</li>
								<li><label>计算PK业绩</label> <house:xtdm id="isCalPkPerf"
										dictCode="YESNO" value="${perfCycle.isCalPkPerf}" ></house:xtdm>
								</li>
							</div>
							<div class="row" style="padding:0px">
								<li style="padding:0px"><label>业绩折扣</label> <input style="width:150px"
									type="number" id="perfMarkup" name="perfMarkup" value="${perfCycle.perfMarkup}" class="calcfee"/>
								</li>
								<li><label>折扣前业绩</label> <input style="width:150px"
									type="number" id="befMarkupPerf" name="befMarkupPerf" value="${perfCycle.befMarkupPerf}" readonly/>
								</li>
							</div>
						</div>
						<span
						...
						<div class="panel panel-info"
							style="width:275px;height:108px;position:absolute;left:1130px;top:230px">
							<div class="row" style="padding:2px">
								<li style="padding:0px"><label>业绩比例</label> <input style="width:150px"
									type="number" id="perfPerc" name="perfPerc" class="init"  value="${perfCycle.perfPerc}"/>
								</li>
							</div>
							<div class="row" style="padding:2px">
								<li style="padding:0px"><label>业绩折扣金额</label> <input style="width:150px"
									type="number" id="perfDisc" name="perfDisc" class="init calcfee" value="${perfCycle.perfDisc}" />
								</li>
							</div>
						</div>
					</div>
							...
							<div class="row" style="padding:2px">
								<div class="col-xs-1">
									<li><label style="width:50px">首付款</label> <input style="width:150px"
										type="number" id="firstPay" name="firstPay" class="init" value="${perfCycle.firstPay}" />
									</li>
									<li><label style="width:50px">达标应收</label> <input style="width:150px"
										type="number" id="mustReceive" name="mustReceive" class="init" value="${perfCycle.mustReceive}" /></li>
									<li><label style="width:50px">实收</label> <input style="width:150px"
										type="number" id="realReceive" name="realReceive" class="init" value="${perfCycle.realReceive}" /></li>
									<li><label style="width:50px">数据类型</label> <house:xtdm
											id="dataType" dictCode="PERFDATATYPE"  value="${perfCycle.dataType}"></house:xtdm>
									</li>
								</div>
								<div class="col-xs-1">
									<li><label style="width:50px">签订时间</label> <input style="width:150px"
										...
										style="width:160px;"
										value="<fmt:formatDate value='${perfCycle.signDate }' pattern='yyyy-MM-dd'/>"
										onFocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd'})" />
									</li>
									<li><label style="width:50px">下定时间</label> <input style="width:150px"
										type="text" id="setDate" name="setDate" class="i-date"
										style="width:160px;"
										value="<fmt:formatDate value='${perfCycle.setDate }' pattern='yyyy-MM-dd'/>"
										onFocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd'})" />
									</li>
									...
									<li><label style="width:50px">人工修改</label> <house:xtdm
											id="isModified" dictCode="YESNO" onchange="changeIsModified()" value="${perfCycle.isModified}"></house:xtdm>
									</li>
								</div>
								<div class="col-xs-1">
									<li style="padding:0px"><label style="width:50px">优惠折扣</label>
										<input style="width:150px" type="number" id="markup" name="markup" class="init"  value="${perfCycle.markup}"/></li>
									<li style="padding:0px"><label class="control-textarea"
										style="width:50px">优惠政策</label> <textarea id="discRemark"
											...
											style="overflow-y:scroll; overflow-x:hidden; height:92px;width: 170px;"
											 />${perfCycle.discRemark}</textarea></li>
									
								</div>
								<div class="col-xs-1">
									<li style="padding:0px"><label style="width:50px">赠送项目</label>
										<button type="button" style="font-size: 12px;line-height: 1.5;padding: 1px 5px;" 
											class="btn btn-system "  onclick="viewZsxm()">查看明细</button>
									</li>
									<li style="padding:0px"><label class="control-textarea"
								...
								<div class="col-xs-1">
									<li><label style="width:60px">单量</label> <input style="width:150px"
										type="number" id="quantity" name="quantity" class="init" value="${perfCycle.quantity}" />
									</li>
									<li><label style="width:60px">面积</label> <input style="width:150px"
										type="number" id="area" name="area" class="init" value="${perfCycle.area}" />
									</li>
									<li><label style="width:60px">主材毛利率</label> <input style="width:150px"
										type="number" id="mainProper" name="mainProper" class="init" value="${perfCycle.mainProPer}" />
									</li>
									<li>
										...
										<house:dict id="payeeCode" dictCode="" style="width:150px"
											sql=" select code val,code+' '+Descr descr from tTaxPayee 
												where Expired='F' "  width="150px" disabled="true"
											sqlValueKey="val" sqlLableKey="descr" value="${perfCycle.payeeCode}"/>
									</li>
								</div>
							</div>
							<div class="row" style="padding:2px">
								<li style="padding:0px"><label class="control-textarea"
									style="width:50px">业绩公式</label> <textarea id="perfExprRemarks"
										...
										/>${perfCycle.perfExprRemarks}</textarea></li>
							</div>
						</div>
					</div>
				</ul>
	...
	<div class="container-fluid" style="padding:402px 10px 0px 0px">
