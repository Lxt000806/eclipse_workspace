...
//保存
function save(){
	if ($.trim($("#commiType").val())=="" && $.trim($("#commiPerc").val())==""){
		art.dialog({content: "没有设置批量修改值",width: 200});
		return false;
  	} 
  	if ($.trim($("#commiType").val())!="0" && $.trim($("#commiPerc").val())==""){
		art.dialog({content: "提成比例未填写",width: 200});
		return false;
 	 } 
	 var sCode="${commi.reqPks}";
	 var sCommiType=$.trim($("#commiType").val());
	 var sCommiPerc=$.trim($("#commiPerc").val());
	 $.ajax({
		url:"${ctx}/admin/commi/doUpdateBatch",
		type:'post',
		data:{reqPks:sCode,commiType:sCommiType,commiPerc:sCommiPerc},
		cache:false,
		error:function(obj){
			...
			<div class="panel-body">
				<form role="form" class="form-search" id="dataForm" action=""
					...
					<house:token></house:token>
								<input hidden="true" id="reqPks" name="reqPks" value="${commi.reqPks}" />
					<ul class="ul-form">
