...
$(function(){
	Global.JqGrid.initJqGrid("dataTable_reorderbuilds",{
		url:"${ctx}/admin/PrjDeptPerf/goReOrderBuildsJqGrid",
		postData:{empCode:$("#empCode").val(),dateFrom:$("#dateFrom").val(),dateTo:$("#dateTo").val(),custType:$("#custType").val(),department2:$("#department2").val()},
        autowidth: false,
        rowNum:10000000,
        height:400,
		width:800, 
		...
		colModel : [
			...
			{name: "reorderfee", index: "reorderfee", width: 110, label: "签单业绩金额", sortable: true, align: "right",sum:true,formatter:function(cellvalue, options, rowObject){return myRound(cellvalue);}},
			{name: "contractamount", index: "contractamount", width: 90, label: "到账合同额", sortable: true, align: "right", sum: true},
        ]
	});
});
</script>
<table id="dataTable_reorderbuilds"></table>
