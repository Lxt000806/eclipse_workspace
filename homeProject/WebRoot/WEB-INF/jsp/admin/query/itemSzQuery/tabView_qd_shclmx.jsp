...
$(function(){
	Global.JqGrid.initJqGrid("dataTable_shclmx",{
        autowidth: false,
        height:220,
		width:1259, 
		styleUI: 'Bootstrap',
        ...
        gridComplete:function(){	
        	var id = $("#dataTable_shclmx").jqGrid('getGridParam', 'selrow');
		    if (id) {
		      var ret = $("#dataTable_shclmx").jqGrid("getRowData",id);
		      $("#dataTable_llmx_shclmx").jqGrid("setGridParam",{url:"${ctx}/admin/itemSzQuery/goLlmxJqGrid",postData:{no:ret.no.trim(),custCode:$("#code").val()},page:1}).trigger("reloadGrid");
		    }
        },
        beforeSelectRow:function(rowid, e){
        	var ret = $("#dataTable_shclmx").jqGrid("getRowData",rowid);
        	$("#dataTable_llmx_shclmx").jqGrid("setGridParam",{url:"${ctx}/admin/itemSzQuery/goLlmxJqGrid",postData:{no:ret.no.trim(),custCode:$("#code").val()},page:1}).trigger("reloadGrid");
        }
	});
});
...
<table id="dataTable_shclmx"></table>

<jsp:include page="tabView_llmx.jsp">
	<jsp:param value="shclmx" name="fromClcbmx"/>
</jsp:include>
