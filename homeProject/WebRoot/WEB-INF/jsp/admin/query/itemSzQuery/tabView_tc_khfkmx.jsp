...
$(function(){
	Global.JqGrid.initJqGrid("dataTable_khfkmx_tc",{
		...
		colModel : [
			...
			{name: "lastupdate", index: "lastupdate", width: 163, label: "最后修改时间", sortable: true, align: "left", formatter: formatTime},
	...
	$.ajax({
		...
		success: function(data){
			var inputs = $("#khfkmxUl_tc").children().children("input");
			for(var i=0;i<inputs.length;i++){
				if(data[inputs[i].id.substring(0,inputs[i].id.lastIndexOf("_"))] != undefined){
					if(inputs[i].id == "SignDate_tc"){
			...
			//var contractFee = isNaN( parseFloat($("#ContractFee_tc").val()) ) ? 0.0:parseFloat($("#ContractFee_tc").val());			
			...
			var designFee = isNaN( parseFloat($("#DesignFee_tc").val()) ) ? 0.0:parseFloat($("#DesignFee_tc").val());
			var zjzje = isNaN( parseFloat($("#zjzje_tc").val()) ) ? 0.0:parseFloat($("#zjzje_tc").val());
			var haspay = isNaN( parseFloat($("#Haspay_tc").val()) ) ? 0.0:parseFloat($("#Haspay_tc").val());
			var isRcvDesignFee = isNaN(parseFloat(data.isRcvDesignFee)) ? 0.0:parseFloat(data.isRcvDesignFee);
			
			$("#Wdz_tc").val(parseFloat(firstPay + secondPay + thirdPay + fourPay + designFee*isRcvDesignFee + zjzje - haspay).toFixed(1));
		}
	});	
});
</script>
...
<div class="panel-info">  
	<div class="panel-body">
		<form action="" method="post" id="dataForm"  class="form-search">
			...
			<ul id="khfkmxUl_tc" class="ul-form">
			<c:if test="${isWorkcost=='1'}">
					...
					<li><label>签订时间</label> <input type="text" id="SignDate_tc"
						name="SignDate_tc" />
					</li>
					...
					<li><label>工程造价</label> <input type="text"
						id="ContractFee_tc" name="ContractFee_tc" />
					</li>
					<li><label>设计费</label> <input type="text" id="DesignFee_tc"
						name="DesignFee_tc" />
					</li>
					...
					<li><label>增减总金额</label> <input type="text" id="zjzje_tc"
						name="zjzje_tc" />
					</li>
					<li><label>已付款</label> <input type="text" id="Haspay_tc"
						name="Haspay_tc" />
					</li>
					<li><label>未达账</label> <input type="text" id="Wdz_tc"
						name="Wdz_tc" />
					</li>
				</c:if>
				<c:if test="${isWorkcost!='1'}">
					...
					<li hidden><label>设计费</label> <input type="text"
						id="DesignFee_tc" name="DesignFee_tc" />
					</li>
					<li hidden><label>增减总金额</label> <input type="text"
						id="zjzje_tc" name="zjzje_tc" />
					</li>
					<li><label>已付款</label> <input type="text" id="Haspay_tc"
						name="Haspay_tc" />
					</li>
...
<table id="dataTable_khfkmx_tc"></table>
