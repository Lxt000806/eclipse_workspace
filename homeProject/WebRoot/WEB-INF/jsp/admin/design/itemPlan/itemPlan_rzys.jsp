...
function add(isInsert){
	 ...
	 if(ret) itemType2Str+="&itemType2="+ret.itemtype2;
		 var url="${ctx}/admin/itemPlan/goYsQuickSave?custCode=${customer.code}&itemType1=RZ&isOutSet=${customer.isOutSet}&custType=${customer.custType}&isService=0"+itemType2Str;
		 if(ret) url="${ctx}/admin/itemPlan/goYsQuickSave?custCode=${customer.code}&itemType1=RZ&isOutSet=${customer.isOutSet}&custType=${customer.custType}&isService=0&fixAreaPk="+ret.fixareapk+itemType2Str;
...
function view(){
    ...
    if (rowId) {
	    Global.Dialog.showDialog("ys_detailView",{
			  title:"软装预算--查看",
			  url:"${ctx}/admin/itemPlan/goItemPlan_ys_detailView?itemType1=RZ&custCode=${customer.code}&isOutSet=${customer.isOutSet}&rowId="+rowId+"&isService=0",
			  height:600,
			  width:1000
		});
    } else {
    ...
    if (rowId) {
	    ...
	    if(itemSetNo){
	     ...
	     Global.Dialog.showDialog("ys_detailViewEdit",{
			  title:"软装预算--编辑",
			  url:"${ctx}/admin/itemPlan/goItemPlan_ys_detailViewEdit?itemType1=RZ&custCode=${customer.code}&isOutSet=${customer.isOutSet}&rowId="+rowId+"&isService=0&itemCode="+itemCode+"&pk="+pk,
			  height:600,
			  width:1000,
			  resizable:false, 
			  returnFun: function(data){
...
function clearDetail(sContent,stitle,sMustImportTemp){
	if (!sContent){
		sContent= '是否清除【${customer.code}-${customer.address}】的软装预算<br/><input type="checkbox" id="clearDetail" />同时清空装修区域<br/>注：勾选此项将直接删除,无须进行保存操作即生效！ ';
	}
	art.dialog({
		 content:sContent,
		 lock: true,
...
$(function(){
	var  isCellEdit=true;
	if ('${customer.status}'=='4'||'${customer.status}'=='5' || "${contractStatus}" == "2" 
			|| "${contractStatus}" == "3" || "${contractStatus}" == "4" || "${wfProcStatus}" == "1"){
	...
	if('${customer.isPartDecorate}'=='3'){
		$("#discTokenNo_li").show();
		document.getElementById("amount_lb").style.marginRight = "-5px";
	...
	Global.JqGrid.initJqGrid("dataTable",{
		url:"${ctx}/admin/itemPlan/goItemPlan_RZJqGrid?custCode=${customer.code}&itemType1=RZ",
		...
		{name: "itemdescr", index: "itemdescr", width: 220, label: "材料名称", sortable: true, align: "left",cellattr: addCellAttr,editable:true,edittype:'text',
				editoptions:{ dataEvents:[{type:'focus',fn:function(e){ $(e.target).openComponent_itemDescr({condition:{'itemType1':"RZ".trim(),'containCode':"1",custType:"${customer.custType}"},onSelect:onSelect});}}]}},
        ...
        {name: "isclearinv", index: "isclearinv", width: 90, label: "清库存材料", sortable:false, align: "right",hidden: true},
    ], 
    ...
    afterSaveCell:function(id,name,val,iRow,iCol){
		...
		Global.JqGrid.initJqGrid("referenceDetailDataTable",{
			url:"${ctx}/admin/itemPlan/goItemPlan_RZYJJqGrid?custCode=${customer.code}",
...
function save(){
	...
	var rowData=$('#dataTable').jqGrid("getRowData");
	var isAddAllInfo = $.trim("${customer.isAddAllInfo}");
	var planSendDate = $("#planSendDate").val();
	if(isAddAllInfo == "0" && planSendDate == ""){
		art.dialog({
			content:"软装独立销售预算,预计发货日期不能为空"
		});
		return ;
	}
	var x=0;
...
/* function goImportExcel(){
   Global.Dialog.showDialog("itemPlan_importExcel",{
		  title:"软装预算明细--从excel导入",
		  url:"${ctx}/admin/itemPlan/goItemPlan_importExcel?itemType1=RZ&custCode=${customer.code}&custType=${customer.custType}&isService=0&isCupboard=0",
		  height:600,
		  width:1000,
		  returnFun: function(data){
		  		$.each(data,function(k,v){
...
function goImportExcel(){
	...
	var sContent='导入前,需要清除【${customer.code}-${customer.address}】的软装预算<br/><input type="checkbox" id="clearDetail" />同时清空装修区域<br/>注：勾选此项将直接删除,无须进行保存操作即生效！ ';
	var rowData=$('#dataTable').jqGrid("getRowData");
	art.dialog({
...
function goImportTab(stitle,sMustImportTemp ){
	Global.Dialog.showDialog("itemPlan_importExcel",{
	  	title:stitle,
	  	url:"${ctx}/admin/itemPlan/goItemPlan_importExcel?itemType1=RZ&custCode=${customer.code}&custType=${customer.custType}&isService=0&isCupboard=0"+"&mustImportTemp="+sMustImportTemp,
	  	height:600,
	  	width:1000,
	    returnFun: function(data){
		  		$.each(data,function(k,v){
...
function copy(){
	Global.Dialog.showDialog("itemPlanCopy",{
		  title:"搜寻--客户编号",
		  url:"${ctx}/admin/itemPlan/goCopy?itemType1=RZ&custCode=${customer.code}&isService=0&&custType=${customer.custType}",
		  height: 600,
		  width:1000,
		  returnFun: function(data){
	  		 $.each(data,function(k,v){
...
function discTokenSel(){
	...
	$("#tokenNoSelect").openComponent_discToken({
		condition:{'itemType1':'RZ',custCode:$("#code").val(),containOldCustCode:'1',status:'2', hasSelectNo:"${customer.discTokenNo}"},
			...
			<div class="panel-body">
				...
				<%-- 	<house:token></house:token> --%>
					<input type="hidden" id="expired" name="expired" value="${itemPlan.expired}" /> <input type="hidden"
						...
						name="custCode" value="${customer.code }" /> <input type="hidden" name="itemType1" value="RZ" /> <input
					...
					<ul class="ul-form">
						...
						<li><label id="amount_lb" >实际总价</label> <input type="text" id="amount" name="amount" readonly="readonly" />
						</li>
