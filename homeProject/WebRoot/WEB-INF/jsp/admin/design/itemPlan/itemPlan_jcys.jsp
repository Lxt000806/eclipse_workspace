...
function loadFee(rowData,baseFeeComp){
	if(!baseFeeComp)
	var baseFeeComp=getSumByFixarea(rowData,"综合项目");//综合费
	var amount=parseFloat($("#dataTable").getCol('lineamount',false,'sum'));
	var baseFeeDirct=amount-baseFeeComp;//直接费
	...
	//Round((基础直接费*系数+基础综合费*系数+面积*系数)*基础管理费系数,0)   //需要减去不计算管理费的项目总金
	var manageFeeBase=myRound((baseFeeDirct*${customer.baseFeeDirctPer}+baseFeeComp*${customer.baseFeeCompPer}+${customer.area}*${customer.areaPer}-dNoManageFee)*${customer.manageFeeBasePer});//基础管理费
	
	var manageFee=myRound(manageFeeBase+${customer.manageFeeMain}*${customer.containMain}+${customer.manageFeeInt}*${customer.containInt}
	+${customer.manageFeeServ}*${customer.containMainServ}+${customer.manageFeeSoft}*${customer.containSoft}+${customer.manageFeeCup}*${customer.containCup});
	
	var baseFee=amount+manageFee;//基础总金额
	$("#baseFeeComp").val(baseFeeComp);
	$("#baseFeeDirct").val(baseFeeDirct);
	$("#manageFeeBase").val(manageFeeBase);//基础管理费
...
function add(isInsert){
	...
	var posi = $('#dataTable').jqGrid('getGridParam', 'selrow')-1;
 	...
 	var url="${ctx}/admin/itemPlan/goJzysQuickSave?custCode=${customer.code}&itemType1=JZ&isOutSet=1&custTypeType=1&m_umState=A&custType=${customer.custType}&canUseComBaseItem=${custType.canUseComBaseItem}"+baseItemType1Str;
 	if(ret) url="${ctx}/admin/itemPlan/goJzysQuickSave?custCode=${customer.code}&itemType1=JZ&isOutSet=1&custTypeType=1&m_umState=A&custType=${customer.custType}&canUseComBaseItem=${custType.canUseComBaseItem}&fixAreaPk="+ret.fixareapk+baseItemType1Str;
 	...
 	Global.Dialog.showDialog("quickSave",{
		  ...
		  returnFun: function(data){
			  ...
			  $.each(data,function(k,v){
				 ...
				 v.iscalmangefee=v.iscalmangefee;
...
function view(){
    ...
    if (rowId) {
	    Global.Dialog.showDialog("jcys_detailView",{
			  title:"基础预算--查看",
			  url:"${ctx}/admin/itemPlan/goItemPlan_jcys_detailView?itemType1=JZ&custCode=${customer.code}&rowId="+rowId,
			  height:600,
			  width:1000
		});
    }else{
...
function edit(){
    ...
    if (rowId) {
	    Global.Dialog.showDialog("jcys_detailViewEdit",{
		   title:"基础预算--编辑",
		   url:"${ctx}/admin/itemPlan/goItemPlan_jcys_detailViewEdit?itemType1=JZ&custTypeType=1&custCode=${customer.code}&m_umState=A&custType=${customer.custType}&canUseComBaseItem=${custType.canUseComBaseItem}&rowId="+rowId,
		   height:600,
		   width:1000,
		   resizable:false, 
		   returnFun: function(data){
...
function updatePrice(){
	Global.Dialog.showDialog("itemChgUpdatePrice",{
		  title:"调整单价",
		  url:"${ctx}/admin/itemPlan/goUpdatePrice?m_umState=A",
		  height: 300,
		  width:400,
		  resizable:false, 
		  returnFun: function(data){
...
$(function(){
	...
	Global.JqGrid.initJqGrid("dataTable",{
		...
		colModel : [
			...
			{name: "baseitemdescr", index: "baseitemdescr", width: 240, label: "基础项目名称", sortable: true, align: "left"},
			...
			{name: "unitprice", index: "unitprice", width: 70, label: "人工单价", sortable: true, align: "right",editable:true,edittype:'text',editrules:{number:true,required:true}},
			{name: "material", index: "material", width: 70, label: "材料单价", sortable: true, align: "right",editable:true,edittype:'text',editrules:{number:true,required:true}},
			{name: "tempunitprice", index: "unitprice", width:70, label: "临时人工单价", sortable: true, align: "left", hidden: true},
			{name: "tempmaterial", index: "material", width: 70, label: "临时材料单价", sortable: true, align: "left", hidden: true},
			...
			{name: "autoqty", index: "autoqty", width: 75, label: "系统算量", sortable:false, align: "right"},
			...
			{name: "giftpk", index: "giftpk", width: 0.5, label: "赠送项目编号", sortable: true, align: "left"},
       		...
       		{name: "isoutsetdescr", index: "isoutsetdescr", width: 0.01, label: "套餐外材料", sortable: true, align: "left"},
			...
			{name: "isfixprice", index: "isfixprice", width: 20, label: "是否固定价格", sortable: true, align: "left", hidden: true},
       	    ...
       	    {name: "isoutset", index: "isoutset", width: 68, label: "是否套餐外", sortable: true, align: "left", hidden: true},
		...
		gridComplete:function(){
			...
			var rowNum=parseInt($("#dataTable").getGridParam("records"),10);
	        if(rowNum>0){   
	   			$(".ui-jqgrid-sdiv").show();
	        	var sum_summaterial = $("#dataTable").getCol("summaterial",false,"sum"); 
	        	var sum_sumunitprice = $("#dataTable").getCol("sumunitprice",false,"sum");
	            var sum_lineamount = $("#dataTable").getCol("lineamount",false,"sum");
			    var footerData = {};
			    footerData["summaterial"] =sum_summaterial;
				footerData["sumunitprice"] =sum_sumunitprice;
				footerData["lineamount"] =sum_lineamount;
        ...
        afterSaveCell:function(id,name,val,iRow,iCol){
            ...
            var summary=getGridFootSum(id,'lineamount');
           	var baseFeeComp=myRound($("#baseFeeComp").val());
            ...
            if(rowData.fixareadescr=="综合项目"){
            	baseFeeComp+=-myRound(rowData.lineamount)+myRound(val*rowData.unitprice)+myRound(val*rowData.material);
            	$("#baseFeeComp").val(baseFeeComp);
            }
            var lineamount=myRound($("#dataTable").getCol('lineamount',false,'sum'));
...
function save(){
	...
	if(rowDatas){
		...
		$.each(rowDatas,function(k,v){
			if(parseFloat(v.qty)<0.00){
				sum=0.00;
				$.each(rowDatas,function(k1,v1){
					if(v1.fixareadescr==v.fixareadescr&&v1.baseitemcode==v.baseitemcode){
						sum+=parseFloat(v1.qty);
					}
				}); 
				if (sum<0.00){
					result+='【'+v.fixareadescr+v.baseitemdescr+'】'+',';
				}
			};
			//判断总价是否相等
...
//基础预算从模板导入
 function goBaseBatchTemp(){
	 Global.Dialog.showDialog("baseitemPlan_baseBatchTemp",{
		title:"基础预算--从模板导入",
		url:"${ctx}/admin/itemPlan/goBaseFromTemp?custType=${customer.custType}&custCode=${customer.code}&m_umState=A",
		height:250,
		width:430,
		returnFun: function(data){
		   	if(data.length){
	...
	<div class="body-box-form">
			...
			<div class="panel-body">
				<form action="" method="post" id="dataForm" class="form-search">
					...
					<ul class="ul-form">
						...
						<li><label>直接费</label> <input type="text" id="baseFeeDirct" name="baseFeeDirct" value="0"
							readonly="readonly" />
						</li>
						<li><label>综合费用</label> <input type="text" id="baseFeeComp" name="baseFeeComp" value="0"
							readonly="readonly" />
						</li>
						...
						<li><label>管理费</label> <input type="text" id="manageFee" name="manageFee" value="0"
							readonly="readonly" />
						</li>
						<li><label>不计管理费金额</label> <input type="text" id="noManageFee" name="noManageFee"
							readonly="readonly" /></li>
						<li><label>基础总金额</label> <input type="text" id="baseFee" name="baseFee" value="0"
							readonly="readonly" />
						</li>
		...
		<div class="btn-panel">
			<div class="btn-group-sm">
			    ...
			    <button type="button" id="quickLoad" class="btn btn-system " onclick="goBaseBatchTemp()" >从模板导入</button>
				<button id="importBtn" type="button" class="btn btn-system " onclick="goImportExcel()">从备份导入</button>
