...
function clickTree(treeNode){
	if(treeNode.parentIdStr){
		window.document.getElementById("listFrame_q").src="${ctx}/admin/baseItem/goItemSelect?isOutSet=${itemPlan.isOutSet}&&custType=${itemPlan.custType}&&canUseComBaseItem=${itemPlan.canUseComBaseItem}&&custCode=${itemPlan.custCode}&&baseItemType1="+treeNode.parentIdStr+"&baseItemType2="+treeNode.menuIdStr;
	}else{
		window.document.getElementById("listFrame_q").src="${ctx}/admin/baseItem/goItemSelect?isOutSet=${itemPlan.isOutSet}&&custType=${itemPlan.custType}&&canUseComBaseItem=${itemPlan.canUseComBaseItem}&&custCode=${itemPlan.custCode}&&baseItemType1="+treeNode.menuIdStr;
	}
}
</script>
  <frameset cols="200,*" rows="*"  border="0" framespacing="1" frameborder="yes" bordercolor="#B2D9FF" style="border-style: hidden;"  >
  		<frame id="treeFrame_q" name="treeFrame_q" src="${ctx}/admin/baseItemType1/goTree?baseItemType1=${itemPlan.baseItemType1}" frameborder="0" border="1" noresize="noresize" >
  		<frame id="listFrame_q" name="listFrame_q" src="${ctx}/admin/baseItem/goItemSelect?isOutSet=${itemPlan.isOutSet}&&custType=${itemPlan.custType}&&canUseComBaseItem=${itemPlan.canUseComBaseItem}&&custCode=${itemPlan.custCode}" frameborder="0" border="1" noresize="noresize" >
  </frameset>
  </head>
<body>
</body>
