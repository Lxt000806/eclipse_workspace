...
function loadFee(rowData,baseFeeComp){
	...
	if("${custType.setAddPer}" == 0){
 	...
 	var manageFeeBase=myRound((baseFeeDirct*${customer.baseFeeDirctPer}+baseFeeComp*${customer.baseFeeCompPer}
 					+${customer.area}*${customer.areaPer}+mainSetFee*${customer.mainSetFeePer}
 					+setMinus*${customer.setMinusPer}+setAdd*${customer.setAddPer}+longFee*${customer.longFeePer}-dNoManageFee)
 					*${customer.manageFeeBasePer});//基础管理费
...
function add(isInsert){
	...
	if(ret){ baseItemType1Str+="&baseItemType1="+ret.baseitemtype1;
		url="${ctx}/admin/itemPlan/goJzysQuickSave?custCode=${customer.code}&isOutSet=2&itemType1=JZ&custTypeType=2&m_umState=A&custType=${customer.custType}&canUseComBaseItem=${custType.canUseComBaseItem}&fixAreaPk="+ret.fixareapk+baseItemType1Str;
	}else{
	    url="${ctx}/admin/itemPlan/goJzysQuickSave?custCode=${customer.code}&isOutSet=2&itemType1=JZ&custTypeType=2&m_umState=A&custType=${customer.custType}&canUseComBaseItem=${custType.canUseComBaseItem}"+baseItemType1Str; 
	}
...
function edit(){
    ...
    if (rowId) {
	    Global.Dialog.showDialog("jcys_detailViewEdit",{
			  title:"基础预算--编辑",
			  url:"${ctx}/admin/itemPlan/goItemPlan_jcys_detailViewEdit?itemType1=JZ&custCode=${customer.code}&custType=${customer.custType}&canUseComBaseItem=${custType.canUseComBaseItem}&m_umState=A&rowId="+rowId,
			  height:600,
			  width:1000,
			  resizable:false, 
			  returnFun: function(data){
...
/* 清除 sContent:提示内容,stitle：标题,sMustImportTemp：是否必须模板备份导入，sType：清除类型(1:正常清除，2导入清除，3去除草签清除)*/
...
function goImportTab(stitle,sMustImportTemp,isOutSet){
	...
	Global.Dialog.showDialog("itemPlan_importExcel_jcys",{
	  	...
	  	returnFun: function(data){
			...
			$("#dataTable").jqGrid("setGridParam",{url:"${ctx}/admin/itemPlan/goTmpJqGrid",postData:{'params':params,'orderBy':'fixareapk,isgroup'},page:1,rowNum:10000}).trigger("reloadGrid");  
		 }
	});
}
			...
			<div class="panel-body">
				<form action="" method="post" id="dataForm" class="form-search">
					...
					<ul class="ul-form">
						...
						<li><label>模板</label> <input type="text" width: 130px;  id="baseTempNo" name="baseTempNo"
							value="${customer.baseTempNo}" readonly="readonly" />
						</li>
						<li class="search-group-shrink"  style="margin-left:80px" id = "isInitSign_li">
							<input type="checkbox" id="isInitSign" name="isInitSign" value="${customer.isInitSign}",
								onclick="changeIsInitSign(this)" ${customer.isInitSign=="1"?"checked":"" }/>
							<p>草签(无需详细报价,需重签)</p>
						</li>
					</ul>
				</form>
