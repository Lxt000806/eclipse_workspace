...
$(function(){
	...
	function setDetail(data){
		if(data){
			$("#quoteModule").val(data.quotemodule);
			$("#type").val(data.type);
			...
			$("#isCalcDiscCtrl").val(data.iscalcdiscctrl);
	...
	$.ajax({
		...
		type:'post',
		data:{pk:"${custGift.giftPK}",area:"${customer.area}",custType:"${customer.custType}"},
		dataType:'json',
		cache:false,
		async: false,
		error:function(obj){
		...
		success:function(obj){
			var expr = $.trim("${custGift.maxDiscAmtExpr}");
	...
	if($.trim("${custGift.discAmtType }")=="JZ"){
		...
		url="${ctx}/admin/itemPlan/goBaseItemPlanJqGrid?custCode=${customer.code}&giftPk="+"${custGift.giftPK}";
	} else{
		...
		url="${ctx}/admin/itemPlan/goItemPlanJqGrid?isCustGift=1&custCode=${customer.code}&giftPk="+"${custGift.giftPK}";
	}
	$("#giftPK").openComponent_gift({callBack:setDetail,showValue:"${custGift.giftPK}",showLabel:"${custGift.giftDescr}",readonly:true});
	...
	var discAmtTypeSet = {
		...
		firstValue:'${custGift.discAmtType}',
	};
	var quoteModuleSet = {
		firstSelect:"quoteModule",
		firstValue:'${custGift.quoteModule}',
	};
	
	Global.LinkSelect.setSelect(discAmtTypeSet);
	Global.LinkSelect.setSelect(quoteModuleSet);
	
	var postData = $("#page_form").jsonForm();
    //初始化表格
	Global.JqGrid.initJqGrid("dataTable",{
		url:"${ctx}/admin/itemPlan/goCustGiftItemJqgrid",
		postData:{custGiftPK:"${custGift.pK}",custCode:"${customer.code}"},
		...
		colModel : [
			...
			{name: "cost", index: "cost", width: 70, label: "成本", sortable:false, align: "right",hidden: true},
			{name: "beflineamount", index: "beflineamount", width: 90, label: "销售额", sortable:false, align: "right", sum: true},
			{name: "totalcost", index: "totalcost", width: 90, label: "成本额", sortable:false, align: "right", sum: true,hidden:true},
	...
	if("${custGift.discAmtType }".trim()=='JZ'){
	...
	$("#saveBtn").on("click",function(){
		...
		Global.Form.submit("page_form","${ctx}/admin/itemPlan/doCustGiftUpdate",param,function(ret){
			if(ret.rs==true){
				art.dialog({
					content:ret.msg,
					time:1000,
	...
	<div class="body-box-list">
		...
		<div class="query-form">
			<form role="form" class="form-search" id="page_form"  action="" method="post" target="targetFrame">
				...
				<input type="hidden" id="discPer" name="discPer" style="width:160px;" value="${custGift.discPer }"/>
				<input type="hidden" id="custCode" name="custCode" style="width:160px;" value= "${custGift.custCode }"/>
				<input type="hidden" id="pK" name="pK" style="width:160px;" value= "${custGift.pK }"/>
				<input type="hidden" id="totalCost" name="totalCost" style="width:160px;" value= "${custGift.totalCost }"/>
				...
				<ul class="ul-form">
					...
					<li>
						<label>实际优惠金额</label>
						<input type="text" id="discAmount" name="discAmount" style="width:160px;" value= "${custGift.discAmount }" onkeyup="chgPerfDiscType()"/>
					</li>
					<li>
						<label>类型</label>
					...
					<li>
						<label>销售额</label>
						<input type="text" id="saleAmount" name="saleAmount" style="width:160px;" value= "${custGift.saleAmount }"/>
					</li>
					<li>
						<label>优惠金额分类</label>
						<select type="text" id=discAmtType name="discAmtType" style="width:160px;" value= "${custGift.discAmtType }"></select>
					</li>
					<li>
						<label>业绩扣减方式</label>
						<house:xtdm  id="perfDiscType" dictCode="GIFTPERFDSTYPE" style="width:160px;" onchange="chgPerfDiscType()"
						 value= "${custGift.perfDiscType }" ></house:xtdm>
					</li>
					<li>
						<label>业绩扣减比例</label>
						<input type="text" id="perfDiscPer" name="perfDiscPer" style="width:160px;" value= "${custGift.perfDiscPer }"/>
					</li>
					<li>
						<label>业绩扣减额</label>
						<input type="text" id="perfDiscAmount" name="perfDiscAmount" style="width:160px;" value= "${custGift.perfDiscAmount }"/>
					</li>
					<li>
					...
					<li>
						<label>是否软装券</label>
						<house:xtdm id="isSoftToken" dictCode="YESNO" style="width:160px;" value= "${custGift.isSoftToken }"></house:xtdm>
					</li>
					<li>
						<label>最高优惠金额</label>
					...
					<li hidden="true">
						<label>项目经理结算额</label>
						<input type="text" id="projectAmount" name="projectAmount" value="${custGift.projectAmount}" readonly="true"/>
					</li>
					<li>
						<label>优惠额度计算方式</label>
						<house:xtdm id="discAmtCalcType" dictCode="GIFTDACALCTYPE" value="${custGift.discAmtCalcType}" ></house:xtdm>
					</li>
					 <li>
						<label>额度预支项目</label>
						<house:xtdm id="isAdvance" dictCode="YESNO" value="${custGift.isAdvance}" ></house:xtdm>
					</li>
					<li>
						<label>显示顺序</label>
						<input type="text" id="dispSeq" name="dispSeq" style="width:160px;" value= "${custGift.dispSeq }"/>
					</li>
					<%-- <li>
						<label class="control-textarea">说明</label>
						<textarea id="remarks" name="remarks" rows="2">${custGift.remarks }</textarea>
 					</li> --%>
				</ul>	
			</form>
		</div>
