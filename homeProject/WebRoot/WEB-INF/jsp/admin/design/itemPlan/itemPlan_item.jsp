...
function clickTree(treeNode){
	...
	var src=window.document.getElementById("listFrame_q").src.substring(0,len);
	if(treeNode.parentIdStr){
 		src+="?sqlCode="+treeNode.menuIdStr+"&itemType1=${itemPlan.itemType1}&custCode=${itemPlan.custCode}"+"&moduleCall=itemPlan";
	}else{
 		src+="?itemType2="+treeNode.menuIdStr+"&itemType1=${itemPlan.itemType1}&custCode=${itemPlan.custCode}";
	}
	// 套餐材料限制客户类型
	// if(src.indexOf("custTypeItem")!=-1) 
	src+="&custType=${itemPlan.custType}"
...
<frameset cols="120,*" rows="*" border="0" framespacing="1" frameborder="yes" bordercolor="#B2D9FF"
	...
	<frame id="treeFrame_q" name="treeFrame_q"
		src="${ctx}/admin/itemType2/goTree?itemType1=${itemPlan.itemType1}&itemType2=${itemPlan.itemType2}&custCode=${itemPlan.custCode}"
