...
<body>
				...
				<div class="panel-body" >
			    	<div class="btn-group-xs" >
						...
						<c:if test="${customer.status.trim()=='4' && module.trim()=='custDoc'}">
							<span style="color:red">&nbsp&nbsp该客户为合同施工状态，只能修改安装电梯和搬运楼层。</span>
						</c:if>
					</div>
				</div>
			</div>
					...
					<form role="form" class="form-search" id="page_form"  action="" method="post" target="targetFrame" >
						...
						<ul class="ul-form">
							<div class="validate-group row" >
								...
								<li>
									<label><span class="required">*</span>安装电梯</label>
								...
								<li >
									<label><span class="required">*</span>硬搬楼层数</label>
									<input type="text" id="carryFloor" name="carryFloor" onkeyup="value=value.replace(/\D/g,'')" style="width:160px;" value="${customer.carryFloor == 0?'':customer.carryFloor }" />
									<span style="color:red">说明：材料须硬搬的楼层数</span>
								</li>
							</div>
						</ul>
  					</form>
	...
	function save(){
		...
		if(wallArea6=="") $("#wallArea6").val(0);
		if(wallArea12=="") $("#wallArea12").val(0);
		if(wallArea24=="") $("#wallArea24").val(0);
		
		var installElev=$("#installElev").val();
		if(installElev==""){
			art.dialog({content:"请选择是否安装电梯！"});
			return;
		}
		if($.trim($("#carryFloor").val())==""){
			art.dialog({content:"请填写硬搬楼层数！"});
			return;
		}
	    ...
	    var result = checkAreaAndPerimeter()
	    if (result.abnormal) notify += result.content
	...
	// 检查面积与周长是否有异常
	function checkAreaAndPerimeter() {
	    var grid = $("#dataTable")
	    var area = parseFloat($("#area").val())
	    var rows = grid.getRowData()
	    var totalArea = 0, totalPerimeter = 0
	    var abnormal = false
	    
	    for (var i = 0; i < rows.length; i++) {
	        totalArea += parseFloat(rows[i].area)
	        totalPerimeter += parseFloat(rows[i].perimeter)
	    }
	    
	    var template = "<br/><span style='color:red'>请检查空间数据是否异常：</span><br/>"
	    var content = ""
	    
	    if (totalArea > area) {
	        abnormal = true
	        content += "所有空间面积总合计大于【" + area + "】<br/> "
	    }
	    
	    if (totalPerimeter > area * 1.5) {
	        abnormal = true
	        content += "所有空间周长总合计大于【" + area * 1.5 + "】 "
	    }
	    
	    return {
	        abnormal: abnormal,
	        content: template += "<span>" + content + "</span>"
	    }
	}
	...
	$(function(){
		...
		var gridOption = {
			...
			colModel : [
				...
				{name: "area", index: "area", width: 58, label: "面积", sortable: true, align: "right", sum:true,editable:true,editrules:{number:true}},
				{name: "perimeter", index: "perimeter", width: 58, label: "周长", sortable: true, align: "right", sum:true,editable:true,editrules:{number:true}},
				{name: "height", index: "height", width: 58, label: "高度", sortable: true, align: "right",editable:true,editrules:{number:true} },
				{name: "length1", index: "length1", width: 58, label: "长", sortable: true, align: "right",editable:true ,editrules:{number:true}},
				{name: "height1", index: "height1", width: 58, label: "高", sortable: true, align: "right",editable:true ,editrules:{number:true}},
				{name: "length2", index: "length2", width: 58, label: "长", sortable: true, align: "right",editable:true ,editrules:{number:true}},
				{name: "height2", index: "height2", width: 58, label: "高", sortable: true, align: "right",editable:true ,editrules:{number:true}},
			...
			beforeSaveCell:function(rowId,name,val,iRow,iCol){
				...
				if($.trim(ret.isdefaultarea)!="1"){
					...
					if(((checkArea(ret.fixareatype,val) && name=="height")|| name !="height") && !((name == "height1" || name == "height2") && val>3)){
						$.ajax({
						...
						if(name="height1" || name=="height1"){
							art.dialog({
								content:"门洞、窗洞高度不允许超过3米",
								time:2000,
							});
						}
		...
		$("#dataTable").jqGrid('setGroupHeaders', {
			...
			groupHeaders:[{startColumnName: 'length1', numberOfColumns: 2, titleText: '门洞'},
							{startColumnName: 'length2', numberOfColumns: 2, titleText: '窗洞'},
		...
		(function ctlBtn(){
			...
			if($.trim("${module }")==""){
				if($.trim(status)!="1"&&$.trim(status)!="2"&&$.trim(status)!="3" && "${canUpdateArea}" != "true"){
