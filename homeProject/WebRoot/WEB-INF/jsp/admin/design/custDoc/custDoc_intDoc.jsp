...
$(function() {
	...
	var gridOption = {	
		...
		colModel : [
			...
			{name:'UploadCZY',	index:'UploadCZY',	width:95,	label:'UploadCZY',	sortable:true,align:"left" ,hidden:true,},
		...
		beforeSaveCell:function(rowId,name,val,iRow,iCol){
			var ret=selectDataTableRow();
 			//alert(rowId+","+name+","+val+","+iRow+","+iCol);
 			$.ajax({
	...
	$("#del").on("click",function(){
		...
		var date1=new Date(ret.UploadDate);
		var date2=new Date();
		var UploadDate=date1.getFullYear()+":"+(date1.getMonth()+1)+":"+date1.getDate();
		var NowDate=date2.getFullYear()+":"+(date2.getMonth()+1)+":"+date2.getDate();
		if((updatedescr!=lastUpdatedBy || NowDate!=UploadDate)&&hasAuthByCzybh!='true'){
			art.dialog({
				content:'只能删除本人当天上传的图片',
			});
			return false;
		}
