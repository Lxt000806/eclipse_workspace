...
$(function() {
	//独立销售 施工工期 合同工期 默认为0
	if($.trim("${customer.contractDay}") =="" && $.trim("${custType.isAddAllInfo }")=="0" ){
		$("#contractDay").val(0);
	}
	if($.trim("${customer.constructDay}") =="" && $.trim("${custType.isAddAllInfo }")=="0" ){
		$("#constructDay").val(0);
	}
	...
	var isMatch =true;
$(function(){
	if($.trim("${customer.status}")!="4"){
		// 做‘施工工期’的规则匹配，如果存在规则返回true
		$.ajax({
			url:"${ctx}/admin/customerSghtxx/getMatchConDay",
			type:"post",
	...
	if("${customer.status}"=="4"||"${customer.status}"=="5"){
		...
		$("#softTokenAmount").attr("readonly",true);
		$("#softTokenNo").attr("readonly",true);	
	}
...
function calculateContract(){
	...
	if("${customer.tax}"==0 || firstLoad != 0){
		calculateTax();//$("#tax").val(myRound(calculateTax()));
...
function calculateTax(){
	...
	//return result;
	var discAmount = calculateDiscAmount();
	if (parseFloat($("#contractFee").val())!=""){
		$.ajax({
			url:"${ctx}/admin/itemPlan/getTax",
			type:"post",
			data:{code:"${customer.code}", contractFee:parseFloat($("#contractFee").val()==""?0:$("#contractFee").val()),
				designFee:parseFloat($("#stdDesignFee").val()==""?0:$("#stdDesignFee").val())-parseFloat($("#returnDesignFee").val()==""?0:$("#returnDesignFee").val()),
				discAmount:discAmount,
				setAdd:parseFloat(${customer.setAdd}),
				manageFeeBase:parseFloat(${customer.manageFeeBase}),
				baseFeeComp: parseFloat(${customer.baseFeeComp}),
				baseFeeDirct: parseFloat(${customer.baseFeeDirct}),
		...
		$.ajax({
		    ...
		    success: function(obj){
		    	...
		    	if(position==""||area==""||area=="0"){//||designFee=="0"||designFee==""
					...
					calculateTax();//$("#tax").val(calculateTax());
...
function changeConDay(){
	var contractDay=parseFloat($.trim($("#contractDay").val()));
	var constructDay=parseFloat("${customer.constructDay }");
	if(contractDay!=""){
		//return;
		console.log(isMatch);
		if("${customer.status}"!="4"&&"${customer.status}"!="5"){
			if(!isMatch){
				$("#contractDay").val(contractDay);
				//初始化为空时,会转换成NaN,导致无法比较  add by zzr 2020/11/19
				$("#constructDay").val(contractDay <= constructDay || isNaN(constructDay) ? contractDay : constructDay);//取小的那个
			}
		}
	}
}
...
// 计算优惠金额
function calculateDiscAmount(){
	var result = 0;
	result = parseFloat($("#baseDisc").val()==""? 0: $("#baseDisc").val())
		     + parseFloat($("#mainDisc").val()==""? 0: $("#mainDisc").val())
		     + parseFloat($("#softDisc").val()==""? 0: $("#softDisc").val())
		     + parseFloat($("#integrateDisc").val()==""? 0: $("#integrateDisc").val())
			 + parseFloat($("#cupBoardDisc").val()==""? 0: $("#cupBoardDisc").val())
	return result;
}
...
function count(){
	Global.Dialog.showDialog("countDisc",{
		...
		postData:{id:"${customer.code}"},
		height:720,
		width:1170,
		returnFun:function(data){
			$("#gift").val(data.swyh);
			$("#softTokenAmount").val(data.softToken);
		}
	});
}
            ...
            <form role="form" class="form-search" id="dataForm" action="" method="post" target="targetFrame">
            	<house:token></house:token>
					...
					<div class="validate-group row">
						...
						<div class="col-sm-3">
            			<li class="form-validate">
							<label><input type="checkbox" id="ck_mainFee" name="ck_mainFee" onclick="checkClick(this)">主材费</label>
							<input type="text" id="mainFee" name="mainFee" style="width:160px;" value="${customer.mainFee}" readonly="true"/>
						</li>
						</div>
						<div class="col-sm-3">
            			<li class="form-validate">
							<label><input type="checkbox" id="ck_mainServFee" name="ck_mainServFee" onclick="checkClick(this)">服务性产品费</label>
					...
					<div class="validate-group row">
						<div class="col-sm-3">
							<li class="form-validate">
								...
								<input type="text" style="width:160px;" id="area"  name="area" onkeyup="changeDesignFee(this)" value="${customer.area }" />
							</li>
						</div>
						<div class="col-sm-3">
						<li class="form-validate">
					...
					<div class="validate-group row">
            			...
            			<li class="form-validate">
							...
							<input type="text" id="designFee" name="designFee" style="width:160px;" onkeyup="changeDesignFee(this)" value="${customer.designFee}"/>
						</li>
						</div>
						<div class="col-sm-3">
            			<li class="form-validate">
							<label><input type="checkbox" id="ck_integrateFee" name="ck_integrateFee" onclick="checkClick(this)">集成费</label>
					...
					<div class="validate-group row">
            			...
            			<li class="form-validate">
							...
							<input type="text" style="width:160px;" id="achievDed" name="achievDed" value="${customer.achievDed }" onkeyup="value=value.replace(/[^\-?\d.]/g,'')"/>
						</li>
						</div>
						<div class="col-sm-3">
            			<li class="form-validate">
						...
						<div class="col-sm-3">
            			<li class="form-validate">
							<label><input type="checkbox" id="ck_baseFee" name="ck_baseFee" onclick="checkClick(this)" hidden="true">基础费</label>
					...
					<div class="validate-group row">
            			...
            			<li class="form-validate">
							...
							<input type="text" style="width:160px;" id="measureFee" name="measureFee" value="${customer.measureFee }" onkeyup="value=value.replace(/[^\-?\d.]/g,'')"/>
						</li>
						</div>
						<div class="col-sm-3">
            			<li class="form-validate">
							<label><input type="checkbox" id="ck_cupBoardFee" name="ck_cupBoardFee" onclick="checkClick(this)">橱柜费</label>
					...
					<div class="validate-group row">
            			...
            			<li class="form-validate">
							...
							<input type="text" style="width:160px;" id="drawFee" name="drawFee" value="${customer.drawFee }" onkeyup="value=value.replace(/[^\-?\d.]/g,'')"/>
						</li>
						</div>
						<div class="col-sm-3">
            			<li class="form-validate">
					...
					<div class="validate-group row">
            			...
            			<li class="form-validate">
							...
							<input type="text" style="width:160px;" id="colorDrawFee" name="colorDrawFee" value="${customer.colorDrawFee }" onkeyup="value=value.replace(/[^\-?\d.]/g,'')"/>
						</li>
						</div>
						<div class="col-sm-3">
            			<li class="form-validate">
							<label><input type="checkbox" id="ck_softFee" name="ck_softFee" onclick="checkClick(this)">软装费</label>
					...
					<div class="validate-group row">
            			...
            			<li class="form-validate">
							...
							<input type="text" style="width:160px;" id="baseDisc" name="baseDisc" onkeyup="changeDisc()" value="${customer.baseDisc }"/>
						</li>
						</div>
						<div class="col-sm-3">
            			<li class="form-validate">
					...
					<div class="validate-group row">
						<div class="col-sm-3" style="width:356px">
							<div>
		            			<li class="form-validate">
									<label><span class="required">*</span>实物赠送</label>
									<input type="text" style="width:120px;" id="gift" name="gift" value="${customer.gift }" onkeyup="value=value.replace(/[^\-?\d.]/g,'')"/>
									<button type="button" class="btn btn-system "  style="width:40px;height: 24px ;
										font-size: 5px;text-align: center ;float:right;" onclick="count()" >
										<span>查看</span>
									</button>
								</li>
							</div>
						</div>
						<div class="col-sm-3" style="width:305.58px">
            			<li class="form-validate">
							<label style="width:75px">软装其他费用</label>
					...
					<div class="validate-group row">
            			...
            			<li class="form-validate">
							...
							<input type="text" style="width:160px;" id="contractDay" name="contractDay" value="${customer.contractDay }" onchange="changeConDay()" onkeyup="value=value.replace(/[^\-?\d.]/g,'')"/>
            			...
            			<li class="form-validate">
							<label>砌墙面积</label>
							<input type="text" style="width:160px;" id="wallArea" name="wallArea" value="${customer.wallArea }" onkeyup="value=value.replace(/[^\-?\d.]/g,'')"/>
						</li>
						</div>
						<div class="col-sm-3">
            			<li class="form-validate">
					...
					<div class="validate-group row">
            			...
            			<li class="form-validate" id="custTypeHide_4">
							...
							<input type="text" style="width:160px;" id="softBaseFee" name="softBaseFee" value="${customer.softBaseFee }" onkeyup="value=value.replace(/[^\-?\d.]/g,'')"/>
						</li>
						</div>
						<div class="col-sm-3">
            			...
            			<li class="form-validate">
							...
							<input type="text" style="width:160px;" id="softTokenAmount" name="softTokenAmount" value="${customer.softTokenAmount }" onkeyup="value=value.replace(/[^\-?\d.]/g,'')"/>
						</li>
						</div>
					</div>
					<div class="validate-group row">
            			...
            			<li class="form-validate">
							...
							<input type="text" style="width:160px;" id="marketFund" name="marketFund" value="${customer.marketFund }" onkeyup="changeDisc()"/>
						</li>
						</div>
					</div>
					<div class="validate-group row">
