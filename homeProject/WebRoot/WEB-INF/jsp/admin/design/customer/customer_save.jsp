...
function save(){
	...
	if ($("#netChanel").val()=='1' && $.trim($("#againMan").val())==''){
		art.dialog({
	...
	// 	软装客户，客户来源为网络客户的，需要填翻单员  --2018/3/29 且网络渠道
	if (($("#custType").val()=='2' && isNetCust() ) && $.trim($("#againMan").val())==''&& $("#netChanel").val()=='1'){
		art.dialog({
			content: "请输入完整的信息,翻单员未选！",
			width: 200
		});
	    	...
	    	if(obj.rs){
	    		art.dialog({
					...
					beforeunload: function () {
	    				closeWin("${customer.resrCustCode}"==""?true:false);//资源客户转意向不刷新表格
				    }
				});
	    	}else{
...
$(function() {
	function getBuilderNum(data){
		...
		var builderNumArr=$.trim($("#builderNum").val()).split("#");
		...
		if(builderNumArr.length<2){
			$("#address").val($("#openComponent_builder_builderCode").val().split("|")[1]+$.trim($("#builderNum").val())+"#");
		}else{
			$("#address").val($("#openComponent_builder_builderCode").val().split("|")[1]+$.trim($("#builderNum").val()));
		}
	}						 
	$("#builderNum").openComponent_builderNum({showValue:"${customer.builderNum}",condition:{builderCode:"${customer.builderCode}"},
	...
	$("#dataForm").bootstrapValidator({
        ...
        fields: {
      	...
      	openComponent_builderNum_buildeNum:{  
	        validators: {  
	            remote: {
    	            message: '',
    	            url: '${ctx}/admin/builderNum/getBuilderNum',
    	            data: getValidateVal,  
...
function showAgainMan(){
	...
	/* if ($('#netChanel').val()=='1'){
		...
		if ($('#saleType').val()=='1' && $('#source').val()=='6'){
			...
			if (($('#custType').val()=='2' && $('#source').val()=='3') || ($('#source').val()=='6'&&$.trim($("#hasAgainMan").val()))){
				$('.againMan_show').show();
				$('.againMan_stat').show();
				$("#dataForm").bootstrapValidator("removeField","openComponent_employee_againMan");
				$("#dataForm").bootstrapValidator("addField", "openComponent_employee_againMan", {  
...
function changeSaleType(){
	if ($("#saleType").val()=='2'){
		...
		$("#builderCode").openComponent_builder({showLabel:"${customer.builderCodeDescr}",showValue:"${customer.builderCode}",
				...
				validateRefresh('address');isDelivBuilder(data["code"]);}});
	}
	changeSource();
}
	...
	for (var i=0;i<arr.length;i++){
		if (arr[i].code==$("#custType").val() ){
			if (arr[i].isAddAllInfo=='0'){
				...
				//$("#area").val('0');
				$("#status").val("2");
				$("#visitDate").val($("#crtDate").val());
				$("#comeTimes").val("1");
				$("#constructType").val('1');
				if($.trim($("#saleType").val())=='2'){
					$("#address").val($("#addressString").val()+arr[i].desc1);
					$('#dataForm').data('bootstrapValidator')
		                   .updateStatus('address', 'NOT_VALIDATED',null)  
		                   .validateField('address') ;
				}
			} else {
				if(oldCode!=''){
					$("#address").val($("#addressString").val());
				}
				$("#comeTimes").val("");
				$("#visitDate").val("");
				$("#status").val("");
...
function changeStatus(){
	if ($("#status").val()=='1'){
        ...
        if(${customer.resrCustCode != null} && $("#needMeasureDate").val() == "1"){
        	$("#measureDate_star").show();
        	$("#visitDate_star").hide();
        	$("#dataForm").bootstrapValidator("addField","measureDate",{
        		validators: {  
	               notEmpty: {  
	                    message: '量房日期不能为空'  
	                },
        	...
        	$("#dataForm").bootstrapValidator("removeField","visitDate");
        }else{
        	$("#measureDate_star").hide();
        }
	}else{
		$("#visitDate").val($("#crtDate").val());
		$("#comeTimes").val("1");
        ...
        if(${customer.resrCustCode != null}){
	       	$("#visitDate_star").show();
	       	$("#measureDate_star").hide();
	       	$("#dataForm").bootstrapValidator("addField","visitDate",{
	       		validators: {  
	               notEmpty: {  
	                    message: '到店日期不能为空'  
	                },
	       	...
	       	$("#dataForm").bootstrapValidator("removeField","measureDate");
	       	$("#measureDate").next("small").remove();
       }
	}
}
function isDelivBuilder(builderCode){
			...
			for (var i=0;i<arr.length;i++){
				if (arr[i].code==$("#custType").val()){
					if (arr[i].isAddAllInfo=='0'){
						...
						if(builderNumSave!=""&&builderNumSaveArr.length<2){
							$("#builderNum").val(builderNumSave+"#");
							$("#address").val(data["builderdescr"]+$.trim($("#builderNum").val())+"#");
						}
					}
				}
			}
	...
	$("#builderNum").openComponent_builderNum({condition:{builderCode:builderCode},callBack:getBuilderNum});
	$.ajax({
		url:'${ctx}/admin/customer/getIsDelivBuilder',
		type: 'post',
		data: {builderCode:builderCode},
...
function isNeedAgainMan(code){
	$.ajax({
	    ...
	    success: function(obj){
			...
			$("#needMeasureDate").val(obj.NeedMeasureDate);
			$("#channelType").val(obj.ChannelType);
	    }	
	 });
}
	...
	<div class="panel panel-info">  
        <div class="panel-body">
            <form role="form" class="form-search" id="dataForm"  action="" method="post" target="targetFrame">
				...
				<input type="hidden" name="addressString" id="addressString"/>
				<input type="hidden" name="needAgainMan" id="needAgainMan"/>
				<input type="hidden" name="needMeasureDate" id="needMeasureDate"/>
				...
				<input type="hidden" name="resrCustCode" id="resrCustCode" value="${customer.resrCustCode}"/>
					<ul class="ul-form">
					<div class="validate-group row">
						<div class="col-sm-4">
						<li class="form-validate">
							<label><span class="required">*</span>客户编号</label>
							<input type="text" style="width:160px;" id="code" name="code" value="${customer.code }" readonly="readonly" placeholder="保存时生成"/>
						</li>
						</div>
						<div class="col-sm-4">
						<li class="form-validate">
							<label><span class="required">*</span>销售类型</label>
							<house:xtdm id="saleType" dictCode="SALETYPE" value="${czybm.saleType}" onchange="changeSaleType()" unShowValue="0"></house:xtdm>
						</li>
						</div>
						<div class="col-sm-4">
						<li class="form-validate">
					...
					<div class="validate-group row">
						...
						<li class="form-validate">
							<label><span class="required">*</span>客户类型</label>
								...
								sql="select code,code+' '+desc1 desc1 from tCustType where expired= 'F' order by dispSeq " value="${czybm.custType }" sqlValueKey="Code" sqlLableKey="Desc1"></house:dict>
							</li>
						</div>
						<div class="col-sm-4">
							<li class="form-validate">
					...
					<div class="validate-group row">
						...
						<div class="col-sm-4">
							<li class="form-validate">
								...
								<input type="text" style="width:160px;" id="builderNum" onchange="changeBuilderNum()" name="builderNum" value="${customer.builderNum }" />
							</li>
						</div>
						<div class="col-sm-4">
						<li class="form-validate">
					...
					<div class="validate-group row">
						...
						<li class="againMan_show">
							<label><span class="required againMan_stat">*</span>翻单员/导客</label>
					...
					<div class="validate-group row">
						...
						<li class="form-validate">
							<label>到店次数</label>
							<input type="text" style="width:160px;" id="comeTimes" name="comeTimes" value="0"/>
						</li>
						</div>
						<div class="col-sm-4">
						<li class="form-validate">
							<label>创建时间</label>
							<input type="text" style="width:160px;" id="crtDate" name="crtDate" class="i-date" onfocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd'})" value="<fmt:formatDate value='${customer.crtDate }' pattern='yyyy-MM-dd HH:mm:dd'/>" readonly="readonly" ${flag=='doc'?'disabled':'' }/>
						</li>
						</div>
						<div class="col-sm-4">
						<li class="form-validate">
					...
					<div class="validate-group row">
						...
						<li class="form-validate">
							...
							<house:xtdm id="status" dictCode="CUSTOMERSTATUS" unShowValue="3,4,5" value="${customer.status }" 
							disabled="${flag=='doc'?true:false }" onchange="changeStatus()"></house:xtdm>
						</li>
						</div>
						<div class="col-sm-4">
						<li class="form-validate">
							<label><span class="required" id="visitDate_star">*</span>到店日期</label>
							<input type="text" style="width:160px;" id="visitDate" name="visitDate" class="i-date" onfocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd'})" value="<fmt:formatDate value='${customer.visitDate }' pattern='yyyy-MM-dd HH:mm:dd'/>" readonly="readonly" ${flag=='doc'?'disabled':'' }/>
						</li>
						</div>
						<c:if test="${customer.resrCustCode != null}">
							<div class="col-sm-4">
								<li class="form-validate">
									<label><span class="required" id="measureDate_star">*</span>量房日期</label>
									<input type="text" style="width:160px;" id="measureDate" name="measureDate" class="i-date" onfocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd',maxDate:'%y-%M-%d',minDate:'%y-%M-#{%d-3}'})" value="<fmt:formatDate value='${customer.measureDate }' pattern='yyyy-MM-dd HH:mm:dd'/>" />
								</li>
							</div>
						</c:if>
						<div class="col-sm-4">
