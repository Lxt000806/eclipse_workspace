...
$(function() {
	...
	$("#add").on("click",function(){
		Global.Dialog.showDialog("add",{
			title:"浮动规则明细--新增",
			url:"${ctx}/admin/businessCommiFloatRule/goAdd",
			postData:{},
			height: 285,
			width:760,
	...
	$("#update").on("click",function(){
		...
		Global.Dialog.showDialog("addUpdate",{
			title:"浮动规则明细--编辑",
			url:"${ctx}/admin/businessCommiFloatRule/goAddUpdate?map="+encodeURIComponent(JSON.stringify(ret)),
			height: 285,
			width:760,
			returnFun:function(data){
				if(data){
	...
	$("#saveBtn").on("click",function(){
		...
		Global.Form.submit("dataForm","${ctx}/admin/businessCommiFloatRule/doUpdate",param,function(ret){
			if(ret.rs==true){
				art.dialog({
					content:ret.msg,
					time:1000,
					...
					<form action="" method="post" id="dataForm" class="form-search">
						...
						<ul class="ul-form">
							<li class="form-validate">
								...
								<input type="text" id="descr" name="descr" style="width:160px;" readonly="true" value="${businessCommiFloatRule.descr }"/>
							</li>
							<li>
								<label class="control-textarea">备注</label>
								<textarea id="remarks" name="remarks" rows="2">${businessCommiFloatRule.remarks}</textarea>
							</li>
						</ul>	
					</form>
				</div>
