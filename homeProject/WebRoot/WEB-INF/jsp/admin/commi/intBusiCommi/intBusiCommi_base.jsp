...
<head>
	...
	$(function(){
		$("#commiNo").openComponent_itemCommiCycle({
			showValue:"${intBusiCommi.commiNo}"
		});
		
		Global.JqGrid.initJqGrid("dataTable",{
			url:"${ctx}/admin/intBusiCommi/goBaseJqGrid",
			...
			colModel : [
				...
				{name: "disccost", index: "disccost", width: 90, label: "优惠金额分摊", sortable: true, align: "right",hidden:"${intBusiCommi.isInd }" == "1"?false:true,sum:true},
				...
				{name: "crtmon", index: "crtmon", width: 70, label: "生成月份", sortable: true, align: "left",hidden:"${intBusiCommi.isInd }" == "1"?true:false},
	...
	function doExcel(){
		var url = "${ctx}/admin/intBusiCommi/doBaseExcel";
		doExcelAll(url);
	}
	</script>
</head>
<body>
	<div class="body-box-list">
		<c:if test="${intBusiCommi.chgNo != '' && intBusiCommi.chgNo != null}">
			<form role="form" class="form-search" id="page_form" action="" method="post" target="targetFrame">
				<input type="hidden" name="jsonString" value="" />
				<input type="hidden" name="chgNo" value="${intBusiCommi.chgNo }" />
				<input type="hidden" name="isInd" value="${intBusiCommi.isInd }" />
			</form>
		</c:if>
		<c:if test="${intBusiCommi.chgNo == '' || intBusiCommi.chgNo eq null}">
			...
			<div class="query-form">
				<form role="form" class="form-search" id="page_form" action=""
					...
					<input type="hidden" name="isInd" value="${intBusiCommi.isInd }" />
					<ul class="ul-form">
						<c:if test="${intBusiCommi.crtMon != null}">
							<li>
								<label>生成月份</label>
								<input type="text" id="crtMon" name="crtMon" class="i-date" 
								onFocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyyMM'})" value="${intBusiCommi.crtMon }"/>
							</li>
						</c:if>
						<c:if test="${intBusiCommi.commiNo != null}">
							<li>
								<label>提成周期</label>
								<input type="text" id="commiNo" name="commiNo" value="${intBusiCommi.commiNo }"/>
							</li>
						</c:if>
						<li>
							<label>楼盘</label> 
