...
$(function(){
        //初始化表格
		Global.JqGrid.initJqGrid("dataTable_rzdlxstc",{
			url:"${ctx}/admin/softBusiCommi/goIndJqGrid",
			postData: $("#page_form").jsonForm(),
			rowNum: 100000000,
			height:400,
			colModel : [
			    {name: "address", index: "address", width: 200, label: "楼盘", sortable: true, align: "left",formatter:rzdlxstc_custBtn},
				...
				{name: "chgno", index: "chgno", width: 75, label: "单号", sortable: true, align: "left",formatter:rzdlxstc_chgBtn },
			    ...
			    {name: "totalprovideamount", index: "totalprovideamount", width: 100, label: "累计已发提成额", sortable: true, align: "right",formatter:rzdlxstc_hisBtn},
...
function rzdlxstc_custBtn(v,x,r){
	return "<a href='#' onclick='rzdlxstc_viewCust("+x.rowId+");'>"+v+"</a>";
} 

function rzdlxstc_chgBtn(v,x,r){
	return "<a href='#' onclick='rzdlxstc_viewChg("+x.rowId+");'>"+v+"</a>";
}     	

function rzdlxstc_hisBtn(v,x,r){
	return "<a href='#' onclick='rzdlxstc_viewHis("+x.rowId+");'>"+v+"</a>";
}     	

function rzdlxstc_viewChg(id){
	var ret = $("#dataTable_rzdlxstc").jqGrid('getRowData', id);
	if (ret) {
		Global.Dialog.showDialog("viewChg",{
			title:"查看材料明细",
			url:"${ctx}/admin/softBusiCommi/goBase",
			postData:{chgNo:ret.itemchgno,isInd:"1"},
			height:800,
			width:1300
		});
	}else{
...
function rzdlxstc_viewBase(){
	Global.Dialog.showDialog("viewBase",{
		title:"材料明细查询",
		url:"${ctx}/admin/softBusiCommi/goBase?commiNo="+$("#commiNo").val()+"&&isInd=1",
		height:800,
		width:1300
	});
}

function rzdlxstc_viewSum(){
	Global.Dialog.showDialog("viewSum",{
		title:"提成汇总查询",
		url:"${ctx}/admin/softBusiCommi/goSum?commiNo="+$("#commiNo").val()+"&&isInd=1",
		height:600,
		width:1000
	});
}

function rzdlxstc_viewStakeholder(){
	var ret = selectDataTableRow("dataTable_rzdlxstc");
	if (ret) {
...
<div class="body-box-list">
	<div class="panel panel-system">
		<div class="panel-body">
			<form role="form" class="form-search" id="form_rzdlxstc" method="post">
				<input type="hidden" name="jsonString" value="" />
			</form>
			<div class="btn-group-xs">
				<button style="align:left" type="button" class="btn btn-system "
					onclick="rzdlxstc_viewSum()">
					<span>提成汇总查询</span>
				</button>
				<button style="align:left" type="button" class="btn btn-system "
					onclick="rzdlxstc_viewBase()">
					<span>材料明细查询</span>
				</button>
				<button style="align:left" type="button" class="btn btn-system "
					onclick="rzdlxstc_viewStakeholder()">
					<span>查看相关干系人</span>
				</button>
			</div>
		</div>
	...
	<div class="pageContent" >
		<table id="dataTable_rzdlxstc"></table>
	</div>
</div>
