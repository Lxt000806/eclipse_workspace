...
$(function(){
        //初始化表格
		Global.JqGrid.initJqGrid("dataTable_jcdlxstc",{
			url:"${ctx}/admin/intBusiCommi/goIndJqGrid",
			postData: $("#page_form").jsonForm(),
			rowNum: 100000000,
			height:400,
			colModel : [
			    {name: "address", index: "address", width: 200, label: "楼盘", sortable: true, align: "left",formatter:jcdlxstc_custBtn},
				...
				{name: "chgno", index: "chgno", width: 75, label: "单号", sortable: true, align: "left",formatter:jcdlxstc_chgBtn },
			    ...
			    {name: "totalprovideamount", index: "totalprovideamount", width: 100, label: "累计已发提成额", sortable: true, align: "right",formatter:jcdlxstc_hisBtn},
...
function jcdlxstc_custBtn(v,x,r){
	return "<a href='#' onclick='jcdlxstc_viewCust("+x.rowId+");'>"+v+"</a>";
} 

function jcdlxstc_chgBtn(v,x,r){
	return "<a href='#' onclick='jcdlxstc_viewChg("+x.rowId+");'>"+v+"</a>";
}  
   	
function jcdlxstc_hisBtn(v,x,r){
	return "<a href='#' onclick='jcdlxstc_viewHis("+x.rowId+");'>"+v+"</a>";
} 

function jcdlxstc_viewChg(id){
	var ret = $("#dataTable_jcdlxstc").jqGrid('getRowData', id);
	if (ret) {
		Global.Dialog.showDialog("viewChg",{
			title:"查看材料明细",
			url:"${ctx}/admin/intBusiCommi/goBase?chgNo="+ret.itemchgno+"&&isInd=1",
			height:800,
			width:1300
		});
	}else{
...
function jcdlxstc_viewHis(id){
	var ret = $("#dataTable_jcdlxstc").jqGrid('getRowData', id);
	if (ret) {
		Global.Dialog.showDialog("viewHis",{
			title:"查看历史提成",
			url:"${ctx}/admin/intBusiCommi/goHis",
			postData:{custCode:ret.custcode,commiNo:$("#commiNo").val()},
			height:600,
			width:1200
		});
	}else{
...
function jcdlxstc_viewBase(){
	Global.Dialog.showDialog("viewBase",{
		title:"材料明细查询",
		url:"${ctx}/admin/intBusiCommi/goBase?commiNo="+$("#commiNo").val()+"&&isInd=1",
		height:800,
		width:1300
	});
}

function jcdlxstc_viewSum(){
	Global.Dialog.showDialog("viewSum",{
		title:"提成汇总查询",
		url:"${ctx}/admin/intBusiCommi/goSum?commiNo="+$("#commiNo").val()+"&&isInd=1",
		height:600,
		width:1000
	});
}

function jcdlxstc_viewStakeholder(){
	var ret = selectDataTableRow("dataTable_jcdlxstc");
	if (ret) {
...
<div class="body-box-list">
	<div class="panel panel-system">
		<div class="panel-body">
			<form role="form" class="form-search" id="form_jcdlxstc" method="post">
				<input type="hidden" name="jsonString" value="" />
			</form>
			<div class="btn-group-xs">
				<button style="align:left" type="button" class="btn btn-system "
					onclick="jcdlxstc_viewSum()">
					<span>提成汇总查询</span>
				</button>
				<button style="align:left" type="button" class="btn btn-system "
					onclick="jcdlxstc_viewBase()">
					<span>材料明细查询</span>
				</button>
				<button style="align:left" type="button" class="btn btn-system "
					onclick="jcdlxstc_viewStakeholder()">
					<span>查看相关干系人</span>
				</button>
			</div>
		</div>
	...
	<div class="pageContent" >
		<table id="dataTable_jcdlxstc"></table>
	</div>
</div>
