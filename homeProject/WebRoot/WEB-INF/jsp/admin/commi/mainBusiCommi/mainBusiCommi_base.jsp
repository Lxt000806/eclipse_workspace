...
<head>
	...
	$(function(){
		$("#commiNo").openComponent_itemCommiCycle({
			showValue:"${mainBusiCommi.commiNo}"
		});
		
		Global.JqGrid.initJqGrid("dataTable",{
			url:"${ctx}/admin/mainBusiCommi/goBaseJqGrid",
			...
			colModel : [
				...
				{name: "disccost", index: "disccost", width: 90, label: "优惠金额分摊", sortable: true, align: "right",hidden:"${mainBusiCommi.isInd }" == "1"?false:true,sum:true},
				{name: "amount", index: "amount", width: 70, label: "成本总价", sortable: true, align: "right",sum:true},
				...
				{name: "crtmon", index: "crtmon", width: 70, label: "生成月份", sortable: true, align: "left",hidden:"${mainBusiCommi.isInd }" == "1"?true:false},
	...
	function doExcel(){
		var url = "${ctx}/admin/mainBusiCommi/doBaseExcel";
		doExcelAll(url);
	}
	</script>
</head>
<body>
	<div class="body-box-list">
		<c:if test="${mainBusiCommi.chgNo != '' && mainBusiCommi.chgNo != null}">
			<form role="form" class="form-search" id="page_form" action="" method="post" target="targetFrame">
				<input type="hidden" name="jsonString" value="" />
				<input type="hidden" name="chgNo" value="${mainBusiCommi.chgNo }" />
				<input type="hidden" name="isInd" value="${mainBusiCommi.isInd }" />
			</form>
		</c:if>
		<c:if test="${mainBusiCommi.chgNo == '' || mainBusiCommi.chgNo eq null}">
			...
			<div class="query-form">
				<form role="form" class="form-search" id="page_form" action=""
					...
					<input type="hidden" name="isInd" value="${mainBusiCommi.isInd }" />
					<ul class="ul-form">
						<c:if test="${mainBusiCommi.crtMon != null}">
							<li>
								<label>生成月份</label>
								<input type="text" id="crtMon" name="crtMon" class="i-date" 
								onFocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyyMM'})" value="${mainBusiCommi.crtMon }"/>
							</li>
						</c:if>
						<c:if test="${mainBusiCommi.commiNo != null}">
							<li>
								<label>提成周期</label>
