...
$(function(){
        //初始化表格
		Global.JqGrid.initJqGrid("dataTable_zctc",{
			url:"${ctx}/admin/mainBusiCommi/goJqGrid",
			postData: $("#page_form").jsonForm(),
			rowNum: 100000000,
			height:400,
			colModel : [
			    {name: "address", index: "address", width: 120, label: "楼盘", sortable: true, align: "left",formatter:zctc_custBtn},
				...
				{name: "chgno", index: "chgno", width: 75, label: "单号", sortable: true, align: "left",formatter:zctc_chgBtn },
				...
				{name: "totalprovideamount", index: "totalprovideamount", width: 100, label: "累计已发提成额", sortable: true, align: "right",formatter:zctc_hisBtn},
...
function zctc_custBtn(v,x,r){
	return "<a href='#' onclick='zctc_viewCust("+x.rowId+");'>"+v+"</a>";
} 

function zctc_chgBtn(v,x,r){
	return "<a href='#' onclick='zctc_viewChg("+x.rowId+");'>"+v+"</a>";
}     	

function zctc_hisBtn(v,x,r){
	return "<a href='#' onclick='zctc_viewHis("+x.rowId+");'>"+v+"</a>";
}     	

function zctc_viewChg(id){
	var ret = $("#dataTable_zctc").jqGrid('getRowData', id);
	if (ret) {
...
function zctc_viewHis(id){
	var ret = $("#dataTable_zctc").jqGrid('getRowData', id);
	if (ret) {
		Global.Dialog.showDialog("viewHis",{
			title:"查看历史提成",
			url:"${ctx}/admin/mainBusiCommi/goHis",
			postData:{custCode:ret.custcode,crtMon:ret.mon,busiType:ret.busitype},
			height:600,
			width:1200
		});
	}else{
...
function zctc_viewBase(){
	Global.Dialog.showDialog("viewBase",{
		title:"材料明细查询",
		url:"${ctx}/admin/mainBusiCommi/goBase?crtMon="+$("#month").val(),
		height:800,
		width:1300
	});
}

function zctc_viewSum(){
	Global.Dialog.showDialog("viewSum",{
		title:"提成汇总查询",
		url:"${ctx}/admin/mainBusiCommi/goSum?crtMon="+$("#month").val(),
		height:600,
		width:1000
	});
}

function zctc_viewStakeholder(){
	var ret = selectDataTableRow("dataTable_zctc");
	if (ret) {
...
<div class="body-box-list">
	<div class="panel panel-system">
		<div class="panel-body">
			<form role="form" class="form-search" id="form_zctc" method="post">
				<input type="hidden" name="jsonString" value="" />
			</form>
			<div class="btn-group-xs">
				<button style="align:left" type="button" class="btn btn-system "
					onclick="zctc_viewSum()">
					<span>提成汇总查询</span>
				</button>
				<button style="align:left" type="button" class="btn btn-system "
					onclick="zctc_viewBase()">
					<span>材料明细查询</span>
				</button>
			</div>
		</div>
