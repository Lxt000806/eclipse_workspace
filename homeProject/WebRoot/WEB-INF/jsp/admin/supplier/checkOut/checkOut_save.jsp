...
$(function() {
	...
	var gridOption = {
		...
		colModel : [
				  {name : 'checkoutno',index : 'checkoutno',width : 100,label:'checkoutno',sortable : true,align : "left",hidden:true},
		  	      ...
		  	      {name : 'othercost',index : 'othercost',width : 75,label:'其他费用',sortable : true,align : "left"},//${isEdit}
		  	      {name : 'othercostadj',index : 'othercostadj',width : 90,label:'其他费用调整',sortable : true,align : "left",hidden:true}, //去掉其他费用调整
		...
		afterSaveCell:function(rowId,name,val,iRow,iCol){
	    	...
	    	rowData["hjamount"] = (parseFloat(rowData.splamount)+parseFloat(rowData.othercost)).toFixed(2);//+parseFloat(rowData.othercostadj)合计金额计算也不加其他费用调整。 byzjf20190624
	...
	$("#saveBtn").on("click",function(){
		...
		Global.Form.submit("dataForm","${ctx}/admin/supplierCheckOut/doSave",param,function(ret){
			if(ret.rs==true){
				art.dialog({
					content: ret.msg,
					time: 1000,
    	...
    	Global.Dialog.showDialog("addOthercost",{
			 ...
			 returnFun:function(data){
				 if(data){
			        ...
			        rowData["hjamount"] = (parseFloat(rowData.splamount)+parseFloat(data)).toFixed(2); //+parseFloat(rowData.othercostadj)合计金额计算也不加其他费用调整。 byzjf20190624
			    	Global.JqGrid.updRowData("splCheckOutDataTable",rowId,rowData);
				 }
			}
    	});
