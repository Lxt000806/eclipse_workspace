...
$(function(){
	...
	var lastCellRowId;
	/**初始化表格*/
        Global.JqGrid.initJqGrid("dataTable_FH",{
			url:"${ctx}/admin/itemAppSendPhoto/goJqGrid",
			postData:{sendNo:'${itemAppSend.no}'},
			height:480,
			width:200,
		styleUI: 'Bootstrap', 
			colModel : [
             ...
             gridComplete:function(){
				 var a= $("#dataTable_FH").getCol('photoname',false) ;
				 Name=a[0],
				 $('#photoPath').val(Name);
				 $.ajax({
	...
	$("#next").on("click",function(){
			x++;
			var sendphoto = Global.JqGrid.allToJson("dataTable_FH","photoname");
			var type = Global.JqGrid.allToJson("dataTable_FH","type");
			  ...
			  <form role="form" class="form-search" id="dataForm"  action="" method="post" target="targetFrame" >
				...
				<input type="hidden" name="m_umState" id="m_umState" value="M"/>
						<ul class="ul-form">
							<li hidden="true">
								<label>no</label>
								<input type="text" id="no" name="no" style="width:160px;"  value="${itemAppSendPhoto.no}" />
							</li>
							<%-- <li hidden="true">
								<label>客户编号</label>
								<input type="text" id="custCode" name="custCode" style="width:160px;"  value="${itemAppSendPhoto.custCode}" />
							</li>
							<li hidden="true">
								<label>施工节点</label>
								<input type="text" id="address" name="address" style="width:160px;"  value="${itemAppSendPhoto.prjItem}" />
							</li> --%>
						</ul>	
			</form>
			</div>
		...
		<div class="pageContent" >
			...
			<div  style="width:48%;float: left;margin-left: 1px; class="container" >
				<input style="width:160px" value="${itemAppSend.photoPath }" id="photoPath" name="photoPath" hidden ="true"/>
				 <div >  
	           		 <div >  
	             		 <img id="conPicture" src="${itemAppSend.photoPath } " onload="AutoResizeImage(600,460,'conPicture');" width="700" height="490" >  
		          	 </div>  
		  		 </div> 
			</div>
		</div>
