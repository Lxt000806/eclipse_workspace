...
function add(){
	Global.Dialog.showDialog("itemBatchDetailAdd",{
		  title:"添加ItemBatchDetail",
		  url:"${ctx}/admin/itemBatchDetail/goSave",
		  height: 600,
		  width:1000,
...
function update(){
    ...
    if (ret) {
      Global.Dialog.showDialog("itemBatchDetailUpdate",{
		  title:"修改ItemBatchDetail",
		  url:"${ctx}/admin/itemBatchDetail/goUpdate?id="+ret.PK,
		  height:600,
		  width:1000,
		  returnFun: goto_query
		});
...
function copy(){
	var ret = selectDataTableRow();
    if (ret) {
      Global.Dialog.showDialog("itemBatchDetailCopy",{
		  title:"复制ItemBatchDetail",
		  url:"${ctx}/admin/itemBatchDetail/goSave?id="+ret.PK,
		  height:600,
		  width:1000,
		  returnFun: goto_query
		});
...
function view(){
	var ret = selectDataTableRow();
    if (ret) {
      Global.Dialog.showDialog("itemBatchDetailView",{
		  title:"查看ItemBatchDetail",
		  url:"${ctx}/admin/itemBatchDetail/goDetail?id="+ret.PK,
		  height:600,
		  width:1000
		});
    } else {
...
function del(){
	var url = "${ctx}/admin/itemBatchDetail/doDelete";
	beforeDel(url);
}
//导出EXCEL
function doExcel(){
	$.form_submit($("#page_form").get(0), {
		"action": "${ctx}/admin/itemBatchDetail/doExcel"
	});
}
/**初始化表格*/
$(function(){
		Global.JqGrid.initJqGrid("dataTable",{
			url:"${ctx}/admin/itemBatchDetail/goJqGrid",
			multiselect: true,
			height:$(document).height()-$("#content-list").offset().top-70,
			colModel : [
			  {name : 'PK',index : 'PK',width : 100,label:'pk',sortable : true,align : "left"}
			  ,
		      {name : 'IBDNo',index : 'IBDNo',width : 100,label:'ibdno',sortable : true,align : "left"}
			  ,
	...
	<div class="body-box-list">
		<div class="query-form">
			<form action="" method="post" id="page_form" >
				<input type="hidden" id="expired" name="expired" value="${itemBatchDetail.expired}" />
				<table cellpadding="0" cellspacing="0" width="100%">
					...
					<tbody>
						<tr class="td-btn">	
							...
							<td class="td-value">
							<input type="text" id="pk" name="pk" style="width:160px;"  value="${itemBatchDetail.pk}" />
							</td>
						...
						<tr>
							<td colspan="6" class="td-btn">
								<div class="sch_qk_con"> 
									<input type="checkbox" id="expired_show" name="expired_show" value="${itemBatchDetail.expired}" onclick="hideExpired(this)" ${itemBatchDetail.expired!='T'?'checked':'' }/>隐藏过期记录&nbsp;
		...
		<div class="pageContent">
			...
			<div class="panelBar">
            	<ul>
            	<house:authorize authCode="ITEMBATCHDETAIL_SAVE">
                	<li>
				...
				<house:authorize authCode="ITEMBATCHDETAIL_COPY">
                	<li>
				...
				<house:authorize authCode="ITEMBATCHDETAIL_UPDATE">
                	<li>
                ...
                <house:authorize authCode="ITEMBATCHDETAIL_DELETE">
                	<li>
				 ...
				 <house:authorize authCode="ITEMBATCHDETAIL_VIEW">
                	<li>
