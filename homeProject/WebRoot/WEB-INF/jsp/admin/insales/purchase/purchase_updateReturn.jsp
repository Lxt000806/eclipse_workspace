...
function changeRemainAmount(){
	...
	$("#remainAmount").val(parseFloat(-amount)+parseFloat(otherCost));
}
			  ...
			  <form role="form" class="form-search" id="dataForm"  action="" method="post" target="targetFrame" >
					...
					<input type="hidden" name="jsonString" value="" />
							<ul class="ul-form">
								...
								<li>
								<label>其他费用</label>
									<input type="text" id="otherCost" name="otherCost"  onchange="changeRemainAmount()" style="width:160px;" value="${purchase.otherCost }" />
								</li>
								<li>
								<label>客户编号</label>
...
/* 
	...
	var gridOption = {	
		...
		rowNum:10000000,
			colModel : [
				...
				{name:'beflineprice', index:'beflineprice', width:80, label:'折扣前单价',sortable : true,align : "left"},
			...
			var selectedIds = $("#dataTable").jqGrid("getGridParam", "selarrrow");
				 ...
				 var mobile=$("#dataTable").getCol('mobile',false) ;
	             	     var otherCost = $.trim($("#otherCost").val());
					   $("#amount").val(a);
	             	    $("#remainAmount").val(-a+parseInt(otherCost));
	             if(sino!=null) 
	...
	$("#add").on("click",function(){
		...
		Global.Dialog.showDialog("save",{
			  title:"采购入库-新增",
			  url:"${ctx}/admin/purchase/goAddReturn",
	...
	$("#fastAdd").on("click",function(){
		...
		Global.Dialog.showDialog("save",{
			  ...
			  postData:{itemType1:item1,supplier:'${purchase.supplier}',no:'${purchase.oldPUNo}'},
			  height: 680,
			  width:1000,
			    returnFun:function(data){
				  if(data){
	 	...
	 	if(ret){
			Global.Dialog.showDialog("AddView",{
				...
				postData:{itcode:ret.itcode,qtyCal:ret.qtycal,unitPrice:ret.unitprice,sqlCodeDescr:ret.sqlcodedescr,itemtype1:item1,
							remarks:ret.remarks,amount:ret.amount,itdescr:ret.itdescr,befLinePrice:ret.beflineprice
							,puno:'${purchase.oldPUNo }',color:ret.color,uniDescr:ret.unidescr
							,markup:ret.markup},
				height:700,
				width:1000,
				returnFun:function(data){
				  ...
				  if(data){
					  $.each(data,function(k,v){
						  var json = {
							...
							beflineamount:myRound(v.befLineAmount,4),
							markup:v.markup,
						  };
						  Global.JqGrid.addRowData("dataTable",json);
					  });
