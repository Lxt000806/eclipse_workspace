  		...
  		<div class="content-form">
			...
			<div class="panel-body">			 
			 <form role="form" class="form-search" id="page_form"  action="" method="post" target="targetFrame" >
					...
					<input type="hidden" id="itemType2"  name="itemType2" value="${purchaseDetail.itemType2 }"/>
					<input type="hidden" id="projectCost"  name="projectCost" value="${purchaseDetail.projectCost }"/>
					<input type="hidden" id="befLinePrice"  name="befLinePrice"/>
						<ul class="ul-form">
							<li>
							...
							<li>
								<label>可用数量</label>
								<input type="text" id="useqty" name="useqty" style="width:160px;" value="${purchaseDetail.useqty}" readonly="true"/>
							</li>
							<li>
								<label id="qtyCal_label"><span class="required">*</span>采购数量</label>
...
<script type="text/javascript">
	$("#tabs").tabs();
$(function(){
	if("${purchaseDetail.m_umState}"=="R"){
		$("#qtyCal_label").html("退回数量"); 
	}
	if("${purchaseDetail.fromPage}"=="whBack"){
	...
	Global.JqGrid.initJqGrid("dataTable",{
		url:"${ctx}/admin/itemWHBal/goPurchJqGrid?itCode="+'000000',
		...
		$.ajax({
			...
			success:function(obj){
				if (obj){ 
					...
					$('#ypqty').val(obj.ypqty);	    
					$('#useqty').val(obj.allqty+obj.purqty-obj.applyqty-obj.appqty-obj.saleqty-obj.ypqty);	
				}						//库存数量+在途采购-领料申请数-领料审核数-销售申请数-样品库数量 
			}
		});
	}
	$("#itcode").openComponent_item({
	    ...
	    condition:{
	        whCode:"${whcode }",
	        itemType1:'${purchaseDetail.itemtype1}',
...
function changeAmount(){
	...
	var appqty=$("#appqty").val();
   	...
   	var amount=myRound(befLinePrice*markup,2)*qtyCal;
   	var unitPrice=befLinePrice*markup;
