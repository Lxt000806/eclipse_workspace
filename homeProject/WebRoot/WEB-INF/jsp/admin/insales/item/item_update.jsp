...
function save(){
	...
	if ($("#CommiType").val()=="2"&&(parseFloat($("#commiPerc").val())>1||parseFloat($("#commiPerc").val())<0)){
		art.dialog({
			content: "提佣比例不在0-1之间！",
			width: 200
		});
	...
	if($("#descr").val()!="${item.descr}"){
		var sMsg="材料名称被修改,是否继续保存";
		if("${hasItemPlan}"=="1"){
			sMsg="该材料名称被修改,且存在预算,是否继续保存";
		}
   		art.dialog({
			content: sMsg,
			width: 200,
...
function checkIsExistItem() {
	...
	datas.code=$.trim($("#code").val());
	datas.m_umState="${item.m_umState}";
	datas.expired=$.trim($("#expired").val());
...
$(function() {
	...
	if(!${isUpdatePrice}){
   	    ...
   	    $("#projectCost").attr("readonly",true);
...
<body>
	<div class="body-box-form">
		...
		<form action="" method="post" id="dataForm" class="form-search" target="targetFrame" style="padding-top:5px; ">
