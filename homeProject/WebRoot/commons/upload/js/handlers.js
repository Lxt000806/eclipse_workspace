...
function fileQueueError(file, errorCode, message) {
	try {
		if (errorCode === SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED) {
			alert("你的列表有太多的文件。\n" + (message === 0 ? "你已经达到上传限制。" : "您可以选择" + (message > 1 ? "到" + message + " 文件。" : "一个文件。")));
			return;
		}
...
function uploadSuccess(file, serverData) {
	try {
       ...
       if(data.error=='0'){
		    ...
		    *上传成功文件列表
		    */
		   var successTarget = document.getElementById(this.customSettings.successTarget);
		   var serviceData = document.getElementById('serviceData');
			
			if(serviceData){
				if(serviceData.value!=null&&serviceData.value!=""){
					serviceData.value = serviceData.value +","+data.id;
				}else{
					serviceData.value = data.id;
				}
			}
		   if (successTarget) {
				var fileSuccessElement = document.createElement("div");
				fileSuccessElement.className = "progressContainer";
		
				var progressText = document.createElement("div");
				progressText.className = "progressName";
				
				var progressA = document.createElement("a");
				progressA.href = data.url;
				progressA.target="_blank";
				progressA.appendChild(document.createTextNode(file.name));
				
				progressText.appendChild(progressA);
				...
				progressStatus.innerHTML = "上传完成，用时:<b><font color=red> " + zmin + "分:" + zsec + "秒</font></b>";
				
				fileSuccessElement.appendChild(progressText);
				fileSuccessElement.appendChild(progressStatus);
				fileSuccessElement.appendChild(progressBar);
				successTarget.appendChild(fileSuccessElement);
			} 
			
			//回调父窗口
			if(typeof successCallParent == "function"){
				successCallParent(data, file.name);
			}
...
function startUploadFile(swfu){
	swfu.startUpload();
}

function cancelQueue(swfu){
	swfu.cancelUpload();
}

function stopUploadFile(swfu){
	swfu.stopUpload();
}
