...
var artDialog = function (config, ok, cancel) {
    ...
    if (config.ok) {
    	// 增加不显示ok按钮 add by zzr 2019/03/04
    	if(!(config.unShowOkButton == true)){
            config.button.push({
                id: 'ok',
    ...
    */
    time: function (time) {
        ...
        if (time) {
            this._timer = setTimeout(function(){
            	// 增加自定义选择定时关闭后触发的按钮 modify by zzr 2018/05/29 begin
            	if(that.config.timeCloseButtonEvent && that.config.timeCloseButtonEvent != ""){
            		that._click(that.config.timeCloseButtonEvent);
            	}else{
                    that._click('cancel');
            	}
            	// 增加自定义选择定时关闭后触发的按钮 modify by zzr 2018/05/29 end
            }, time);
        };
    ...
    lock: function () {
        ...
        $div.bind('click', function () {
            ...
            //that._click('cancel'); by Victor.chen
        });
    ...
    _click: function (id) {
        ...
        var fn = this._listeners && this._listeners[id] && this._listeners[id].callback;
...
artDialog.defaults = {
    ...
    // 设定time后选择关闭的按钮 add by zzr 2018/05/29
    timeCloseButtonEvent:null,
    
    // 不显示ok按钮 add by zzr 2019/03/04
    unShowOkButton: null
