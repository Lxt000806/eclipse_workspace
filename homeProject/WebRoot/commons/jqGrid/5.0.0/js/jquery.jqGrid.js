...
$.extend($.jgrid,{
	...
	from : function(source){
		...
		this._compare=function(a,b,d){
			...
			// modify by zzr 2018/11/12 修正拼音首字母排序问题 begin
			if (typeof a == "string" && typeof b == "string"){
				if(a.localeCompare(b,'zh') > 0){
					return d;
				}
				if(a.localeCompare(b,'zh') < 0){
					return -d;
			...
			if(a<b){return -d;}
			if(a>b){return d;}
			// modify by zzr 2018/11/12 修正拼音首字母排序问题 end
			return 0;
		};
	...
	return this.each( function() {
		...
		sortData = function (index, idxcol,reload,sor, obj){
			...
			if($.isFunction(ts.p.onSortCol)) {
				...
				//add by zzr 2018/03/19 onSortCol执行后结束标志,避免继续访问数据或者排序 begin
				if(ts.p.onSortColEndFlag == true){
					ts.p.lastsort = idxcol;
					return;
				}
				//add by zzr 2018/03/19 onSortCol执行后结束标志,避免继续访问数据或者排序 end
			}
			if(ts.p.datatype === "local") {
		...
		if(this.p.multiselect) {
			...
			$('#cb_'+$.jgrid.jqID(ts.p.id),this).bind('click',function(){
				...
				if (this.checked) {
					$(ts.rows).each(function(i) {
						if (i>0) {
							if(!$(this).hasClass("ui-subgrid") && !$(this).hasClass("jqgroup") && !$(this).hasClass(disabled) && !$(this).hasClass("jqfoot")){
								...
								//add by xuxiaoxi on 20160110 begin
								if(ts.p.multiSelectBoxOnly) {
									$(this).attr("aria-selected", "true");
								}else{
									$(this).addClass(highlight).attr("aria-selected", "true");
									ts.p.selrow = this.id;
								}
								//add by xuxiaoxi on 20160110 end
								ts.p.selarrrow.push(this.id);
								//ts.p.selrow = this.id;//remove by xuxiaoxi on 20160110
								if(froz) {
					...
					if(ts.p.multiSelectBoxOnly) { ts.p.selrow = null; }
					chk=false;
				}
		...
		$("th",thr).each(function ( j ) {
			...
			//兼容固定列和列拖动 add by zzr 2018/05/28 begin
			var flag = false;
			var selectId = e.currentTarget.id.substring(e.currentTarget.id.indexOf("_")+1);

			var sumWidth = 0;
			
			for(var i = 0;i < ts.p.colModel.length;i++){
				if(ts.p.colModel[i].index == selectId){
					if(ts.p.colModel[i].frozen == true){
						flag = true;
					}
					break;
				}
				if(!(ts.p.colModel[i].hidden == true)){
					sumWidth += parseFloat(ts.p.colModel[i].width);//计算所选列头距离表格最左边的距离
				}
			}
			if(flag && ts.p.mustUseSort == true){
				return false;
			}
			//兼容固定列和列拖动 add by zzr 2018/05/28 end
			
			//解决列拖动因为滚动条导致的标签偏移消失问题 add by zzr 2018/07/04 begin
			Global.Variable.uiSortableWidth = sumWidth;//保存在全局变量上,方便jquery-ui.js进行调用 ①
			
			if(e.target.id.indexOf("jqgh_") != -1){ //由于触发的元素有两种,通过id区分判断,计算偏移值
				Global.Variable.uiSortableLeft = $(e.target).parent().offset().left;//同 ①
			}else{
				Global.Variable.uiSortableLeft = $(e.target).offset().left;//同 ①
			}
			//解决列拖动因为滚动条导致的标签偏移消失问题 add by zzr 2018/07/04 end
		...
		$(ts).before(grid.hDiv).click(function(e) {
			...
			var sSelRowId = ts.p.selrow;//add by xuxiaoxi on 20160110
			if(ts.p.cellEdit === true) {
				if(ts.p.multiselect && scb && cSel){
					//add by xuxiaoxi on 20160110
					if (ts.p.multiSelectBoxOnly){
						if(parseInt(ts.p.iCol,10)>=0  && parseInt(ts.p.iRow,10)>=0) {
							//$("td:eq("+$t.p.iCol+")",$t.rows[$t.p.iRow]).removeClass("edit-cell " + highlight);
							$(ts.rows[ts.p.iRow]).removeClass("selected-row " + hover).find("td:eq("+ts.p.iCol+")").removeClass("edit-cell " + highlight);
						}
					}
					else{//modifiy by xuxiaoxi on 20160114
						$(ts).jqGrid("setSelection", ri ,true,e);
						return; // 修改单元格处于编辑状态下，复选框不能选中问题 add by liuxiaobin 2015-10-31
					}
				} else if (td.length > 0) {
					ri = ptr[0].rowIndex;
			...
			if ( !ts.p.multikey ) {
				if(ts.p.multiselect && ts.p.multiboxonly) {
					//add by xuxiaoxi on 20160109 begin
					if(ts.p.multiSelectBoxOnly) {
						if (sSelRowId) { $($(ts).jqGrid('getGridRowById', sSelRowId)).removeClass(highlight); }
						if (scb) {
							$(ts).jqGrid("setSelection", ri, true, e);
						}else {
							...
							$(ts).jqGrid("setSelection", ri, true, e);
						}//add by xuxiaoxi on 20160109 end
					}else {
						if (scb) {
							$(ts).jqGrid("setSelection", ri, true, e);
						}
				...
				//add by xuxiaoxi on 20160111 begin
				if(ts.p.multiselect && ts.p.multiSelectBoxOnly) {
					if (sSelRowId) {$($(ts).jqGrid('getGridRowById', sSelRowId)).removeClass(highlight);}
					$(ts).jqGrid("setSelection", ri, true, e);
				}else{ //add by xuxiaoxi on 20160111 end
					if(e[ts.p.multikey]) {
						$(ts).jqGrid("setSelection",ri,true,e);
		...
		$(document).bind( "mouseup.jqGrid" + ts.p.id, function () {
			Global.Variable.uiSortableLeft = 0;
			Global.Variable.uiSortableWidth = 0;
...
$.jgrid.extend({
	...
	setSelection : function(selection,onsr, e) {
		return this.each(function(){
			...
			if(!$t.p.multiselect) {	
				...
				//add by xuxiaoxi on 20160109 begin
				if($t.p.multiSelectBoxOnly === true) {
					$t.setHeadCheckBox(false);
					$t.p.selrow = pt.id;
					var td = (e == undefined ? undefined : e.target), scb = (td === undefined ? false : $(td).hasClass("cbox"));
					ia = $.inArray($t.p.selrow, $t.p.selarrrow);
					if (pt.className !== "ui-subgrid") {$(pt).addClass(highlight);};
					if (scb === true || $t.p.multikey && e[$t.p.multikey]) {
						if (ia === -1) {
							if (pt.className !== "ui-subgrid") {$(pt).attr("aria-selected", "true");};
							stat = true;
							$t.p.selarrrow.push($t.p.selrow);
						} else {
							if (pt.className !== "ui-subgrid") {$(pt).attr("aria-selected", "false");};
							stat = false;
		...
		this.each(function() {
			...
			if( $t.p.altRows === true && success ) {
				...
				$($t.rows).each(function(i){
					//modify by liuxiaobin 2015-09-18 原代码为 if(i % 2 === 1) { $(this).addClass(cn); }
	...
	setLabel : function(colname, nData, prop, attrp ){
		return this.each(function(){
			...
			if(pos>=0) {
				//modify by zzr 2018/08/22 固定列标题修改,增加thecolFrozen begin
				var thecol = $("tr.ui-jqgrid-labels th:eq("+pos+")",$t.grid.hDiv);
				var thecolFrozen = null;
				if($t.grid.fhDiv){
					thecolFrozen = $("tr.ui-jqgrid-labels th:eq("+pos+")",$t.grid.fhDiv);
				}
				if (nData){
					...
					if(thecolFrozen){
						var icoFrozen = $(".s-ico",thecolFrozen);
						$("[id^=jqgh_]",thecolFrozen).empty().html(nData).append(icoFrozen);
					}
				}
				if (prop) {
					if(typeof prop === 'string') {
						...
						if(thecolFrozen){
							$(thecolFrozen).addClass(prop);
						}
					} else {
						$(thecol).css(prop);
						if(thecolFrozen){
							$(thecolFrozen).css(prop);
						}
					}
				}
				if(typeof attrp === 'object') {
					$(thecol).attr(attrp);
					if(thecolFrozen){
						$(thecolFrozen).attr(attrp);
					}
				}
				//modify by zzr 2018/08/22 固定列标题修改,增加thecolFrozen end
			}
		});
	},
	...
	getCol : function (col, obj, mathopr) {
		...
		this.each(function(){
			...
			if(pos>=0) {
				var ln = $t.rows.length, i =0, dlen=0, lcount=0;
				if (ln && ln>0){
					while(i<ln){
	...
	setFrozenColumns : function (headerHeight) {
		return this.each(function() {
			if ( !this.grid ) {return;}
			...
			//兼容固定列和列拖动,增加mustUseSort属性屏蔽sortable属性 add by zzr 2018/05/28
			if($t.p.subGrid === true || $t.p.treeGrid === true || ($t.p.cellEdit === true && $t.p.mustUseEdit != true)   || $t.p.scroll || ($t.p.sortable && $t.p.mustUseSort != true))
			{
				return;
			}
			if($t.p.rownumbers) { i++; }
			...
			if(maxfrozen>=0 && frozen) {
				...
				hth = headerHeight || $(".ui-jqgrid-htable","#gview_"+$.jgrid.jqID($t.p.id)).height();
...
//module begin
$.jgrid.extend({
	editCell : function (iRow,iCol, ed, delValue){
		return this.each(function (){
			...
			if (cm.editable===true && ed===true && !cc.hasClass("not-editable-cell") && (!$.isFunction($t.p.isCellEditable) || $t.p.isCellEditable.call($t,nm,iRow,iCol))) {
				if(parseInt($t.p.iCol,10)>=0  && parseInt($t.p.iRow,10)>=0) {
					...
					//add by xuxiaoxi on 20160110 begin
					if($t.p.multiSelectBoxOnly === true){$($t.rows[$t.p.iRow]).removeClass(highlight);}
					//add by xuxiaoxi on 20160110 end
				}
				...
				window.setTimeout(function () { $(elc).focus().select();},1);//添加select()方法 add by xuxiaoxi 2015-12-16
				...
				$("input, select, textarea",cc).bind("keydown",function(e) {
					...
					if (e.keyCode === 13 && !e.shiftKey) {
						...
						//add by liuxiaobin 按回车向下移动可编辑单元格
						e.keyCode = 40;
						// Prevent default action
						//return false;//remove by liuxiaobin 按回车向下移动可编辑单元格删除这一行
					} //Enter
					if (e.keyCode === 9)  {
						if(!$t.grid.hDiv.loading ) {
					...
					// up key
					if(e.keyCode === 38 ){//add by liuxiaobin 按键盘方向键 up
						$($t).jqGrid("saveCell",iRow,iCol);
						if(iRow-1 > 0){
					...
					//if key is down arrow
					if(e.keyCode === 40){//add by liuxiaobin 按键盘方向键 down
						$($t).jqGrid("saveCell",iRow,iCol);
						/* mark by xzp 开启grouping时，iRow+1有可能是汇总行，不可编辑
						if(iRow+1 < $t.rows.length){
							$($t).jqGrid("editCell",iRow+1,iCol,true);
							return false;
						}*/
						/* add by xzp 开启grouping时，iRow+1有可能是汇总行，不可编辑，
						 * 因此往下遍历，直到找到数据行(jqgrow)才进行editCell
						 */
						for (var i = 1; i < $t.rows.length-iRow; i++) {
							if ($($t.rows[iRow+i]).hasClass('jqgrow')) {
								$($t).jqGrid("editCell",iRow+i,iCol,true);
								return false;
							}
						}
					}
					e.stopPropagation();
				}).bind("change",function(k){//add by liuxiaobin 标识数据是否修改 2015-09-21
					isChange = true;
				}).bind("blur",function(e){//add by liuxiaobin 添加失去焦点保存数据 2015-09-21
					if(isChange){
						$($t).jqGrid("saveCell",iRow,iCol);
						isChange = false;
					}else{
				...
				//add by xuxiaoxi 2015-12-16
				if (delValue){
					$("input, select, textarea",cc).val(delValue);
				}
			} else {
				if (parseInt($t.p.iCol,10)>=0  && parseInt($t.p.iRow,10)>=0) {
					//$("td:eq("+$t.p.iCol+")",$t.rows[$t.p.iRow]).removeClass("edit-cell " + highlight);
	...
	GridNav : function() {
		return this.each(function () {
			...
			.keydown(function (e){
				...
				// 默认按任何键都进入编辑状态  add by liuxiaobin 
				if (e.keyCode >= 65 && e.keyCode <= 90 || e.keyCode >= 48 && e.keyCode <= 57 || e.keyCode >= 97 && e.keyCode <= 122){	
					if (parseInt($t.p.iCol,10)>=0 && parseInt($t.p.iRow,10)>=0) {
						$($t).jqGrid("editCell",$t.p.iRow,$t.p.iCol,true,String.fromCharCode(e.keyCode));
						return false;
					}
				}
			});
...
//Helper functions
	...
	info_dialog : function(caption, content,c_b, modalopt) {
		var mopt = {
			...
			resize: false,
			//left:250,
			left:($(document).width()-290)/2, //modify by liuxiaobin 2015-09-24
			top:170,
			zIndex : 10000,
			jqModal : true,
			modal : false,
	...
	createEl : function(eltype,options,vl,autowidth, ajaxso) {
		...
		switch (eltype)
		{
			...
			default :
				...
				$(elem).attr("role",role);
				//修正点击编辑时整行增高问题 modify by zzr 2018/07/25
				if(role == "textbox"){
					$(elem).css({
						"height":"18px"
					});
			...
			if(edtrul.custom === true) {
				if( !(rqfield === false && $.jgrid.isEmpty(val)) ) {
					if($.isFunction(edtrul.custom_func)) {
						var ret = edtrul.custom_func.call(g,val,nm,valref,g.p.iRow);// add rowId(this.p.iRow) to custom_func by zzr 2018/02/05
		...
		return this.each(function(){
			...
			$.each(grp.groups,function(i,n){
				...
				if(grp.groupSummaryPos[n.idx] === 'header')  {
					...
					str += "<tr id=\""+hid+"\"" +(grp.groupCollapse && n.idx>0 ? " style=\"display:none;\" " : " ") + "role=\"row\" class= \"" + common.content + " jqgroup ui-row-"+$t.p.direction+" "+clid+"\"><td style=\"padding-left:"+(n.idx * 12) + "px;"+"\" colspan=\""+(grp.groupColumnShow[n.idx] === false ? colspans-1 : colspans)+"\">" + icon + grpTextStr;
					
					//有配送方式标识，加入配送方式下拉框 modified by cjg 20190321  begin
					if($t.p.showDelivType){
						str=showDelivType(str,grpTextStr,i)
					} 
					//有配送方式标识，加入配送方式下拉框 modified by cjg 20190321  end
	...
	setGroupHeaders : function ( o ) {
		...
		return this.each(function(){
			...
			if (o.useColSpanStyle) {
				...
				$htable.find("span.ui-jqgrid-resize").each(function () {
					...
					if ($parent.is(":visible")) {
						this.style.cssText = 'height: ' + $parent.height() + 'px !important; cursor: col-resize;width:10px';
					}
				});
				...
				$htable.find("div.ui-jqgrid-sortable").each(function () {
					...
					if ($parent.is(":visible") && $parent.is(":has(span.ui-jqgrid-resize)")) {
						...
						var t = ($parent.height() - $ts.outerHeight()) / 2  - 4;
						if (t < 0) t = 0;
						$ts.css('top', t +  'px');
					}
				});
			...
			$(ts).bind('jqGridResizeStop.setGroupHeaders', function (e, nw, idx) {
				$firstRow.find('th').eq(idx).width(nw-7);
			});
		});				
	},
...
$.extend($.jgrid,{
	...
	$.jgrid.extend({
		...
		jqGridExcelExport:function(o){//add by liuxiaobin 2015-10-06 导出表格数据至excel文档中
			o = $.extend({
				exptype : "remote",
				url : null,
				oper: "oper",
			...
			return this.each(function(){
				...
				if(o.exptype === "remote") {
					...
					var colHeader = [];
					if(gprm.colNames && gprm.colModel){
						for(var i=0;i<gprm.colModel.length;i++){
							if(!gprm.colModel[i].hidden){
								var cn = gprm.colModel[i].name,cl = gprm.colModel[i].label||gprm.colNames[i];
								if(gprm.colModel[i].name.lastIndexOf("Link")>0 && gprm.colModel[i].formatter && $.isFunction(gprm.colModel[i].formatter)){
									cn = cn.substring(0,cn.lastIndexOf("Link")); 
								}
								colHeader.push({colName:cn,colLabel:cl});
							}
						}
					}
					var pdata = $.extend(true,{},this.p.postData);
					pdata[o.oper] = o.tag;
					pdata["exportTitle"] = o.exportTitle;
					if(o.exportForm){
						pdata["colHeader"] = colHeader;
						$("input[name='exportData']",$("#"+o.exportForm)).val(JSON.stringify(pdata));
						$("#"+o.exportForm).attr("action",o.url);
						$("#"+o.exportForm).submit();
						$("input[name='exportData']",$("#"+o.exportForm)).val("");
						$("input[name='tableData']",$("#"+o.exportForm)).val("");
						$("#"+o.exportForm).attr("action","");
					}else{
						pdata["colHeader"] = JSON.stringify(colHeader);
						var params = jQuery.param(pdata);
						var url;
						if(o.url.indexOf("?") !== -1) { url = o.url+"&"+params; }
						else { url = o.url+"?"+params; }
