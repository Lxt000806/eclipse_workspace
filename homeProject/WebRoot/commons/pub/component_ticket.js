(function($) {
	"use strict";
	//初始化ticket选择组件
	$.fn.openComponent_ticket = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_ticket_",
	    	blurUrl : ctx+"/admin/ticket/getTicket",
	    	blurCode : "TicketNo",
	    	blurDescr : "",
	    	fetchUrl : ctx+"/admin/ticket/goCode",
	    	fetchCode : "TicketNo",
	    	fetchDescr :"",
	    	fetchTitle : "选择活动编号",
	    	valueOnly : true
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_ticket_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_ticket({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_ticket({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_ticket({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_ticket({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_ticket = function(options){
		if (!options) {
	...
	$.fn.openComponent_ticketNo = function(options){
		var opt={
