(function($) {
	"use strict";
	//初始化员工编号选择组件
	$.fn.openComponent_baseCheckItem = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_baseCheckItem_",
	    	blurUrl : ctx+"/admin/baseCheckItem/getBaseCheckItem",
	    	blurCode : "code",
	    	blurDescr : "descr",
	    	fetchUrl : ctx+"/admin/baseCheckItem/goCode",
	    	fetchCode : "code",
	    	fetchDescr : "descr",
	    	fetchTitle : "基础结算项目"
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_baseCheckItem_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_baseCheckItem({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_baseCheckItem({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_baseCheckItem({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_baseCheckItem({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_baseCheckItem = function(options){
