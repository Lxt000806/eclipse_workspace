(function($) {
	"use strict";
	//初始化领料预申请单号选择组件
	$.fn.openComponent_itemPreApp = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_itemPreApp_",
	    	blurUrl : ctx+"/admin/itemPreApp/getItemPreApp",
	    	blurCode : "no",
	    	blurDescr : "",
	    	fetchUrl : ctx+"/admin/itemPreApp/goCode",
	    	fetchCode : "no",
	    	fetchDescr : "",
	    	fetchTitle : "选择领料预申请单号"
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_itemPreApp_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_itemPreApp({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_itemPreApp({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_itemPreApp({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_itemPreApp({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_itemPreApp = function(options){
		if (!options) {
