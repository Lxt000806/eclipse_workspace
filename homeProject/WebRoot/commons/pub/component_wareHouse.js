(function($) {
	"use strict";
	//初始化仓库编号选择组件
	$.fn.openComponent_wareHouse = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_wareHouse_",
	    	blurUrl : ctx+"/admin/wareHouse/getWareHouse",
	    	blurCode : "code",
	    	blurDescr : "desc1",
	    	fetchUrl : ctx+"/admin/wareHouse/goCode",
	    	fetchCode : "code",
	    	fetchDescr : "desc1",
	    	fetchTitle : "选择仓库编号"
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_wareHouse_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_wareHouse({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_wareHouse({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_wareHouse({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_wareHouse({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_wareHouse = function(options){
		if (!options) {
