(function($) {
    ...
    $.fn.openComponent_purchaseApp = function(options) {
        if (!this.length) {
            return this;
        }
        ...
        $.extend(options, {
            preStr: "openComponent_purchaseApp_",
            blurUrl: ctx + "/admin/purchaseApp/getPurchaseApp",
            blurCode: "no",
            blurDescr: "",
            fetchUrl: ctx + "/admin/purchaseApp/goCode",
            fetchCode: "no",
            fetchDescr: "",
            fetchTitle: "选择采购申请单号",
            valueOnly: true
        ...
        if (!$input1.attr("id")) {
            ...
            if (validator) {
                ...
                $input1.rules("add", {openComponent_purchaseApp_valid: true});
            }
                    ...
                    if (e.keyCode === 13) {
                        if (isSet) {
                            if (options.valueOnly) {
                                $input.setComponent_purchaseApp({showValue: $.trim($input1.val()).split("|")[0]});
                            } else {
                                $input.setComponent_purchaseApp({
                                    showLabel: $.trim($input1.val()).split("|")[1],
                ...
                if (options.isValidate) {
                    if ($.trim($input1.val()) != "" && !isSet) {
                        ...
                        $.post(options.blurUrl, {id: $.trim(inputVal)}, function(ret) {
                            if (ret.code == '0') {
                                if (options.valueOnly) {
                                    $input.setComponent_purchaseApp({showValue: ret.data[options.blurCode]});
                                } else {
                                    $input.setComponent_purchaseApp({
                                        showLabel: ret.data[options.blurDescr],
    ...
    $.fn.setComponent_purchaseApp = function(options) {
        if (!options) {
