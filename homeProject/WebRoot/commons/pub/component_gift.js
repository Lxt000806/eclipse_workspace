(function($) {
	"use strict";
	//初始化销售客户编号选择组件
	$.fn.openComponent_gift = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_gift_",
	    	blurUrl : ctx+"/admin/gift/getgift",
	    	blurCode : "pk",
	    	blurDescr : "descr",
	    	fetchUrl : ctx+"/admin/gift/goCode",
	    	fetchCode : "pk",
	    	fetchDescr : "descr",
	    	fetchTitle : "选择赠送项目"
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_gift_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_gift({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_gift({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_gift({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_gift({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_gift = function(options){
		if (!options) {
