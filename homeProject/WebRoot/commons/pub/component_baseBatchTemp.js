(function($) {
	"use strict";
	//初始化员工编号选择组件
	$.fn.openComponent_baseBatchTemp = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_baseBatchTemp_",
	    	blurUrl : ctx+"/admin/baseBatchTemp/getBaseBatchTemp",
	    	blurCode : "no",
	    	blurDescr : "descr",
	    	fetchUrl : ctx+"/admin/baseBatchTemp/goCode",
	    	fetchCode : "no",
	    	fetchDescr : "descr",
	    	fetchTitle : "基础批量报价模板"
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_baseBatchTemp_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_baseBatchTemp({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_baseBatchTemp({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_baseBatchTemp({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_baseBatchTemp({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_baseBatchTemp = function(options){
