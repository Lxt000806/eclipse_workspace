(function($) {
	"use strict";
	//初始化菜单选择组件
	//修改点：1、Component_menu全文替换，2、$.extend(options) 3、注释修改
	$.fn.openComponent_menu = function(options){
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_menu_",
	    	blurUrl : ctx+"/admin/menu/getMenu",
	    	blurCode : "menuUrl",
	    	blurDescr : "menuName",
	    	fetchUrl : ctx+"/admin/menu/goCode",
	    	fetchCode : "menuurl",
	    	fetchDescr : "menuname",
	    	fetchTitle : "选择菜单编号"
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_menu_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_menu({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_menu({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_menu({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_menu({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_menu = function(options){
		if (!options) {
