(function($) {
	"use strict";
	//初始化latitude选择组件
	$.fn.openComponent_latitude = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_latitude_",
	    	blurUrl : ctx+"/admin/builder/getLatitude",
	    	blurCode : "latitude",
	    	blurDescr : "latitude",
	    	fetchUrl : ctx+"/admin/builder/goLatitude",
	    	fetchCode : "latitude",
	    	fetchDescr :"latitude",
	    	fetchTitle : "获取坐标",
	    	valueOnly : true
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_latitude_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_latitude({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_latitude({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_latitude({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_latitude({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_latitude = function(options){
		if (!options) {
