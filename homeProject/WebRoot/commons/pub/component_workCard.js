(function($) {
	"use strict";
	//初始化workCard选择组件
	$.fn.openComponent_workCard = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_workCard_",
	    	blurUrl : ctx+"/admin/workCard/getWorkCard",
	    	blurCode : "CardID",
	    	blurDescr : "",
	    	fetchUrl : ctx+"/admin/workCard/goCode",
	    	fetchCode : "ActName",
	    	fetchDescr :"",
	    	fetchTitle : "选择工人卡号",
	    	valueOnly : true
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_workCard_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_workCard({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_workCard({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_workCard({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_workCard({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_workCard = function(options){
		if (!options) {
