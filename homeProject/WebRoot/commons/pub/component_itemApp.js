(function($) {
	"use strict";
	//初始化领料单编号选择组件
	$.fn.openComponent_itemApp = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_itemApp_",
	    	blurUrl : ctx+"/admin/itemApp/getItemApp",
	    	blurCode : "no",
	    	blurDescr : "",
	    	fetchUrl : ctx+"/admin/itemApp/goCode",
	    	fetchCode : "no",
	    	fetchDescr : "",
	    	fetchTitle : options.title?options.title:"选择领料单编号",
	    	valueOnly : true
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_itemApp_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_itemApp({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_itemApp({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_itemApp({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_itemApp({showLabel:ret.data[options.blurDescr],
	    ...
	    $input1.next("button").unbind("mousedown").on("mousedown",function(){
			...
			Global.Dialog.showDialog("fetch_"+$input.attr("id"),{
				...
				returnFun:function(returnData){
					if(returnData){
						if (options.valueOnly){
							$input.val($.trim(returnData[options.fetchCode]))&&$input1.val($.trim(returnData[options.fetchCode]));
						}else{
							$input.val($.trim(returnData[options.fetchCode]))&&$input1.val($.trim(returnData[options.fetchCode])
	...
	//设置回填组件值
	$.fn.setComponent_itemApp = function(options){
		if (!options) {
	    ...
	    if(options.showValue){
	    	if (options.showLabel){
	    		$(this).val($.trim(options.showValue))&&$input1.val($.trim(options.showValue)
	    				+"|"+$.trim(options.showLabel))&&$input1.data("data-valid","true");
	    	}else{
	    		$(this).val($.trim(options.showValue))&&$input1.val($.trim(options.showValue))&&$input1.data("data-valid","true");
	    	}
	    }
	}
}(jQuery));