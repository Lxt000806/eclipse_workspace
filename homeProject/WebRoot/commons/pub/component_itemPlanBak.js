(function($) {
	"use strict";
	//初始化ticket选择组件
	$.fn.openComponent_itemPlanBak = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_itemPlanBak_",
	    	blurUrl : ctx+"/admin/itemPlanBak/getItemPlanBak",
	    	blurCode : "no",
	    	blurDescr : "",
	    	fetchUrl : ctx+"/admin/itemPlanBak/goCode",
	    	fetchCode : "no",
	    	fetchDescr :"",
	    	fetchTitle : "选择材料备份编号",
	    	valueOnly : true
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_itemPlanBak_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_itemPlanBak({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_itemPlanBak({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_itemPlanBak({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_itemPlanBak({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_itemPlanBak = function(options){
		if (!options) {
	...
	$.fn.openComponent_itemPlanBakNo = function(options){
		var opt={
