(function($) {
	"use strict";
	//初始化发货时限选择组件
	$.fn.openComponent_sendTime = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_sendTime_",
	    	blurUrl : ctx+"/admin/sendTime/goJqGrid",
	    	blurCode : "no",
	    	blurDescr : "",
	    	fetchUrl : ctx+"/admin/sendTime/goCode",
	    	fetchCode : "no",
	    	fetchDescr :"",
	    	fetchTitle : "选择发货时限编号",
	    	valueOnly : true //true只有一个数据要返回
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_sendTime_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_sendTime({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_sendTime({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_sendTime({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_sendTime({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_sendTime = function(options){
		if (!options) {
