(function($) {
	"use strict";
	//初始化softPerf选择组件
	$.fn.openComponent_softPerf = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_softPerf_",
	    	blurUrl : ctx+"/admin/softPerf/getSoftPerf",
	    	blurCode : "no",
	    	blurDescr : "",
	    	fetchUrl : ctx+"/admin/softPerf/goCode",
	    	fetchCode : "no",
	    	fetchDescr :"",
	    	fetchTitle : "选择周期编号",
	    	valueOnly : true
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_softPerf_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_softPerf({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_softPerf({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_softPerf({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_softPerf({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_softPerf = function(options){
		if (!options) {
