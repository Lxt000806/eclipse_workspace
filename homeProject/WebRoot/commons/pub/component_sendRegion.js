(function($) {
	"use strict";
	//初始化sendRegion选择组件
	$.fn.openComponent_sendRegion = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_sendRegion_",
	    	blurUrl : ctx+"/admin/sendRegion/getSendRegion",
	    	blurCode : "no",
	    	blurDescr : "descr",
	    	fetchUrl : ctx+"/admin/sendRegion/goCode",
	    	fetchCode : "no",
	    	fetchDescr :"descr",
	    	fetchTitle : "选择配送区域编号",
	    	valueOnly : false
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_sendRegion_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_sendRegion({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_sendRegion({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_sendRegion({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_sendRegion({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_sendRegion = function(options){
		if (!options) {
