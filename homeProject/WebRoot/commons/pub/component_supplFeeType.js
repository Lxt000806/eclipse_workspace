(function($) {
	"use strict";
	//初始化员工编号选择组件
	$.fn.openComponent_supplFeeType = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_supplFeeType_",
	    	blurUrl : ctx+"/admin/supplFeeType/getSupplFeeType",
	    	blurCode : "code",
	    	blurDescr : "descr",
	    	fetchUrl : ctx+"/admin/supplFeeType/goCode",
	    	fetchCode : "code",
	    	fetchDescr :"descr",
	    	fetchTitle : "选择供应商费用类型"
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_supplFeeType_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_supplFeeType({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_supplFeeType({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_supplFeeType({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_supplFeeType({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_supplFeeType = function(options){
