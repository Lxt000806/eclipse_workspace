(function($) {
	"use strict";
	//初始化activity选择组件
	$.fn.openComponent_activity = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_activity_",
	    	blurUrl : ctx+"/admin/activity/getActivity",
	    	blurCode : "no",
	    	blurDescr : "actName",
	    	fetchUrl : ctx+"/admin/activity/goCode",
	    	fetchCode : "no",
	    	fetchDescr :"actname",
	    	fetchTitle : "选择活动编号",
	    	valueOnly : false
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_activity_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_activity({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_activity({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_activity({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_activity({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_activity = function(options){
		if (!options) {
