(function($) {
	"use strict";
	//初始化act_user选择组件
	$.fn.openComponent_act_user = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_act_user_",
//	    	blurUrl : ctx+"/admin/actUser/getActUser",
//	    	blurCode : "",
//	    	blurDescr : "",
	    	fetchUrl : ctx+"/admin/actUser/goCode",
	    	fetchCode : "ID_",
	    	fetchDescr :"",
	    	fetchTitle : "选择OA操作员编号",
	    	valueOnly : true
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_act_user_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_act_user({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_act_user({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_act_user({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_act_user({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_act_user = function(options){
		if (!options) {
