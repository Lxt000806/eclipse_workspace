(function($) {
	"use strict";
	//初始化专盘编号选择组件
	$.fn.openComponent_spcBuilder = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_spcBuilder_",
	    	blurUrl : ctx+"/admin/spcBuilder/getSpcBuilder",
	    	blurCode : "code",
	    	blurDescr : "descr",
	    	fetchUrl : ctx+"/admin/spcBuilder/goCode",
	    	fetchCode : "Code",
	    	fetchDescr : "Descr",
	    	fetchTitle : "选择专盘编号"
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_spcBuilder_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_spcBuilder({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_spcBuilder({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_spcBuilder({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_spcBuilder({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_spcBuilder = function(options){
		if (!options) {
