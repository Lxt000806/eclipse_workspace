(function($) {
	"use strict";
	//初始化员工编号选择组件
	$.fn.openComponent_laborFeeType = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_laborFeeType_",
	    	blurUrl : ctx+"/admin/laborFeeType/getLaborFeeType",
	    	blurCode : "code",
	    	blurDescr : "descr",
	    	fetchUrl : ctx+"/admin/laborFeeType/goCode",
	    	fetchCode : "code",
	    	fetchDescr :"descr",
	    	fetchTitle : "选择费用类型"
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_laborFeeType_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_laborFeeType({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_laborFeeType({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_laborFeeType({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_laborFeeType({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_laborFeeType = function(options){
