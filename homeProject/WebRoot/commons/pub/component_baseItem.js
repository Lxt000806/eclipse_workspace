(function($) {
	"use strict";
	//初始化员工编号选择组件
	$.fn.openComponent_baseItem = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_baseItem_",
	    	blurUrl : ctx+"/admin/baseItem/getBaseItem",
	    	blurCode : "code",
	    	blurDescr : "descr",
	    	fetchUrl : ctx+"/admin/baseItem/goCode",
	    	fetchCode : "code",
	    	fetchDescr : "descr",
	    	fetchTitle : "基础项目"
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_baseItem_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_baseItem({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_baseItem({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_baseItem({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_baseItem({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_baseItem = function(options){
