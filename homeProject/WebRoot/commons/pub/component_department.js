(function($) {
	"use strict";
	//初始化员工编号选择组件
	$.fn.openComponent_department = function(options){	
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_department_",
	    	blurUrl : ctx+"/admin/department/getDepartment",
	    	blurCode : "code",
	    	blurDescr : "desc2",
	    	fetchUrl : ctx+"/admin/department/goCode",
	    	fetchCode : "code",
	    	fetchDescr :"desc2",
	    	fetchTitle : "选择部门",
	    	id:"openComponent_department_"+$(this)[0].id,
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_department_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_department({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_department({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_department({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_department({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_department = function(options){
	...
	$.fn.openComponent_departmentDescr = function(options){
		console.log(this);
		options = options||{};
		$.extend(options,{
