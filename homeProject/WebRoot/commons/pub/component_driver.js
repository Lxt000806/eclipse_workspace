(function($) {
	"use strict";
	//初始化driver选择组件
	$.fn.openComponent_driver = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_driver_",
	    	blurUrl : ctx+"/admin/driver/getDriver",
	    	blurCode : "Code",
	    	blurDescr : "NameChi",
	    	fetchUrl : ctx+"/admin/driver/goCode",
	    	fetchCode : "Code",
	    	fetchDescr :"NameChi",
	    	fetchTitle : "选择司机编号",
	    	valueOnly : false
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_driver_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_driver({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_driver({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_driver({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_driver({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_driver = function(options){
		if (!options) {
