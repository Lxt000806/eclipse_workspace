(function($) {
	"use strict";
	//初始化项目大类编号选择组件
	$.fn.openComponent_builderGroup = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_builderGroup_",
	    	blurUrl : ctx+"/admin/builderGroup/getBuilderGroup",
	    	blurCode : "code",
	    	blurDescr : "descr",
	    	fetchUrl : ctx+"/admin/builderGroup/goCode",
	    	fetchCode : "Code",
	    	fetchDescr : "Descr",
	    	fetchTitle : "选择项目大类编号"
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_builderGroup_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_builderGroup({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_builderGroup({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_builderGroup({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_builderGroup({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_builderGroup = function(options){
		if (!options) {
