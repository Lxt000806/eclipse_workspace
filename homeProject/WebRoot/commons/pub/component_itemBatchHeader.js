(function($) {
	"use strict";
	//初始化员工编号选择组件
	$.fn.openComponent_itemBatchHeader = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_itemBatchHeader_",
	    	blurUrl : ctx+"/admin/itemBatchHeader/getItemBatchHeader",
	    	blurCode : "no",
	    	blurDescr : "remarks",
	    	fetchUrl : ctx+"/admin/itemBatchHeader/goCode",
	    	fetchCode : "no",
	    	fetchDescr : "remarks",
	    	fetchTitle : "选择批次编号"
	    });
	    ...
	    if(!$input1.attr("id")){
	    	var $component = "<div class=\"input-group\"><input type=\"text\" style=\"width:180px\" class=\"form-control\""+validateStr+"name=\""+options.preStr
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_itemBatchHeader_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_itemBatchHeader({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_itemBatchHeader({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_itemBatchHeader({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_itemBatchHeader({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_itemBatchHeader = function(options){
