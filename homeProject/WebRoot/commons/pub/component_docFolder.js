(function($) {
	"use strict";
	//初始化资产类型选择组件
	$.fn.openComponent_docFolder = function(options){
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_docFolder_",
	    	blurUrl : ctx+"/admin/docFolder/getDocFolder",
	    	blurCode : "pk",
	    	blurDescr : "folderName",
	    	fetchUrl : ctx+"/admin/docFolder/goCode",
	    	fetchCode : "pk",
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_docFolder_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_docFolder({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_docFolder({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,params,function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_docFolder({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_docFolder({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_docFolder = function(options){
		if (!options) {
