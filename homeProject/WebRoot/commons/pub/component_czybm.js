(function($) {
	"use strict";
	//初始化操作员编号选择组件
	//修改点：1、Component_czybm全文替换，2、$.extend(options) 3、注释修改
	$.fn.openComponent_czybm = function(options){
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_czybm_",
	    	blurUrl : ctx+"/admin/czybm/getCzybm",
	    	blurCode : "czybh",
	    	blurDescr : "zwxm",
	    	fetchUrl : ctx+"/admin/czybm/goCode",
	    	fetchCode : "czybh",
	    	fetchDescr : "zwxm",
	    	fetchTitle : "选择操作员编号"
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_czybm_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_czybm({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_czybm({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_czybm({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_czybm({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_czybm = function(options){
		if (!options) {
