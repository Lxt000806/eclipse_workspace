(function($) {
	"use strict";
	//初始化销售客户编号选择组件
	$.fn.openComponent_saleCust = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_saleCust_",
	    	blurUrl : ctx+"/admin/saleCust/getSaleCust",
	    	blurCode : "code",
	    	blurDescr : "desc1",
	    	fetchUrl : ctx+"/admin/saleCust/goCode",
	    	fetchCode : "Code",
	    	fetchDescr : "Desc1",
	    	fetchTitle : "选择销售客户编号"
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_saleCust_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_saleCust({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_saleCust({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_saleCust({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_saleCust({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_saleCust = function(options){
		if (!options) {
