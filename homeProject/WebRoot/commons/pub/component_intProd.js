(function($) {
	"use strict";
	//初始化员工编号选择组件
	$.fn.openComponent_intProd = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_intProd_",
	    	blurUrl : ctx+"/admin/intProd/getIntProd",
	    	blurCode : "PK",
	    	blurDescr : "Descr",
	    	fetchUrl : ctx+"/admin/intProd/goCode",
	    	fetchCode : "PK",
	    	fetchDescr : "Descr",
	    	fetchTitle : "集成成品"
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_intProd_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_intProd({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_intProd({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_intProd({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_intProd({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_intProd = function(options){
