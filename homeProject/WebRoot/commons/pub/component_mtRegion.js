(function($) {
	"use strict";
	//初始化材料编号选择组件
	//修改点：1、Component_mtRegion全文替换，2、$.extend(options) 3、注释修改
	$.fn.openComponent_mtRegion = function(options){
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_mtRegion_",
	    	blurUrl : ctx+"/admin/mtRegion/getMtRegion",
	    	blurCode : "regioncode",
	    	blurDescr : "descr",
	    	fetchUrl : ctx+"/admin/mtRegion/goCode",
	    	fetchCode : "regioncode",
	    	fetchDescr : "descr",
	    	fetchTitle : "麦田大区选择"
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_mtRegion_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_mtRegion({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_mtRegion({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.regioncode=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_mtRegion({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_mtRegion({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_mtRegion = function(options){
		if (!options) {
	...
	$.fn.openComponent_mtRegionDescr = function(options){
		var opt={
