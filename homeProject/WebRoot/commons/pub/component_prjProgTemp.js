(function($) {
	"use strict";
	//初始化prjProgTemp选择组件
	$.fn.openComponent_prjProgTemp = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_prjProgTemp_",
	    	blurUrl : ctx+"/admin/prjProgTemp/getPrjProgTemp",
	    	blurCode : "No",
	    	blurDescr : "",
	    	fetchUrl : ctx+"/admin/prjProgTemp/goCode",
	    	fetchCode : "No",
	    	fetchDescr :"",
	    	fetchTitle : "搜寻——工程进度模板",
	    	valueOnly : true
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_prjProgTemp_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_prjProgTemp({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_prjProgTemp({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_prjProgTemp({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_prjProgTemp({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_prjProgTemp = function(options){
		if (!options) {
