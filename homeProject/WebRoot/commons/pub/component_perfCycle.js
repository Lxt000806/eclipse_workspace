(function($) {
	"use strict";
	//初始化perfCycle选择组件
	$.fn.openComponent_perfCycle = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_perfCycle_",
	    	blurUrl : ctx+"/admin/perfCycle/getPerfCycle",
	    	blurCode : "no",
	    	blurDescr : "",
	    	fetchUrl : ctx+"/admin/perfCycle/goCode",
	    	fetchCode : "no",
	    	fetchDescr :"",
	    	fetchTitle : "选择周期编号",
	    	valueOnly : true
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_perfCycle_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_perfCycle({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_perfCycle({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_perfCycle({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_perfCycle({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_perfCycle = function(options){
		if (!options) {
