(function($) {
	"use strict";
	//初始化worker选择组件
	$.fn.openComponent_custWorker = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_custWorker_",
	    	blurUrl : ctx+"/admin/custWorker/getCustWorker",
	    	blurCode : "workercode",
	    	blurDescr : "workername",
	    	fetchUrl : ctx+"/admin/custWorker/goCode",
	    	fetchCode : "workercode",
	    	fetchDescr :"workername",
	    	fetchTitle : "选择工人编号",
	    	valueOnly : false
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_worker_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_custWorker({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_custWorker({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_custWorker({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_custWorker({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_custWorker = function(options){
		if (!options) {
