(function($) {
	"use strict";
	//初始化itemSendNo选择组件
	$.fn.openComponent_itemSendNo = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_itemSendNo_",
	    	blurUrl : ctx+"/admin/laborFee/getItemSendNo",
	    	blurCode : "no",
	    	blurDescr : "",
	    	fetchUrl : ctx+"/admin/laborFee/goItemSendNoCode",
	    	fetchCode : "no",
	    	fetchDescr :"",
	    	fetchTitle : "搜寻——发货单号",
	    	valueOnly : true
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_itemSendNo_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_itemSendNo({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_itemSendNo({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_itemSendNo({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_itemSendNo({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_itemSendNo = function(options){
		if (!options) {
