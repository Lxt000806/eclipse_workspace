(function($) {
	"use strict";
	//初始化配送批次号选择组件
	$.fn.openComponent_itemSendBatch = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_itemSendBatch_",
	    	blurUrl : ctx+"/admin/itemSendBatch/getItemSendBatch",
	    	blurCode : "no",
	    	blurDescr : "",
	    	fetchUrl : ctx+"/admin/itemSendBatch/goCode",
	    	fetchCode : "no",
	    	fetchDescr : "",
	    	fetchTitle : "选择配送批次号"
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_itemSendBatch_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_itemSendBatch({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_itemSendBatch({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_itemSendBatch({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_itemSendBatch({showLabel:ret.data[options.blurDescr],
	    ...
	    $input1.next("button").unbind("mousedown").on("mousedown",function(){
			...
			Global.Dialog.showDialog("fetch_"+$input.attr("id"),{
				...
				height: !options.height?600:options.height,
				width:!options.width?1000:options.width,
	...
	//设置回填组件值
	$.fn.setComponent_itemSendBatch = function(options){
		if (!options) {
