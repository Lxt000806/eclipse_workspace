(function($) {
	"use strict";
	//初始化员工编号选择组件
	$.fn.openComponent_department2 = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_department2_",
	    	blurUrl : ctx+"/admin/department2/getDepartment2",
	    	blurCode : "code",
	    	blurDescr : "",
	    	fetchUrl : ctx+"/admin/department2/goCode",
	    	fetchCode : "code",
	    	fetchDescr :"desc1",
	    	fetchTitle : "选择二级部门",
	    	valueOnly : false,
	    	id:"openComponent_department2_"+$(this)[0].id,
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_department2_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_department2({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_department2({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_department2({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_department2({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_department2 = function(options){
