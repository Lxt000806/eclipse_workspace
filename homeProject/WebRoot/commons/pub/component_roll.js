(function($) {
	"use strict";
	//初始化角色选择组件
	$.fn.openComponent_roll = function(options){
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_roll_",
	    	blurUrl : ctx+"/admin/roll/getRoll",
	    	blurCode : "code",
	    	blurDescr : "descr",
	    	fetchUrl : ctx+"/admin/roll/goCode",
	    	fetchCode : "code",
	    	fetchDescr : "descr",
	    	fetchTitle : "选择角色"
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_roll_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_roll({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_roll({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_roll({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_roll({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_roll = function(options){
		if (!options) {
