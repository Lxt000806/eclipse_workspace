(function($) {
	"use strict";
	//初始化员工编号选择组件
	$.fn.openComponent_itemBatchDetail = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_itemBatchDetail_",
	    	blurUrl : ctx+"/admin/itemBatchDetail/getItemBatchDetail",
	    	blurCode : "no",
	    	blurDescr : "",
	    	fetchUrl : ctx+"/admin/itemBatchDetail/goCode",
	    	fetchCode : "no",
	    	fetchDescr : "",
	    	fetchTitle : "选择材料批次号",
	    	width: 1200	
	    ...
	    if(!$input1.attr("id")){
	    	var $component = "<div class=\"input-group\"><input type=\"text\" style=\"width:130px\" class=\"form-control\""+validateStr+"name=\""+options.preStr
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_itemBatchDetail_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_itemBatchDetail({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_itemBatchDetail({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_itemBatchDetail({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_itemBatchDetail({showLabel:ret.data[options.blurDescr],
					...
					if(returnData){
						if (returnData.length==undefined){
		    				...
		    				if(options.callBack && $.isFunction(options.callBack)){
							  ...
							  {
								  sReturnValue=sReturnValue+returnData[i][options.fetchCode];
								  if( i<returnData.length-1){
									  sReturnValue=sReturnValue+','
								  } 
							  };
							  $input.val(sReturnValue)&&$input1.val( sReturnValue);	
						}
					}
				}
			});	
	...
	//设置回填组件值
	$.fn.setComponent_itemBatchDetail = function(options){
