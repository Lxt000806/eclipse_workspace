(function($) {
	"use strict";
	//初始化prjWithHold选择组件
	$.fn.openComponent_prjWithHold = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_prjWithHold_",
	    	blurUrl : ctx+"/admin/prjWithHold/getprjWithHold",
	    	blurCode : "pk",
	    	blurDescr : "",
	    	fetchUrl : ctx+"/admin/prjWithHold/goCode",
	    	fetchCode : "pk",
	    	fetchDescr :"",
	    	fetchTitle : "选择预扣单号",
	    	valueOnly : true
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_prjWithHold_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_prjWithHold({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_prjWithHold({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_prjWithHold({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_prjWithHold({showLabel:ret.data[options.blurDescr],
	    ...
	    $input1.next("button").unbind("mousedown").on("mousedown",function(){
			...
			if($("#"+options.isWithHold).val()=="0"){
			    art.dialog({
		       		content: "<font color='red'>是否预扣</font>为<font color='red'>1 是</font>时才能选择预扣单号！",
		       	});
			    return;
		    }
	    	if(options.custCode!=undefined)
		    if($("#"+options.custCode).val()==""){
		    	art.dialog({
	       			content: "请输入客户编号！",
	       		});
		    	return;
		    }
	    	if(options.workType2!=undefined)
		    if($("#"+options.workType2).val()==""){
		    	art.dialog({
	       			content: "请输入工种类型2！",
	       		});
		    	return;
	...
	//设置回填组件值
	$.fn.setComponent_prjWithHold = function(options){
		if (!options) {
