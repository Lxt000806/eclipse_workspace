(function($) {
	"use strict";
	//初始化礼品领用单编号选择组件
	$.fn.openComponent_giftToken = function(options){
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_giftToken_",
	    	blurUrl : ctx+"/admin/giftToken/getGiftToken",
	    	blurCode : "pk",
	    	blurDescr : "tokenno",
	    	fetchUrl : ctx+"/admin/giftToken/goCode",
	    	fetchCode : "pk",
	    	fetchDescr : "tokenno",
	    	fetchTitle : "选择礼品券pk",
	    	labelOnly : true,
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_giftToken_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_giftToken({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_giftToken({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_giftToken({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_giftToken({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_giftToken = function(options){
		if (!options) {
