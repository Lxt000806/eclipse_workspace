(function($) {
	"use strict";
	//初始化供应商编号选择组件
	//修改点：1、supplier全文替换，2、$.extend(options) 3、注释修改
	$.fn.openComponent_supplier = function(options){
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_supplier_",
	    	blurUrl : ctx+"/admin/supplier/getSupplier",
	    	blurCode : "code",
	    	blurDescr : "descr",
	    	fetchUrl : ctx+"/admin/supplier/goCode",
	    	fetchCode : "Code",
	    	fetchDescr : "Descr",
	    	fetchTitle : "选择供应商编号"
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_supplier_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_supplier({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_supplier({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_supplier({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_supplier({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_supplier = function(options){
		if (!options) {
