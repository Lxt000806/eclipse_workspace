(function($) {
	"use strict";
	//初始化资产类型选择组件
	$.fn.openComponent_guideTopicFolder = function(options){
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_guideTopicFolder_",
	    	blurUrl : ctx+"/admin/guideTopicFolder/getGuideTopicFolder",
	    	blurCode : "pk",
	    	blurDescr : "folderName",
	    	fetchUrl : ctx+"/admin/guideTopicFolder/goCode",
	    	fetchCode : "pk",
	    	fetchDescr : "foldername",
	    	fetchTitle : "类目编号",
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_guideTopicFolder_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_guideTopicFolder({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_guideTopicFolder({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,params,function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_guideTopicFolder({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_guideTopicFolder({showLabel:ret.data[options.blurDescr],
				    			...
				    			isSet = false;
				    			$input1.parents("form").eq(0).data('bootstrapValidator')  
				                    .updateStatus(options.preStr+$input.attr("name"), 'INVALID','callback');  
				    		}
				    	},"json");
			    	}else{
		    			$input1.data("data-valid","true");
	...
	//设置回填组件值
	$.fn.setComponent_guideTopicFolder = function(options){
		if (!options) {
