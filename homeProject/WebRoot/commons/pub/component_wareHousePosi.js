(function($) {
	"use strict";
	//初始化库位编号选择组件
	$.fn.openComponent_wareHousePosi = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_wareHousePosi_",
	    	blurUrl : ctx+"/admin/wareHousePosi/getWareHousePosi",
	    	blurCode : "pk",
	    	blurDescr : "desc1",
	    	fetchUrl : ctx+"/admin/wareHousePosi/goCode",
	    	fetchCode : "pk",
	    	fetchDescr : "desc1",
	    	fetchTitle : "选择库位编号"
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_wareHousePosi_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_wareHousePosi({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_wareHousePosi({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_wareHousePosi({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_wareHousePosi({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_wareHousePosi = function(options){
		if (!options) {
