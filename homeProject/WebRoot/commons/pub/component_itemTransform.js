(function($) {
	"use strict";
	//初始化仓库编号选择组件
	$.fn.openComponent_itemTransform = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_itemTransform_",
	    	blurUrl : ctx+"/admin/itemTransform/getItemTransform",
	    	blurCode : "no",
	    	blurDescr : "remarks",
	    	fetchUrl : ctx+"/admin/itemTransform/goCode",
	    	fetchCode : "no",
	    	fetchDescr : "remarks",
	    	fetchTitle : "选择转换编号"
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_itemTransform_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_itemTransform({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_itemTransform({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_itemTransform({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_itemTransform({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_itemTransform = function(options){
		if (!options) {
