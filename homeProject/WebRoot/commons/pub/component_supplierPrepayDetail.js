(function($) {
	"use strict";
	//初始化礼品领用单编号选择组件
	$.fn.openComponent_supplierPrepayDetail = function(options){
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_supplierPrepayDetail_",
	    	blurUrl : ctx+"/admin/supplierPrepayDetail/getSupplierPrepayDetail",
	    	blurCode : "pk",
	    	blurDescr : "no",
	    	fetchUrl : ctx+"/admin/supplierPrepayDetail/goCode",
	    	fetchCode : "pk",
	    	fetchDescr : "no",
	    	fetchTitle : "选择预付金单号",
	    	labelOnly : true,
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_supplierPrepayDetail_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_supplierPrepayDetail({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_supplierPrepayDetail({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_supplierPrepayDetail({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_supplierPrepayDetail({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_supplierPrepayDetail = function(options){
		if (!options) {
