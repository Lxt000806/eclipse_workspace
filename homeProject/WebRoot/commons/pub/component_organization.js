(function($) {
	"use strict";
	//初始化员工编号选择组件
	$.fn.openComponent_organization = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_organization_",
	    	blurUrl : ctx+"/admin/organization/getOrganization",
	    	blurCode : "orgid",
	    	blurDescr : "name",
	    	fetchUrl : ctx+"/admin/organization/goCode",
	    	fetchCode : "orgid",
	    	fetchDescr : "name",
	    	fetchTitle : "选择机构",
	    	valueOnly : false
	   ...
	   /* console.log($input.attr("id")+"*******");*/
	    ...
	    if(!$input1.attr("id")){ 
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_organization_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_organization({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_organization({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.orgId=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_organization({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_organization({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_organization = function(options){
