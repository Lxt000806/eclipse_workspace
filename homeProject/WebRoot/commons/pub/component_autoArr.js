(function($) {
	"use strict";
	//初始化autoArr选择组件
	$.fn.openComponent_autoArr = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_autoArr_",
	    	blurUrl : ctx+"/admin/autoArr/getAutoArr",
	    	blurCode : "PK",
	    	blurDescr : "LastUpdate",
	    	fetchUrl : ctx+"/admin/autoArr/goCode",
	    	fetchCode : "PK",
	    	fetchDescr :"LastUpdate",
	    	fetchTitle : "请选择批次号",
	    	valueOnly : false
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_autoArr_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_autoArr({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_autoArr({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_autoArr({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_autoArr({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_autoArr = function(options){
		if (!options) {
