(function($) {
	"use strict";
	//初始化发货时限选择组件
	$.fn.openComponent_intReplenishDT = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_intReplenishDT_",
	    	blurUrl : ctx+"/admin/intReplenishDT/goCodeJqGrid",
	    	blurCode : "pk",
	    	blurDescr : "remarks",
	    	fetchUrl : ctx+"/admin/intReplenishDT/goCode",
	    	fetchCode : "pk",
	    	fetchDescr :"remarks",
	    	fetchTitle : "选择补货单号"
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_intReplenishDT_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_intReplenishDT({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_intReplenishDT({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_intReplenishDT({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_intReplenishDT({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_intReplenishDT = function(options){
		if (!options) {
