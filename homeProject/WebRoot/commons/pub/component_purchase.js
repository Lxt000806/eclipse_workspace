(function($) {
	"use strict";
	//初始化Purchase选择组件
	$.fn.openComponent_purchase = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_purchase_",
	    	blurUrl : ctx+"/admin/purchase/getPurchase",
	    	blurCode : "no",
	    	blurDescr : "",
	    	fetchUrl : ctx+"/admin/purchase/goCode",
	    	fetchCode : "no",
	    	fetchDescr :"",
	    	fetchTitle : "选择采购单号",
	    	valueOnly : true
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_purchase_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_purchase({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_purchase({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_purchase({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_purchase({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_purchase = function(options){
		if (!options) {
