(function($) {
	"use strict";
	//初始化选择活动编号组件
	$.fn.openComponent_cmpactivity = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_cmpactivity_",
	    	blurUrl : ctx+"/admin/cmpActivity/getCmpActivity",
	    	blurCode : "no",
	    	blurDescr : "descr",
	    	fetchUrl : ctx+"/admin/cmpActivity/goCode",
	    	fetchCode : "no",
	    	fetchDescr : "descr",
	    	fetchTitle : "选择活动编号"
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_cmpactivity_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_cmpactivity({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_cmpactivity({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_cmpactivity({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_cmpactivity({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_cmpactivity = function(options){
		if (!options) {
