(function($) {
	"use strict";
	//初始化ticket选择组件
	$.fn.openComponent_baseItemPlanBak = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_baseItemPlanBak_",
	    	blurUrl : ctx+"/admin/baseItemPlanBak/getBaseItemPlanBak",
	    	blurCode : "no",
	    	blurDescr : "",
	    	fetchUrl : ctx+"/admin/baseItemPlanBak/goCode",
	    	fetchCode : "no",
	    	fetchDescr :"",
	    	fetchTitle : "选择基础备份编号",
	    	valueOnly : true
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_baseItemPlanBak_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_baseItemPlanBak({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_baseItemPlanBak({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_baseItemPlanBak({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_baseItemPlanBak({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_baseItemPlanBak = function(options){
		if (!options) {
	...
	$.fn.openComponent_baseItemPlanBakNo = function(options){
		var opt={
	    		url : ctx+"/admin/baseItemPlanBak/goJqGrid",
	    	    code : "No" 	
	 			};
		options=options||{};
	    if (!this.length) {
