(function($) {
	"use strict";
	//初始化员工编号选择组件
	$.fn.openComponent_position = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_position_",
	    	blurUrl : ctx+"/admin/position/getPosition",
	    	blurCode : "code",
	    	blurDescr : "desc2",
	    	fetchUrl : ctx+"/admin/position/goCode",
	    	fetchCode : "code",
	    	fetchDescr : "desc2",
	    	fetchTitle : "选择职位编号",
	    	valueOnly : false
	   ...
	   /* console.log($input.attr("id")+"*******");*/
	    ...
	    if(!$input1.attr("id")){ 
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_position_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_position({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_position({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_position({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_position({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_position = function(options){
