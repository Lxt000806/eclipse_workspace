(function($) {
	"use strict";
	//初始化company选择组件
	$.fn.openComponent_company = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_company_",
	    	blurUrl : ctx+"/admin/company/getcompany",
	    	blurCode : "Code",
	    	blurDescr : "Desc2",
	    	fetchUrl : ctx+"/admin/company/goCode",
	    	fetchCode : "Code",
	    	fetchDescr :"Desc2",
	    	fetchTitle : "搜寻-公司",
	    	dropDownUrl : ctx+"/admin/company/goJqGrid",
	    	dropDownCode : "Code",
	    	dropDownDescr : "Desc2",
	    	width: 300
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_company_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_company({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_company({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_company({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_company({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_company = function(options){
		if (!options) {
	...
	/*$.fn.suggestComponent_company = function(options){
		var opt={
