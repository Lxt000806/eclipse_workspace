(function($) {
	"use strict";
	//初始化材料编号选择组件
	//修改点：1、Component_customerOA全文替换，2、$.extend(options) 3、注释修改
	$.fn.openComponent_customerOA = function(options){
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_customerOA_",
	    	blurUrl : ctx+"/admin/customer/getCustomerForOA",
	    	blurCode : "code",
	    	blurDescr : "descr",
	    	fetchUrl : ctx+"/admin/customer/goCodeOA",
	    	fetchCode : "code",
	    	fetchDescr : "descr",
	    	fetchTitle : "选择客户编号",
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_customerOA_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_customerOA({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_customerOA({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,params,function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_customerOA({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_customerOA({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_customerOA = function(options){
		if (!options) {
