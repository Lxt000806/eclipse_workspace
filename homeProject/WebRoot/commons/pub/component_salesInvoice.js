(function($) {
	"use strict";
	//初始化salesInvoice选择组件
	$.fn.openComponent_salesInvoice = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_salesInvoice_",
	    	blurUrl : ctx+"/admin/salesInvoice/goJqGrid",
	    	blurCode : "no",
	    	blurDescr : "",
	    	fetchUrl : ctx+"/admin/salesInvoice/goCode",
	    	fetchCode : "no",
	    	fetchDescr :"",
	    	fetchTitle : "选择销售单号",
	    	valueOnly : true
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_salesInvoice_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_salesInvoice({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_salesInvoice({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_salesInvoice({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_salesInvoice({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_salesInvoice = function(options){
		if (!options) {
