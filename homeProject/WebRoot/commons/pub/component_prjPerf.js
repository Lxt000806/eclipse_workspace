(function($) {
	"use strict";
	//初始化prjPerf选择组件
	$.fn.openComponent_prjPerf = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_prjPerf_",
	    	blurUrl : ctx+"/admin/prjPerf/getPrjPerf",
	    	blurCode : "no",
	    	blurDescr : "",
	    	fetchUrl : ctx+"/admin/prjPerf/goCode",
	    	fetchCode : "no",
	    	fetchDescr :"",
	    	fetchTitle : "选择周期编号",
	    	valueOnly : true
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_prjPerf_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_prjPerf({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_prjPerf({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_prjPerf({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_prjPerf({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_prjPerf = function(options){
		if (!options) {
