(function($) {
	"use strict";
	//初始化资产类型选择组件
	$.fn.openComponent_assetType = function(options){
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_assetType_",
	    	blurUrl : ctx+"/admin/assetType/getAssetTypeBySql",
	    	blurCode : "code",
	    	blurDescr : "descr",
	    	fetchUrl : ctx+"/admin/assetType/goAssetType",
	    	fetchCode : "code",
	    	fetchDescr : "descr",
	    	fetchTitle : "资产类型编号",
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_assetType_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_assetType({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_assetType({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,params,function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_assetType({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_assetType({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_assetType = function(options){
		if (!options) {
