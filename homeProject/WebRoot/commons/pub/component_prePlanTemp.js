(function($) {
	"use strict";
	//初始化员工编号选择组件
	$.fn.openComponent_prePlanTemp = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_prePlanTemp_",
	    	blurUrl : ctx+"/admin/prePlanTemp/getPrePlanTempNo",
	    	blurCode : "no",
	    	blurDescr : "descr",
	    	fetchUrl : ctx+"/admin/prePlanTemp/goCode",
	    	fetchCode : "no",
	    	fetchDescr : "descr",
	    	fetchTitle : "选择预算模板号"
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_prePlanTemp_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_prePlanTemp({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_prePlanTemp({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_prePlanTemp({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_prePlanTemp({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_prePlanTemp = function(options){
