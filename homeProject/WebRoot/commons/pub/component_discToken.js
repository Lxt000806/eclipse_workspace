(function($) {
	"use strict";
	//初始化优惠券选择组件
	$.fn.openComponent_discToken = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_discToken_",
	    	blurUrl : ctx+"/admin/discTokenQuery/getDiscToken",
	    	blurCode : "no",
	    	blurDescr : "descr",
	    	fetchUrl : ctx+"/admin/discTokenQuery/goCode",
	    	fetchCode : "no",
	    	fetchDescr : "descr",
	    	fetchTitle : "选择优惠券"
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_discToken_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_discToken({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_discToken({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_discToken({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_discToken({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_discToken = function(options){
