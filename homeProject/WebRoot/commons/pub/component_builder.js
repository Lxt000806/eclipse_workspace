(function($) {
	"use strict";
	//初始化项目名称编号选择组件
	$.fn.openComponent_builder = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_builder_",
	    	blurUrl : ctx+"/admin/builder/getBuilder",
	    	blurCode : "code",
	    	blurDescr : "descr",
	    	fetchUrl : ctx+"/admin/builder/goCode",
	    	fetchCode : "code",
	    	fetchDescr : "descr",
	    	fetchTitle : "选择项目名称编号"
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_builder_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_builder({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_builder({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_builder({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_builder({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_builder = function(options){
		if (!options) {
