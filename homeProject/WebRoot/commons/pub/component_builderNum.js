(function($) {
	"use strict";
	//初始化builderNum选择组件
	$.fn.openComponent_builderNum = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_builderNum_",
	    	blurUrl : ctx+"/admin/builderNum/getBuilderNum",
	    	blurCode : "builderNum",
	    	blurDescr : "",
	    	fetchUrl : ctx+"/admin/builderNum/goCode",
	    	fetchCode : "BuilderNum",
	    	fetchDescr :"BuilderNum",
	    	fetchTitle : "",
	    	valueOnly : true
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_builderNum_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_builderNum({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_builderNum({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_builderNum({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_builderNum({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_builderNum = function(options){
		if (!options) {
	...
	/*$.fn.suggestComponent_builderNum = function(options){
		var opt={
