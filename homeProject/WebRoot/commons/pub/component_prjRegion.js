(function($) {
	"use strict";
	//初始化工程大区编号选择组件
	$.fn.openComponent_prjRegion = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_prjRegion_",
	    	blurUrl : ctx+"/admin/prjRegion/getPrjRegion",
	    	blurCode : "code",
	    	blurDescr : "descr",
	    	fetchUrl : ctx+"/admin/prjRegion/goCode",
	    	fetchCode : "code",
	    	fetchDescr : "descr",
	    	fetchTitle : "选择项目名称编号"
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_prjRegion_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_prjRegion({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_prjRegion({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_prjRegion({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_prjRegion({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_prjRegion = function(options){
		if (!options) {
