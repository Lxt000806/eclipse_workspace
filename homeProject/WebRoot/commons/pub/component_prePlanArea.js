(function($) {
	"use strict";
	//初始化装修区域选择组件
	$.fn.openComponent_prePlanArea = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_prePlanArea_",
	    	blurUrl : ctx+"/admin/prePlanArea/getPrePlanArea",
	    	blurCode : "pk",
	    	blurDescr : "descr",
	    	fetchUrl : ctx+"/admin/prePlanArea/goCode",
	    	fetchCode : "pk",
	    	fetchDescr : "descr",
	    	fetchTitle : "空间信息"
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_prePlanArea_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_prePlanArea({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_prePlanArea({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_prePlanArea({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_prePlanArea({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_prePlanArea = function(options){
	...
	$.fn.openComponent_prePlanAreaDescr = function(options){
		var opt={
	    		url : ctx+"/admin/prePlanArea/goJqGrid",
	    	    code : "descr" 	
	 			};
		options=options||{};
	    if (!this.length) {
