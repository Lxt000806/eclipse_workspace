(function($) {
	"use strict";
	//初始化员工编号选择组件
	$.fn.openComponent_fixArea = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_fixArea_",
	    	blurUrl : ctx+"/admin/fixArea/getFixArea",
	    	blurCode : "pk",
	    	blurDescr : "descr",
	    	fetchUrl : ctx+"/admin/fixArea/goCode",
	    	fetchCode : "PK",
	    	fetchDescr : "Descr",
	    	fetchTitle : "装修区域"
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_fixArea_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_fixArea({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_fixArea({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_fixArea({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_fixArea({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_fixArea = function(options){
	...
	$.fn.openComponent_fixAreaDescr = function(options){
		var opt={
