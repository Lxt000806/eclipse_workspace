(function($) {
	"use strict";
	//初始化礼品编号选择组件
	$.fn.openComponent_giftapp = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_giftapp_",
	    	blurUrl : ctx+"/admin/giftApp/getGiftApp",
	    	blurCode : "no",
	    	blurDescr : "",
	    	fetchUrl : ctx+"/admin/giftApp/goCode",
	    	fetchCode : "no",
	    	fetchDescr : "",
	    	fetchTitle : "选择领用单编号",
	    	valueOnly : true
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_giftapp_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_giftapp({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_giftapp({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_giftapp({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_giftapp({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_giftapp = function(options){
		if (!options) {
