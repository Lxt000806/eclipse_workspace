(function($) {
	"use strict";
	//初始化commiCycle选择组件
	$.fn.openComponent_commiCycle = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_commiCycle_",
	    	blurUrl : ctx+"/admin/commiCycle/getCommiCycle",
	    	blurCode : "no",
	    	blurDescr : "",
	    	fetchUrl : ctx+"/admin/commiCycle/goCode",
	    	fetchCode : "no",
	    	fetchDescr :"",
	    	fetchTitle : "选择周期编号",
	    	valueOnly : true
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_commiCycle_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_commiCycle({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_commiCycle({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_commiCycle({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_commiCycle({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_commiCycle = function(options){
		if (!options) {
