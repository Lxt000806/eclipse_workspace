(function($) {
	"use strict";
	//初始化workType2选择组件
	$.fn.openComponent_workType2 = function(options){	    
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_workType2_",
	    	blurUrl : ctx+"/admin/workType2/getWorkType2",
	    	blurCode : "code",
	    	blurDescr : "descr",
	    	fetchUrl : ctx+"/admin/workType2/goCode",
	    	fetchCode : "code",
	    	fetchDescr :"descr",
	    	fetchTitle : "选择工种分类2编号",
	    	valueOnly : false
	    });
	    ...
	    if(!$input1.attr("id")){
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_workType2_valid: true});
		    }
		    //添加事件
		    $input1.on("change",function(){
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_workType2({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_workType2({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
			    		...
			    		$input1.data("data-valid","false");
				    	$.post(options.blurUrl,{code:$.trim(inputVal)},function(ret){
				    		if(ret.code=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_workType2({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_workType2({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_workType2 = function(options){
		if (!options) {
