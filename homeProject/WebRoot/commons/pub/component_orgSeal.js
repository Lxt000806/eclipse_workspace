(function($) {
	"use strict";
	//初始化员工编号选择组件
	$.fn.openComponent_orgSeal = function(options){	   
	    if (!this.length) {
	        return this;
	    }
	    ...
	    $.extend(options,{
	    	preStr : "openComponent_orgSeal_",
	    	blurUrl : ctx+"/admin/orgSeal/getOrgSeal",
	    	blurCode : "sealid",
	    	blurDescr : "",
	    	fetchUrl : ctx+"/admin/orgSeal/goCode",
	    	fetchCode : "sealid",
	    	fetchDescr : "",
	    	fetchTitle : "选择机构印章",
	    	valueOnly : true
	   ...
	   /* console.log($input.attr("id")+"*******");*/
	    ...
	    if(!$input1.attr("id")){ 
		    ...
		    if(validator){
			    ...
			    $input1.rules("add", {openComponent_orgSeal_valid: true});
		    }
		    //添加事件
			    	...
			    	if(e.keyCode === 13){
						if(isSet){
							if (options.valueOnly){
								$input.setComponent_orgSeal({showValue:$.trim($input1.val()).split("|")[0]});
							}else{
								$input.setComponent_orgSeal({showLabel:$.trim($input1.val()).split("|")[1],
			    ...
			    if(options.isValidate){
			    	if($.trim($input1.val())!="" && !isSet){
				    	...
				    	$.post(options.blurUrl,{id:$.trim(inputVal)},function(ret){
				    		if(ret.orgId=='0'){
				    			if (options.valueOnly){
				    				$input.setComponent_orgSeal({showValue:ret.data[options.blurCode]});
				    			}else{
				    				$input.setComponent_orgSeal({showLabel:ret.data[options.blurDescr],
	...
	//设置回填组件值
	$.fn.setComponent_orgSeal = function(options){
