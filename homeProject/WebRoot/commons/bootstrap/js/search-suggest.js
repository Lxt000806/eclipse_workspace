...
(function($) {
    ...
    var methods = {
        ...
        init: function(opts) {
            ...
            var jebind = function(parent, a, ddiv, opts) {
                ...
                ddiv.find('.list').each(function() {
                    $(this).unbind('mouseover').mouseover(function() {
                        ...
                        if(opts.onSelect) onSelect(JSON.parse(getPointDescription($(this))));
                        ddiv.empty().hide();
                        parent.focus();
            ...
            var getPointId = function(p) {
                return p.find('div:first').attr("rel_id");
            };
            var getPointDescription = function(p) {
            ...
            var options = $.extend({
                ...
                width: 200,   /*提示框的宽度，缺省为输入框的宽度相同*/
                style: 'list',                   //风格样式，[line,list,default]
            ...
            return this.each(function() {
                ...
                $this.unbind('keydown').bind('keydown', function(event) {
                    if (dropDiv.css('display') !== 'none') { //当提示层显示时才对键盘事件处理
                        ...
                        if (event.keyCode === options.keyDown) { //如果按的是向下方向键
                            ...
                            if (! currentList.length) {
                        	  ...
                        	  if(opts.onSelect) onSelect(JSON.parse(getPointDescription($(currentList[0]))));
                        	  dropDiv.empty().hide();
                        } 
                   ...
                   options.timer= setTimeout(function(){ getData($.trim(word), $this, refreshDropDiv, dropDiv, options)}, options.delay); 
                    ...
                    //文本输入框失去焦点则清空并隐藏提示层
                    dropDiv.empty().hide();
                }).unbind('click').bind('click', function() { //单击输入框时，应显示提示
                    var word, words;
