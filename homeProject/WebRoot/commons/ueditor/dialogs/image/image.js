    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            uploader.on('uploadSuccess', function (file, ret) {
            	// modify by zzr 2018/10/16 根据返回参数修改 json.state -> json.datas.state begin
                var $file = $('#' + file.id);
                try {
                    var responseText = (ret._raw || ret),
                        json = utils.str2json(responseText);
                    if (json.datas.state == 'SUCCESS') {
                        _this.imageList.push(json);
                        $file.append('<span class="success"></span>');
                    } else {
                        $file.find('.error').text(json.datas.state).show();
                    }
                } catch (e) {
                    $file.find('.error').text(lang.errorServerUpload).show();
                }
            	// modify by zzr 2018/10/16 根据返回参数修改 json.state -> json.datas.state end
            });
        ...
        getInsertList: function () {
            ...
            for (i = 0; i < this.imageList.length; i++) {
                ...
                list.push({
                    src: prefix + data.datas.url,
                    _src: prefix + data.datas.url,
                    title: data.title,
                    alt: data.original,
                    floatStyle: align
                });
