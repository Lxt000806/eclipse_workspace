...
function formSubmit(){
	...
	var url = "<%=path %>/client/driverItemAppSend/itemAppSendUpload";
	ajaxGet(url);
}
</script>
</head>
<body bgcolor="#ffffff">
	<center>
		...
		<h1>到货图片上传接口App</h1>
		<h3>/client/driverItemAppSend/itemAppSendUpload</h3>
		<hr noshade size="3" width=80% />
		<%
			String respString = "";
			int port = request.getServerPort();
		%>
		<form id="uploadForm" method="post" enctype="multipart/form-data" action="<%=path %>/client/driverItemAppSend/itemAppSendUpload">
			<table border="1" align="center" bordercolordark="#EBEBEB"
				bordercolorlight="#ADAAAD">
				<tr>
					<td>图片类型type</td>
					<td colspan="2"><input type="text" name="type" value="0">Integer 1：送货到货图片 2：退货到货图片</td>
				</tr>
				<tr>
					<td></td>
