...
function formSubmit(){
	...
	var url = "<%=path %>/client/prjJob/uploadPrjJob";
	ajaxGet(url);
}
</script>
</head>
<body bgcolor="#ffffff">
	<center>
		...
		<h1>项目任务上传图片接口</h1>
		<h3>/client/prjJob/uploadPrjJob</h3>
		<hr noshade size="3" width=80% />
		<%
			String respString = "";
			int port = request.getServerPort();
		%>
		<form id="uploadForm" method="post" enctype="multipart/form-data" action="<%=path %>/client/prjJob/uploadPrjJob">
			<table border="1" align="center" bordercolordark="#EBEBEB"
				bordercolorlight="#ADAAAD">
				<tr>
					<td>图片类型type</td>
					<td colspan="2"><input type="text" name="type" value="">String</td>
				</tr>
				<tr>
					<td></td>
