<%@ page contentType="text/html; charset=UTF-8"%>
<%
String path = request.getContextPath();
String basePath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path+"/";
%>
<!DOCTYPE html>
<html>
<head>
<style type="text/css">
<!--
table {
	border: 0px double;
}
-->
</style>
<script type="text/javascript" src="${resourceRoot}/js/jquery.js"></script>
<script type="text/javascript" src="${resourceRoot}/js/test.js"></script>
<script type="text/javascript">
function formSubmit(){
	var datas = $("form").serialize();
	var url = "<%=path %>/client/prjProgConfirm/savePrjProgConfirm?"+datas;
	ajaxGet(url);
}
</script>
</head>
<body bgcolor="#ffffff">
	<center>
		<a href="../interfaces.jsp"><div style="color: red;">返回所有接口</div></a>
		<h1>工地验收保存接口</h1>
		<h3>/client/prjProgConfirm/savePrjProgConfirm</h3>
		<hr noshade size="3" width=80% />
		<%
			String respString = "";
			int port = request.getServerPort();
		%>
		<form>
			<table border="1" align="center" bordercolordark="#EBEBEB"
				bordercolorlight="#ADAAAD">
				<tr>
					<td>接入Portal类型portalType</td>
					<td><input type="text" name="portalType" value="2">String 2是验收，3是巡检</td>
				</tr>
				<tr>
					<td>客户编号custCode</td>
					<td><input type="text" name="custCode" value="">String</td>
				</tr>
				<tr>
					<td>施工项目prjItem</td>
					<td><input type="text" name="prjItem" value="">String</td>
				</tr>
				<tr>
					<td>验收说明remarks</td>
					<td><input type="text" name="remarks" value="">String</td>
				</tr>
				<tr>
					<td>gps地址address</td>
					<td><input type="text" name="address" value="">String</td>
				</tr>
				<tr>
					<td>是否通过isPass</td>
					<td><input type="text" name="isPass" value="">String 0：否  1：是</td>
				</tr>
				<tr>
					<td>图片列表photoNameList</td>
					<td><input type="text" name="photoNameList" value="">String多个用,号隔开</td>
				</tr>
				<tr>
					<td>验收级别prjLevel</td>
					<td><input type="text" name="prjLevel" value="1">String 1：合格  2：一级</td>
				</tr>
				<tr>
					<td>结束时间endDate</td>
					<td><input type="text" name="endDate" value="">Date</td>
				</tr>
				<tr>
					<td>访问页面标记fromPageFlag</td>
					<td><input type="text" name="fromPageFlag" value="">String:1:工人APP</td>
				</tr>
				<tr>
					<td></td>
					<td><input type="button" name="tijiao" value="提交" onclick="formSubmit()"><input
					type="reset" value="重填"></td>
				</tr>
			</table>
		</form>
		-----------------------------<br/>
		请求端口：<%=port%><br/>
		-----------------------------<br/>
		返回结果：<%=respString%><div id="respString"></div>
	</center>
</body>
</html>
