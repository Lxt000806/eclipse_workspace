...
function formSubmit(){
	...
	var url = "<%=path %>/client/prjProg/getPrjProgPhotoList?"+datas;
	ajaxGet(url);
}
function download(){
	var datas = $("form").serialize();
	var url = "<%=path %>/client/prjProg/downloadPrjProgPhotoList?"+datas;
	ajaxGet(url);
}
</script>
</head>
<body bgcolor="#ffffff">
	<center>
		...
		<h1>获取工程进度图片列表接口</h1>
		<h3>/client/prjProg/getPrjProgPhotoList</h3>
		...
		<form>
			<table border="1" align="center" bordercolordark="#EBEBEB"
				...
				<tr>
					<td>客户编号custCode</td>
					<td><input type="text" name="custCode" value="CT003449">String</td>
				</tr>
				<tr>
					<td>项目编号prjItem</td>
					<td><input type="text" name="prjItem" value="5">String</td>
				</tr>
				<tr>
					<td></td>
					<td><input type="button" name="tijiao" value="提交" onclick="formSubmit()">
					<input type="reset" value="重填">
					<input type="button" name="xiazai" value="下载" onclick="download()"></td>
				</tr>
			</table>
		</form>
		-----------------------------<br/>
