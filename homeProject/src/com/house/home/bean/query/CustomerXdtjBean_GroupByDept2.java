 ...
 * 客户信息查询bean
 */
public class CustomerXdtjBean_GroupByDept2 implements java.io.Serializable {
    ...
    @ExcelAnnotation(exportName="基础造价", order=3)
    private Double lineBaseFee;
    @ExcelAnnotation(exportName="管理费", order=4)
    private Double manageFee;
    @ExcelAnnotation(exportName="主材", order=5)
    private Double lineMainFee;
    @ExcelAnnotation(exportName="软装", order=6)
    private Double lineSoftFee;
    @ExcelAnnotation(exportName="集成", order=7)
    private Double lineIntegrateFee;
    @ExcelAnnotation(exportName="橱柜", order=8)
    private Double lineCupboardFee;
    @ExcelAnnotation(exportName="服务性产品", order=9)
    private Double mainServFee;
    @ExcelAnnotation(exportName="设计费", order=10)
    private Double designFee;
    @ExcelAnnotation(exportName="制图费", order=11)
    private Double drawFee;
    @ExcelAnnotation(exportName="效果图费", order=12)
    private Double colorDrawFee;
    @ExcelAnnotation(exportName="基础协议优惠", order=13)
    private Double baseDisc;
    @ExcelAnnotation(exportName="实物赠送", order=14)
    private Double gift;
    @ExcelAnnotation(exportName="工程造价", order=15)
    private Double contractFee;
    @ExcelAnnotation(exportName="业绩扣除数", order=16)
    private Double achievded;
    @ExcelAnnotation(exportName="业绩金额", order=17)
    private Double achievFee;
    @ExcelAnnotation(exportName="下定时间", pattern="yyyy-MM-dd", order=18)
	private Date setDate;
    @ExcelAnnotation(exportName="量房时间", pattern="yyyy-MM-dd", order=19)
	private Date measureDate;
    @ExcelAnnotation(exportName="平面时间", pattern="yyyy-MM-dd", order=20)
	private Date planeDate;
    @ExcelAnnotation(exportName="立面时间", pattern="yyyy-MM-dd", order=21)
	private Date facadeDate;
    @ExcelAnnotation(exportName="报价时间", pattern="yyyy-MM-dd", order=22)
	private Date priceDate;
    @ExcelAnnotation(exportName="效果图时间", pattern="yyyy-MM-dd", order=23)
	private Date drawDate;
    @ExcelAnnotation(exportName="放样时间", pattern="yyyy-MM-dd", order=24)
	private Date prevDate;
    @ExcelAnnotation(exportName="签单时间", pattern="yyyy-MM-dd", order=25)
