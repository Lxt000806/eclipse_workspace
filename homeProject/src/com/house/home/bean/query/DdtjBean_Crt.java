...
public class DdtjBean_Crt implements Serializable {
	private static final long serialVersionUID = 1L;

	@ExcelAnnotation(exportName="楼盘", order=1)
	private String address;
	@ExcelAnnotation(exportName="客户状态", order=2)
	private String statusDescr;
	@ExcelAnnotation(exportName="设计师", order=3)
	private String designManDescr;
	@ExcelAnnotation(exportName="业务员", order=4)
	private String businessManDescr;
	@ExcelAnnotation(exportName="创建时间",pattern="yyyy-MM-dd", order=5)
	private Date crtDate;
	@ExcelAnnotation(exportName="下定时间",pattern="yyyy-MM-dd", order=6)
	private Date setDate;
	@ExcelAnnotation(exportName="签单时间",pattern="yyyy-MM-dd", order=7)
	private Date signDate;
	@ExcelAnnotation(exportName="结束时间",pattern="yyyy-MM-dd", order=8)
	private Date endDate;
	@ExcelAnnotation(exportName="结束原因", order=9)
